...
async function applyBackground(theme: ThemeName) {
  ...
  try {
    ...
    if (url) {
      // setProperty(..., "important") beats every stylesheet rule
      // unconditionally, including any future ios27-style facelift pass
      // that hardens the decorative default gradient with !important -
      // a plain body.style.backgroundImage assignment would lose to that.
      body.style.setProperty("background-image", `linear-gradient(rgba(0,0,0,0.15), rgba(0,0,0,0.15)), url('${url}')`, "important");
