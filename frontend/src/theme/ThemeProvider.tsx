...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const channels = getFederatedSupabaseClients().map((client, index) =>
      client.channel(`dispatchops-appearance-live-${index}`)
        .on("postgres_changes", { event: "*", schema: "public", table: "appearance_config" }, () => {
          void applyBackground(resolvedTheme);
        })
        .subscribe()
    );
    return () => { channels.forEach((channel, index) => { void getFederatedSupabaseClients()[index]?.removeChannel(channel); }); };
  }, [resolvedTheme]);
