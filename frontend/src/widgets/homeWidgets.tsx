...
export function KpiTile({ icon, label, value, onClick }: { icon: string; label: string; value: string | number; onClick?: () => void }) {
  return (
      ...
      >
        <span className="flex h-8 w-8 items-center justify-center rounded-lg border border-[var(--glass-border)] bg-[var(--glass-bg-2)] text-[16px] backdrop-blur-[18px]">{icon}</span>
...
export function BoxesByDriverChart({ data, onBarClick }: { data: HomeDashboard; onBarClick?: (driverName: string) => void }) {
  ...
  return (
    <GlassPanel className="dashboard-chart-panel" title="📦 Boxes by Driver">
...
export function FacilityDistributionChart({ data }: { data: HomeDashboard }) {
  ...
  return (
    <GlassPanel className="dashboard-chart-panel" title="🏪 Facility Distribution">
...
export function VehicleTypeChart({ data }: { data: HomeDashboard }) {
  ...
  return (
    <GlassPanel className="dashboard-chart-panel" title="🚗 Units by Vehicle Type">
      <ResponsiveContainer width="100%" height={260}>
