...
export function KpiTile({ icon, label, value, onClick }: { icon: string; label: string; value: string | number; onClick?: () => void }) {
  ...
  if (onClick) {
    return (
      <button type="button" onClick={onClick} title="Click for details" className="glass-card relative flex w-full cursor-pointer flex-col gap-2 border text-left p-5 text-inherit">
        {content}
      </button>
    );
  }
  return <GlassCard padding="md" className="flex flex-col gap-2 text-left">{content}</GlassCard>;
}
