...
export function KpiTile({ icon, label, value, onClick }: { icon: string; label: string; value: string | number; onClick?: () => void }) {
  ...
  return (
    <motion.div {...fadeUp}>
      {onClick ? (
        <button type="button" onClick={onClick} title="Click for details" aria-label={`${label}: ${value}. Click for details`} className="dashboard-kpi-button glass-card flex w-full flex-col gap-2 p-5 text-left">
          {content}
        </button>
      ) : (
        <div className="glass-card flex flex-col gap-2 p-5 text-left">{content}</div>
      )}
    </motion.div>
  );
}
                ...
                <GlassTable.Td>
                  <button
                    ...
                    className={
                      ...
                      (driver === d.driver_name ? "text-[var(--purple)]" : "text-ink")
                    }
