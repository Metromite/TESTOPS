...
export async function exportPriceChangeExcel(campaigns:Campaign[],items:Item[],assignments:Assignment[],visits:Visit[],drivers:any[]=[],listTitle?:string,onProgress?:(p:ExportProgress)=>void){
  // Paint the progress UI before any heavy workbook/attachment work begins.
  onProgress?.({pct:3,status:'preparing',message:'Starting download…'});
  ...
  // Fetch evidence concurrently in small batches. This preserves every attachment
  // while avoiding one-network-request-at-a-time export delays.
  const attachmentJobs: { path:string; kind:'photo'|'signature' }[] = [];
  for (const v of visits) {
    if (v.attachment_path) attachmentJobs.push({path:v.attachment_path, kind:'photo'});
    if (v.signature_path) attachmentJobs.push({path:v.signature_path, kind:'signature'});
  }
  const concurrency = 6;
  let completed = 0;
  for (let i=0; i<attachmentJobs.length; i+=concurrency) {
    const batch=attachmentJobs.slice(i,i+concurrency);
    await Promise.all(batch.map(async j => {
      await offlineRef(j.path,j.kind);
      completed++;
      onProgress?.({pct:Math.min(58,8+Math.round((completed/Math.max(1,attachmentJobs.length))*50)),status:'preparing',message:`Loading attachments ${completed}/${attachmentJobs.length}…`});
    }));
    await new Promise<void>(resolve => setTimeout(resolve,0));
  }
  onProgress?.({pct:60,status:'preparing',message:'Building Excel workbook…'});
  await new Promise<void>(resolve => requestAnimationFrame(() => resolve()));
  ...
  for(const v of visits){
    ...
    const photo=v.attachment_path?attachmentCache.get(v.attachment_path)||'':'';
    const sig=v.signature_path?attachmentCache.get(v.signature_path)||'':'';
  ...
  onProgress?.({pct:65,status:'preparing',message:'Creating Excel file…'});
  await new Promise<void>(resolve => requestAnimationFrame(() => resolve()));
  // XLSX is immediately packed into the final ZIP below; disabling the nested
  // XLSX compression avoids compressing the same data twice and speeds exports.
  const excelBase64=XLSX.write(wb,{bookType:'xlsx',type:'base64',compression:false});
