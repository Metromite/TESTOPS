...
export async function saveDefaultArea(x:{id?:string;area_name:string;sort_order?:number;active?:boolean}){return rpc<any>('dispatchops_price_change_admin_save_default_area',{p_key:await adminKey(),p_row:x});}
export async function deleteDefaultArea(id:string){return rpc<boolean>('dispatchops_price_change_admin_delete_default_area',{p_key:await adminKey(),p_id:id});}
