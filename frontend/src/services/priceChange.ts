...
export async function listDefaultPriceChangeAreas(){ const {data,error}=await db().from('price_change_default_areas').select('id,area_name,sort_order,active').eq('active',true).order('sort_order',{ascending:true}).order('area_name',{ascending:true}); if(error) throw new Error(error.message); return data||[]; }
export async function saveDefaultPriceChangeArea(x:{id?:string;area_name:string;sort_order?:number;active?:boolean}){return rpc<any>('dispatchops_price_change_admin_save_default_area',{p_key:await adminKey(),p_row:x});}
