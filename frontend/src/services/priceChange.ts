...
async function getEvidenceSignedUrl(path:string):Promise<string>{
  ...
  let r:Response;
  try {
    r=await fetch(baseUrl+'/functions/v1/dispatchops-price-change-upload',{method:'POST',headers:{apikey:publishableKey,'Content-Type':'application/json'},body:JSON.stringify({action:'sign',admin_key:key,path,expires_in:60*60*24*30}),signal:controller.signal});
  } catch (e:any) {
    if(e?.name==='AbortError') throw new Error('Evidence link request timed out. Please try again.');
    throw e;
  } finally {
