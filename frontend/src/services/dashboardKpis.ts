 ...
 */
async function getHomeDetailRowsFast(metric: string, f: FilterParams, onProgress?: (rows: Record<string, any>[], totalMatching: number, done: boolean) => void): Promise<HomeDetailResponse> {
  const PAGE = 1000;
  const [areas, fleetDriverNames, configRules, fleetVehicles] = await Promise.all([
  ...
  for (let from = 0; ; from += PAGE) {
    ...
    // Progressive detail rendering: publish the first matching batch before
    // continuing the full scan. This lets DetailWindow paint immediately
    // instead of keeping the UI in a loading state until the entire dataset
    // has been scanned. The final callback still supplies the exact total.
    if (onProgress && keep.length) {
      const previewColumns = ["invoice_no","dispatch_date","invoice_date","driver_name","helper_name","vehicle_num","area","division_desc","customer_name","boxes","normal_boxes","freezer_boxes","not_supplied_reason"];
      const previewRows = keep.slice(0, 100).map((r) => Object.fromEntries(previewColumns.map((c) => [c, r[c]])));
      onProgress(previewRows, totalMatching, false);
      onProgress = undefined;
    }

    if (typeof window !== "undefined") await new Promise<void>((resolve) => setTimeout(resolve, 0));
    if (page.length < PAGE && ((data ?? []).length < PAGE)) break;
  }
  ...
  if (metric === "active_drivers" || metric === "unique_customers" || metric === "active_vehicles") {
    ...
    return { metric, columns: ["name","invoices","boxes","not_supplied"], rows: entityRows, truncated: totalMatching > DETAIL_ROW_LIMIT, total_matching: totalMatching };
  }
  ...
  return { metric, columns, rows: sorted.slice(0, DETAIL_ROW_LIMIT).map((r) => Object.fromEntries(columns.map((c) => [c, r[c]]))), truncated: totalMatching > DETAIL_ROW_LIMIT, total_matching: totalMatching };
}
...
export async function getHomeDetailRows(metric: string, f: FilterParams, onProgress?: (rows: Record<string, any>[], totalMatching: number, done: boolean) => void): Promise<HomeDetailResponse> {
  return getHomeDetailRowsFast(metric, f, onProgress);
}
