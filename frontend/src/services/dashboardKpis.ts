...
function mergeHomeSnapshots(parts: any[]): HomeDashboard {
  ...
  const facilityMap=new Map<string,{value:number;labels:string[]}>();
  const vtMap=new Map<string,{normal:number;freezer:number;label:string}>();
  ...
  for(const p of parts){
    ...
    const fc=p?.charts?.facility; for(let i=0;i<(fc?.labels||[]).length;i++){const raw=String(fc.labels[i]||"").trim();if(!raw)continue;const label=normalizeDimension(raw);const existing=facilityMap.get(label)||{value:0,labels:[]};existing.value+=num(fc.values?.[i]);existing.labels.push(raw);facilityMap.set(label,existing);}
    const vc=p?.charts?.vantype; for(let i=0;i<(vc?.labels||[]).length;i++){const raw=String(vc.labels[i]||"").trim();const label=normalizeDimension(raw);const x=vtMap.get(label)||{normal:0,freezer:0,label:raw};x.normal+=num(vc.normal?.[i]);x.freezer+=num(vc.freezer?.[i]);if(raw && raw !== raw.toUpperCase() && x.label === x.label.toUpperCase()) x.label=raw;vtMap.set(label,x);}
  }
  const activeDrivers=drivers.length;
  const facilityKeys=[...facilityMap.keys()].sort(); const facilityLabels=facilityKeys.map(k=>canonicalLabel(facilityMap.get(k)?.labels||[k])); const vtKeys=[...vtMap.keys()].sort();
  const vtLabels=vtKeys.map(k=>vtMap.get(k)?.label || k);
  return {
    ...
    charts:{driver_boxes:{labels:drivers.map(r=>r.driver_name),values:drivers.map(r=>num(r.boxes))},facility:{labels:facilityLabels,values:facilityKeys.map(x=>facilityMap.get(x)?.value||0)},vantype:{labels:vtLabels,normal:vtKeys.map(x=>vtMap.get(x)?.normal||0),freezer:vtKeys.map(x=>vtMap.get(x)?.freezer||0)},returns:{delivered:valid-notSup,not_supplied:notSup}}
  };
}

function canonicalizeHomeSnapshotToFleet(snapshot: HomeDashboard, fleetVehicles: Map<string, { number: string; type: string }>): HomeDashboard {
  // Fleet Database is the single source of truth for vehicle-type labels.
  // Normalize only for matching/grouping; display the exact Fleet spelling.
  const fleetTypeByKey = new Map<string, string>();
  for (const v of fleetVehicles.values()) {
    const t = String(v.type || "").trim();
    if (t) fleetTypeByKey.set(normalizeDimension(t), t);
  }

  const typeKeyToFleetLabel = new Map<string, string>();
  for (const label of snapshot.charts?.vantype?.labels || []) {
    const key = normalizeDimension(label);
    if (fleetTypeByKey.has(key)) typeKeyToFleetLabel.set(key, fleetTypeByKey.get(key)!);
  }

  const vt = snapshot.charts?.vantype;
  if (!vt) return snapshot;
  const grouped = new Map<string, { label: string; normal: number; freezer: number }>();
  for (let i = 0; i < (vt.labels || []).length; i++) {
    const raw = String(vt.labels[i] || "").trim();
    if (!raw) continue;
    const key = normalizeDimension(raw);
    const label = typeKeyToFleetLabel.get(key) || raw;
    const cur = grouped.get(key) || { label, normal: 0, freezer: 0 };
    cur.normal += Number(vt.normal?.[i] || 0);
    cur.freezer += Number(vt.freezer?.[i] || 0);
    // If Fleet has this type, always use its exact current spelling.
    if (fleetTypeByKey.has(key)) cur.label = fleetTypeByKey.get(key)!;
    grouped.set(key, cur);
  }
  ...
  return {
    ...
    charts: {
      ...snapshot.charts,
      vantype: {
        labels: keys.map(k => grouped.get(k)!.label),
        normal: keys.map(k => grouped.get(k)!.normal),
        freezer: keys.map(k => grouped.get(k)!.freezer),
      },
    },
  };
}
...
async function fetchHomeDashboardRpc(f: FilterParams): Promise<HomeDashboard> {
  ...
  const promise=(async()=>{
    ...
    const merged = mergeHomeSnapshots(parts);
    const fleetVehicles = await fetchFleetVehicleMap();
    return canonicalizeHomeSnapshotToFleet(merged, fleetVehicles);
  })();
...
/** Dashboard dimensions are data-driven, not fixed enums. Normalize only for
 * grouping/matching so VAN/Van/van remain one category while the displayed
 * label comes from the authoritative Fleet value when available. */
function normalizeDimension(v: unknown): string {
  return String(v ?? "").trim().toLowerCase().replace(/[^a-z0-9]+/g, "");
}

function canonicalLabel(values: string[]): string {
  return values.map(v => String(v || "").trim()).filter(Boolean).sort((a,b) => {
...
function wantedFacilityMatch(value: unknown, wantedValues: string[]): boolean {
  const wanted = new Set(wantedValues.map(normalizeDimension));
  return wanted.has(normalizeDimension(value));
}
...
function applyGlobalFilters(rows: SapRow[], f: FilterParams, areas: AreaRow[]): SapRow[] {
  ...
  if (driverList.length) { const wanted = new Set(driverList.map(normalizeDimension)); out = out.filter((r) => wanted.has(normalizeDimension(r.driver_name))); }
  const areaList = csvList(f.areas);
  if (areaList.length) { const wanted = new Set(areaList.map(normalizeDimension)); out = out.filter((r) => wanted.has(normalizeDimension(r.area))); }
  const divisionList = csvList(f.division);
  if (divisionList.length) { const wanted = new Set(divisionList.map(normalizeDimension)); out = out.filter((r) => wanted.has(normalizeDimension(r.division_desc))); }
  const facilityList = csvList(f.facility_type);
  if (facilityList.length) out = out.filter((r) => wantedFacilityMatch(r.facility_type, facilityList));
  const salesmanList = csvList(f.salesman);
  if (salesmanList.length) { const wanted = new Set(salesmanList.map(normalizeDimension)); out = out.filter((r) => wanted.has(normalizeDimension(r.salesman))); }
  ...
  if (routeTypes.size || vehicleTypes.size) {
    const wantedRoutes = new Set([...routeTypes].map(normalizeDimension));
    const wantedVehicles = new Set([...vehicleTypes].map(normalizeDimension));
    const areaRoute = new Map(areas.map(a => [normalizeDimension(a.name), normalizeDimension(a.route_type)]));
    out = out.filter((r) => {
      const vehicleOk = !wantedVehicles.size || wantedVehicles.has(normalizeDimension(r.vehicle_type));
      const routeOk = !wantedRoutes.size || wantedRoutes.has(areaRoute.get(normalizeDimension(r.area)) || "");
      return vehicleOk && routeOk;
    });
  }
  return out;
}
...
async function buildFilteredSapRows(f: FilterParams): Promise<SapRow[]> {
  ...
  for (let from = 0; ; from += PAGE) {
    ...
    allRows.push(...page);
    // Large dashboards can contain many pages. Yield to the browser between
    // pages so opening a KPI detail window can paint its loading state and
    // never monopolizes the main thread.
...
export async function computeHomeDashboard(f: FilterParams): Promise<HomeDashboard> {
  // Fleet is the authoritative source for vehicle identity/type labels. Fetch it
  // before building any chart so every vehicle-type chart uses the current data.
  const fleetVehicles = await fetchFleetVehicleMap();
  ...
  // Every chart dimension is dynamic data, never a fixed enum. Group values
  // case/spacing-insensitively, while choosing one canonical display label.
  const facilityCounts = new Map<string, { value: number; labels: string[] }>();
  for (const r of sapRows) {
    const raw = String(r.facility_type || "").trim();
    if (!raw) continue;
    const key = normalizeDimension(raw);
    const x = facilityCounts.get(key) || { value: 0, labels: [] };
    x.value += 1; x.labels.push(raw); facilityCounts.set(key, x);
  }
  const facilityKeys = [...facilityCounts.keys()].sort((a,b) => a.localeCompare(b));
  const facilityChart = {
    labels: facilityKeys.map(k => canonicalLabel(facilityCounts.get(k)?.labels || [k])),
    values: facilityKeys.map(k => facilityCounts.get(k)?.value || 0),
  };

  const vantypeNormal = new Map<string, number>();
  const vantypeFreezer = new Map<string, number>();
  const vantypeLabels = new Map<string, string[]>();
  for (const r of sapRows) {
    const raw = String(r.vehicle_type || "").trim();
    if (!raw) continue;
    const key = normalizeDimension(raw);
    vantypeNormal.set(key, (vantypeNormal.get(key) ?? 0) + (r.normal_boxes || 0));
    vantypeFreezer.set(key, (vantypeFreezer.get(key) ?? 0) + (r.freezer_boxes || 0));
    const labels = vantypeLabels.get(key) || []; labels.push(raw); vantypeLabels.set(key, labels);
  }
  const fleetTypeByKey = new Map<string, string>();
  for (const v of fleetVehicles.values()) {
    const t = String(v.type || "").trim();
  ...
  const vanKeys = [...new Set([...vantypeNormal.keys(), ...vantypeFreezer.keys()])].sort();
  const vanTypes = vanKeys.map(k => fleetTypeByKey.get(k) || canonicalLabel(vantypeLabels.get(k) || [k]));
  const vantypeChart = {
    labels: vanTypes,
    normal: vanKeys.map((v) => vantypeNormal.get(v) ?? 0),
    freezer: vanKeys.map((v) => vantypeFreezer.get(v) ?? 0),
  };
...
/**
 * Detail-only fetch path. KPI detail dialogs must never build an unbounded
 * in-memory copy of an entire month's SAP dataset. We still scan pages so
 * total_matching remains exact, but retain only the first DETAIL_ROW_LIMIT
 * matching rows needed by the dialog. Filtering/mapping is performed per
 * page and the browser yields between pages.
 */
async function getHomeDetailRowsFast(metric: string, f: FilterParams): Promise<HomeDetailResponse> {
  // Detail windows must never make the browser wait while paging through the
  // entire historical SAP table. The old implementation did exactly that,
  // which is why clicking a KPI could appear to freeze the page. Keep the
  // established popup, but feed it a bounded, immediately useful result set.
  const PAGE = 1000;
  const MAX_DETAIL_ROWS = 10000;
  const selectColumns = "invoice_no,invoice_date,dispatch_date,driver_name,helper_name,vehicle_num,vehicle_key,vehicle_type,area,division_desc,customer_name,boxes,normal_boxes,freezer_boxes,not_supplied_reason";
  const [areas, fleetDriverNames, configRules, fleetVehicles] = await Promise.all([
    fetchValidAreas(), fetchFleetDriverNames(), fetchConfigRules(), fetchFleetVehicleMap(),
  ]);
  const keep: SapRow[] = [];
  let totalMatching = 0;
  let from = 0;

  // Read a small bounded window. If it reaches the cap, mark the result as
  // truncated so the existing search/filter UI remains honest.
  while (from < MAX_DETAIL_ROWS) {
    const to = Math.min(from + PAGE - 1, MAX_DETAIL_ROWS - 1);
    let q = supabase.from("sap_invoice_facts").select(selectColumns);
    if (f.start_date) q = q.gte("dispatch_date", f.start_date);
    if (f.end_date) q = q.lte("dispatch_date", f.end_date);
    if (f.driver) q = q.eq("driver_name", f.driver);
    const { data, error } = await q.range(from, to);
    if (error) throw new Error(error.message);
    const raw = (data ?? []) as SapRow[];
    let page = raw.map((r) => {
      const fleetKey = normalizeVehicle(r.vehicle_num || r.vehicle_key);
      const fleet = fleetVehicles.get(fleetKey);
      return fleet ? { ...r, vehicle_num: fleet.number, vehicle_key: fleetKey, vehicle_type: fleet.type || r.vehicle_type } : r;
    });

    page = applyGlobalFilters(page, f, areas);
    page = applyDashboardConfigExclusions(page, configRules, areas);
    if (fleetDriverNames.size) page = page.filter((r) => fleetDriverNames.has(r.driver_name));
    const validAreaNames = new Set(areas.map((a) => a.name));
    if (validAreaNames.size) page = page.filter((r) => validAreaNames.has(r.area));

    if (metric === "not_supplied") page = page.filter((r) => r.not_supplied_reason);
    else if (metric === "freezer_boxes") page = page.filter((r) => (r.freezer_boxes || 0) > 0);
    else if (metric === "normal_boxes") page = page.filter((r) => (r.normal_boxes || 0) > 0);

    totalMatching += page.length;
    if (keep.length < DETAIL_ROW_LIMIT) keep.push(...page.slice(0, DETAIL_ROW_LIMIT - keep.length));
  ...
  if (metric === "active_drivers" || metric === "unique_customers" || metric === "active_vehicles") {
    const keyFn: Record<string, (r: SapRow) => string> = {
      active_drivers: (r) => r.driver_name, unique_customers: (r) => r.customer_name, active_vehicles: (r) => r.vehicle_key,
    };
    const agg = new Map<string, { name: string; invoices: number; boxes: number; not_supplied: number }>();
    for (const r of keep) {
      const k = keyFn[metric](r); if (!k) continue;
      let e = agg.get(k);
      if (!e) { e = { name: k, invoices: 0, boxes: 0, not_supplied: 0 }; agg.set(k, e); }
      e.invoices += 1; e.boxes += r.boxes || 0; e.not_supplied += r.not_supplied_reason ? 1 : 0;
    }
    const entityRows = [...agg.values()].sort((a,b) => b.invoices-a.invoices).slice(0, DETAIL_ROW_LIMIT);
    return { metric, columns: ["name","invoices","boxes","not_supplied"], rows: entityRows, truncated: totalMatching > DETAIL_ROW_LIMIT || from >= MAX_DETAIL_ROWS, total_matching: totalMatching };
  }

  const sorted = [...keep].sort((a,b) => (b.dispatch_date || "").localeCompare(a.dispatch_date || ""));
  ...
  return { metric, columns, rows: sorted.slice(0, DETAIL_ROW_LIMIT).map((r) => Object.fromEntries(columns.map((c) => [c, r[c]]))), truncated: totalMatching > DETAIL_ROW_LIMIT || from >= MAX_DETAIL_ROWS, total_matching: totalMatching };
}

/** Direct port of get_home_detail_rows(), optimized so a detail click cannot
 * freeze the UI while constructing a huge client-side array. */
export async function getHomeDetailRows(metric: string, f: FilterParams): Promise<HomeDetailResponse> {
  return getHomeDetailRowsFast(metric, f);
}
