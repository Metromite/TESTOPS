...
async function buildFilteredSapRows(f: FilterParams): Promise<SapRow[]> {
  ...
  const [areas, fleetDriverNames, configRules] = await Promise.all([
    fetchValidAreas(),
...
export async function getFilterOptions(): Promise<FilterOptions> {
  // Always federate through the paginated data layer. The previous RPC path
  // only knew about the primary project and therefore hid secondary-project
  // invoices from slicers.
  const rows: { division_desc: string; facility_type: string; salesman: string; area: string; driver_name: string }[] = [];
  const PAGE = 1000;
  for (let from = 0; ; from += PAGE) {
    const { data: page, error: pageErr } = await supabase
      .from("sap_invoice_facts")
      .select("division_desc,facility_type,salesman,area,driver_name")
  ...
  return {
    divisions: distinctNonEmpty(rows.map((r) => r.division_desc).filter((v) => !isUnknownDimension(v))),
    facility_types: distinctNonEmpty(rows.map((r) => r.facility_type).filter((v) => !isUnknownDimension(v))),
    salesmen: distinctNonEmpty(rows.map((r) => r.salesman)),
    areas: distinctNonEmpty(rows.map((r) => r.area)),
    drivers: distinctNonEmpty(rows.map((r) => r.driver_name)),
  };
}
