...
export async function computeHomeDashboard(f: FilterParams): Promise<HomeDashboard> {
  ...
  // buildFilteredSapRows() has already resolved each row's vehicle type from
  // Fleet Database. At this stage we only normalize for grouping; the label
  // is selected from the actual Fleet-resolved row values. Never depend on a
  // separate fixed vehicle-type list here.
  const vanKeys = [...new Set([...vantypeNormal.keys(), ...vantypeFreezer.keys()])].sort();
  const vanTypes = vanKeys.map(k => canonicalLabel(vantypeLabels.get(k) || [k]));
  const vantypeChart = {
    labels: vanTypes,
