...
function applyGlobalFilters(rows: SapRow[], f: FilterParams, areas: AreaRow[]): SapRow[] {
  let out = rows.filter((r) => !isUnknownDimension(r.division_desc) && hasValidDashboardClassification(r as SapRow));
...
async function getHomeDetailRowsFast(metric: string, f: FilterParams, onProgress?: (rows: Record<string, any>[], totalMatching: number, done: boolean) => void): Promise<HomeDetailResponse> {
  // IMPORTANT: KPI details must never perform a full-table client-side scan.
  // The old path fetched every page, applied all dimension rules in JS, and
  // only then completed the dialog. On large SAP tables that monopolised the
  // browser long enough to look like a freeze. The detail window is a user
  // interaction, so it gets a bounded server-side preview immediately.
  const PAGE = 5000;
  const columns = "invoice_no,invoice_date,dispatch_date,driver_name,helper_name,vehicle_num,vehicle_key,vehicle_type,customer_name,area,division_desc,facility_type,salesman,boxes,normal_boxes,freezer_boxes,not_supplied_reason";
  let q: any = supabase.from("sap_invoice_facts").select(columns, { count: "exact" });
  if (f.start_date) q = q.gte("dispatch_date", f.start_date);
  if (f.end_date) q = q.lte("dispatch_date", f.end_date);
  const drivers = csvList(f.drivers);
  if (drivers.length) q = q.in("driver_name", drivers);
  else if (f.driver) q = q.eq("driver_name", f.driver);
  const areas = csvList(f.areas); if (areas.length) q = q.in("area", areas);
  const divisions = csvList(f.division); if (divisions.length) q = q.in("division_desc", divisions);
  const facilities = csvList(f.facility_type); if (facilities.length) q = q.in("facility_type", facilities);
  const vehicleTypes = csvList(f.vehicle_type); if (vehicleTypes.length) q = q.in("vehicle_type", vehicleTypes);
  const salesmen = csvList(f.salesman); if (salesmen.length) q = q.in("salesman", salesmen);
  if (metric === "not_supplied") q = q.not("not_supplied_reason", "is", null).neq("not_supplied_reason", "");
  if (metric === "freezer_boxes") q = q.gt("freezer_boxes", 0);
  if (metric === "normal_boxes") q = q.gt("normal_boxes", 0);
  const { data, error, count } = await q.order("dispatch_date", { ascending: false }).range(0, PAGE - 1);
  if (error) throw new Error(error.message);
  const rows = ((data ?? []) as Record<string, any>[]).filter((r) =>
    !isUnknownDimension(r.division_desc) && hasValidDashboardClassification(r as SapRow)
  );
  const total = typeof count === "number" ? count : rows.length;
  const preview = rows;
  if (onProgress) onProgress(preview, total, true);
  return {
    metric,
    columns: ["invoice_no","dispatch_date","invoice_date","driver_name","helper_name","vehicle_num","area","division_desc","customer_name","boxes","normal_boxes","freezer_boxes","not_supplied_reason"],
    rows: preview,
