...
/**
 * KPI detail data. Keep the original dashboard semantics (all matching rows,
 * aggregation for entity KPIs, newest invoices first) but page the Supabase
 * reads and yield to the browser between pages. This is intentionally based
 * on select("*"), like the original implementation, so a UI-only redesign
 * cannot break the dialog because a column name changed in the database.
 */
async function getHomeDetailRowsFast(
  metric: string,
  f: FilterParams,
  onProgress?: (rows: Record<string, any>[], totalMatching: number, done: boolean) => void
): Promise<HomeDetailResponse> {
  const PAGE = 500;
  const allRows: SapRow[] = [];

  for (let from = 0; ; from += PAGE) {
    let q: any = supabase.from("sap_invoice_facts").select("*");
    if (f.start_date) q = q.gte("dispatch_date", f.start_date);
    if (f.end_date) q = q.lte("dispatch_date", f.end_date);
    ...
    const { data, error } = await q.order("dispatch_date", { ascending: false }).range(from, from + PAGE - 1);
    if (error) throw new Error(error.message);

    const pageRows = ((data ?? []) as SapRow[]).filter((r) =>
      !isUnknownDimension(r.division_desc) && hasValidDashboardClassification(r)
    );
    allRows.push(...pageRows);

    if (!data || data.length < PAGE) break;
    // Give React/browser a real chance to paint the dialog and respond to input.
  ...
  if (metric === "not_supplied") {
    rows = rows.filter((r) => !!r.not_supplied_reason);
  } else if (metric === "freezer_boxes") {
    rows = rows.filter((r) => (r.freezer_boxes || 0) > 0);
  } else if (metric === "normal_boxes") {
    rows = rows.filter((r) => (r.normal_boxes || 0) > 0);
  }
  ...
  if (metric === "active_drivers" || metric === "unique_customers" || metric === "active_vehicles") {
    const keyFn: Record<string, (r: SapRow) => string> = {
      ...
      active_vehicles: (r) => r.vehicle_key || r.vehicle_num,
    };
    ...
    for (const r of rows) {
      ...
      let e = agg.get(k);
      if (!e) {
        e = { name: k, invoices: 0, boxes: 0, not_supplied: 0 };
        agg.set(k, e);
      }
      e.invoices += 1;
      e.boxes += Number(r.boxes || 0);
      e.not_supplied += r.not_supplied_reason ? 1 : 0;
    }

    const totalMatching = agg.size;
    const entityRows = [...agg.values()]
      .sort((a, b) => b.invoices - a.invoices)
      .slice(0, DETAIL_ROW_LIMIT);

    onProgress?.(entityRows, totalMatching, true);
    return {
      metric,
      columns: ["name", "invoices", "boxes", "not_supplied"],
      rows: entityRows,
      truncated: totalMatching > DETAIL_ROW_LIMIT,
  ...
  const sorted = rows
    .slice(0, DETAIL_ROW_LIMIT) // query is already newest-first
    .map((r) => ({
      ...
      area: r.area,
      division_desc: r.division_desc,
      customer_name: r.customer_name,
      boxes: r.boxes,
      normal_boxes: r.normal_boxes,
      freezer_boxes: r.freezer_boxes,
      not_supplied_reason: r.not_supplied_reason,
    }));

  onProgress?.(sorted, totalMatching, true);
  return {
    metric,
...
export async function getHomeDetailRows(
  metric: string,
  f: FilterParams,
  onProgress?: (rows: Record<string, any>[], totalMatching: number, done: boolean) => void
): Promise<HomeDetailResponse> {
