...
export async function loadBulkOrganizerDefaults(monthKey?: string): Promise<BulkOrganizerDefaults | null> {
  const query = supabase.from("bulk_organizer_defaults").select("*").eq("id", 1);
  const { data, error } = await (monthKey ? query.eq("month_key", monthKey) : query).maybeSingle();
  if (error) {
  ...
  return data ? { ...data, id: String(data.id) } as BulkOrganizerDefaults : null;
}

export async function saveBulkOrganizerDefaults(value: BulkOrganizerDefaults): Promise<BulkOrganizerDefaults> {
  const payload = { id: 1, month_key: value.month_key || new Date().toISOString().slice(0,7) + "-01", buildings: value.buildings || [], vehicles: value.vehicles || [], customer_schedules: value.customer_schedules || [], updated_at: new Date().toISOString() };
  ...
  if (error) {
    if (error.code === "PGRST205" || /bulk_organizer_defaults/i.test(error.message)) {
      ...
      return { ...payload, id: String(payload.id) } as BulkOrganizerDefaults;
    }
    throw new Error(error.message);
  }
  localStorage.setItem("dispatchops.bulkOrganizer.defaults", JSON.stringify(data));
  return { ...data, id: String(data.id) } as BulkOrganizerDefaults;
}
