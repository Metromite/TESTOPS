...
export async function getAppearanceConfig(): Promise<AppearanceConfig | null> {
  ...
  const results = await Promise.all(clients.map((client) => client
    ...
    .maybeSingle()));
  const firstData = results.find((r) => r.data)?.data;
  if (firstData) return firstData as AppearanceConfig;
  const firstError = results.find((r) => r.error)?.error;
  if (firstError) throw new Error(`Failed to load appearance config: ${firstError.message}`);
  return null;
}
