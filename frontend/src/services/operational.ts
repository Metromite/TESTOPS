...
export async function getDiagnostics<T>():Promise<T>{
  ...
  // Diagnostics is an inspection page, not an export. The old implementation
  // downloaded every SAP fact + every Landmark fact before rendering, which
  // made a large production database look like a permanently loading page.
  // Counts stay exact; matching/facility details use a bounded recent sample.
  const DIAG_SAMPLE=2500;
  const [sapCount,lmCount,sapRows,lmRows,manual]=await Promise.all([
    supabase.from("sap_invoice_facts").select("*",{count:"exact",head:true}),
    latestLm.data?.id?supabase.from("landmark_visit_facts").select("*",{count:"exact",head:true}).eq("batch_id",latestLm.data.id):Promise.resolve({count:0,error:null}),
    supabase.from("sap_invoice_facts").select("vehicle_key,driver_name,facility_type").order("dispatch_date",{ascending:false}).limit(DIAG_SAMPLE),
    latestLm.data?.id?supabase.from("landmark_visit_facts").select("vehicle_key,driver_name").eq("batch_id",latestLm.data.id).limit(DIAG_SAMPLE):Promise.resolve({data:[],error:null}),
    supabase.from("vehicle_driver_manual_mappings").select("vehicle_key,sap_driver")
  ]);
  if(sapCount.error) throw new Error(sapCount.error.message);
  if(lmCount.error) throw new Error(lmCount.error.message);
  if(sapRows.error) throw new Error(sapRows.error.message);
  if(lmRows.error) throw new Error(lmRows.error.message);
  if(manual.error)throw new Error(manual.error.message);
  const sapByVehicle=new Map<string,string>(); for(const r of (sapRows.data||[])) if(r.vehicle_key&&r.driver_name&&!sapByVehicle.has(r.vehicle_key)) sapByVehicle.set(r.vehicle_key,r.driver_name);
  const manualMap=new Map<string,string>((manual.data||[]).map((r:any)=>[r.vehicle_key,r.sap_driver]));
  const lmKeys=[...new Set((lmRows.data||[]).map((r:any)=>r.vehicle_key).filter(Boolean))] as string[];
  ...
  const fac=new Map<string,number>(); for(const r of (sapRows.data||[])){const k=r.facility_type||"Unclassified";fac.set(k,(fac.get(k)||0)+1)}
  ...
  return {
    data_load_summary:{sap_rows_loaded:Number(sapCount.count||0),sap_unique_vehicles:new Set((sapRows.data||[]).map((r:any)=>r.vehicle_key).filter(Boolean)).size,sap_unique_drivers:new Set((sapRows.data||[]).map((r:any)=>r.driver_name).filter(Boolean)).size,landmark_rows_loaded:Number(lmCount.count||0),landmark_vehicle_sections:lmKeys.length},
    ...
    validation_results:[{level:Number(sapCount.count||0)?"ok":"warn",msg:Number(sapCount.count||0)?`SAP structured facts loaded (${Number(sapCount.count).toLocaleString()} rows)`:"No SAP facts loaded"},{level:Number(lmCount.count||0)?"ok":"warn",msg:Number(lmCount.count||0)?`V-Zone/Landmark facts loaded (${Number(lmCount.count).toLocaleString()} rows)`:"No V-Zone/Landmark facts loaded"},{level:unmatched?"warn":"ok",msg:unmatched?`${unmatched} GPS vehicle(s) remain unmatched in sampled diagnostics`:`GPS vehicle matching is complete (${details.length} sampled vehicle sections checked)`}],
    facility_breakdown,
    data_relationship_notes:["Original import files are retained in Supabase Storage.","Diagnostics uses exact database row counts and a bounded sample for responsive inspection.","The latest completed V-Zone/Landmark batch is used for GPS diagnostics.","Manual vehicle-to-driver mappings override automatic vehicle-key matches."]
  } as unknown as T;
}
