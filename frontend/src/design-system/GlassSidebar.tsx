        ...
        <>
          {isActive && (
            <motion.span
              ...
              className="top-nav-active-pill absolute inset-0 z-0 rounded-[13px]"
              aria-hidden="true"
            />
          )}
      ...
      <AnimatePresence>
        {menuPos && open && createPortal(
          <motion.div
            ...
            initial={{ opacity: 0, y: -6, scale: 0.97, filter: "blur(3px)" }}
            animate={{ opacity: 1, y: 0, scale: 1, filter: "blur(0px)" }}
            exit={{ opacity: 0, y: -4, scale: 0.985, filter: "blur(2px)" }}
            transition={{ duration: 0.18, ease: [0.16, 1, 0.3, 1] }}
            style={{ position: "fixed", left: menuPos.left, top: menuPos.top, zIndex: 9999, transformOrigin: "top left" }}
          >
            {items.map((item) => (
                <NavLink
                  key={item.to}
