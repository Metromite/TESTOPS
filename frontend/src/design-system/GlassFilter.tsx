...
export function GlassFilterBar({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div
      className={cn(
        "dashboard-filter-bar flex flex-wrap items-center gap-2 rounded-[18px] border border-[var(--glass-border)] bg-[var(--glass-bg)] p-2.5 backdrop-blur-glass backdrop-saturate-[180%] shadow-[var(--elevation-1)]",
        className
      )}
    >
      {children}
  ...
  return (
    <span
      className={cn(
        ...
        active ? "bg-[var(--blue)] text-white shadow-sm" : "bg-[var(--navy3)] text-muted border border-[var(--border)]"
      )}
    >
      {label}
      {onRemove && (
