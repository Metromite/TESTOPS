...
export function GlassFilterBar({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div
      className={cn(
        "glass-filter flex flex-wrap items-center gap-2 rounded-[16px] border border-[var(--glass-border)] bg-[var(--glass-bg)] p-2.5 backdrop-blur-glass backdrop-saturate-[175%]",
        className
      )}
    >
      {children}
