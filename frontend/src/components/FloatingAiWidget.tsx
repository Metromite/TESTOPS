...
export default function FloatingAiWidget() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setMessages((m) => [...m, { role: "assistant", content: data.reply, meta: metaParts.join(" · "), pendingAction: data.pending_action || undefined, navigate_to: data.navigate_to }]);
    } catch (e: any) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  async function confirmAction(msgIndex: number, action: PendingAction) {
    setLoading(true);
    try {
      const result = await executeDataAction<Record<string, unknown>>(action);
  ...
  return (
    <div className="floating-ai-widget">
      {open && (
        <div
          className="glass-card modal-pop logi-ai-panel"
          style={{
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
            {messages.length === 0 && (
              <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                ...
                <div className="logi-message-bubble" style={{ padding: "10px 12px", borderRadius: 14, fontSize: 13, maxWidth: "85%" }}>
            ...
            {messages.map((m, i) => (
              <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "92%", display: "flex", gap: 7, alignItems: "flex-end" }}>
                ...
                <div className={m.role === "assistant" ? "logi-chat-bubble logi-message-bubble logi-assistant-bubble" : "logi-message-bubble logi-user-bubble"} style={{
                  color: "var(--text)",
                  padding: "8px 12px", borderRadius: 14, fontSize: 13, whiteSpace: "pre-wrap",
                }}>
                  {m.content}
                </div>
                ...
                {m.pendingAction && (
                  <div className="modal-pop logi-confirm-bubble" style={{ marginTop: 6, borderRadius: 14, padding: 10 }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      <button className="btn" style={{ fontSize: 12, padding: "5px 12px" }} onClick={() => cancelAction(i)}>
                        Cancel
                      </button>
                    </div>
                  </div>
            ...
            {loading && (
              <div style={{ alignSelf: "flex-start", display: "flex", gap: 7, alignItems: "flex-end" }}>
                <TalkingLogiAvatar talking />
                <div className="logi-chat-bubble logi-message-bubble" style={{ padding: "9px 12px", borderRadius: 14, fontSize: 13 }}>
