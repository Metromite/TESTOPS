...
export default function FloatingAiWidget() {
  ...
  return (
    <div className="floating-ai-widget">
      {open && (
        <div
          className="glass-level-3 logi-chat-window modal-pop"
          style={{
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
            {messages.length === 0 && (
              <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                ...
                <div style={{ background: "var(--glass-bg-2)", padding: "10px 12px", borderRadius: 12, fontSize: 13, maxWidth: "85%" }}>
            ...
            {messages.map((m, i) => (
              <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "92%", display: "flex", gap: 7, alignItems: "flex-end" }}>
                ...
                <div className={m.role === "assistant" ? "logi-chat-bubble" : undefined} style={{
                  background: m.role === "user" ? "color-mix(in srgb, var(--blue) 88%, transparent)" : "var(--glass-bg-2)",
                ...
                {m.pendingAction && (
                  <div className="modal-pop" style={{ marginTop: 6, background: "var(--glass-bg-2)", border: "1px solid color-mix(in srgb, var(--amber) 55%, transparent)", borderRadius: 10, padding: 10 }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      <button className="btn" style={{ fontSize: 12, padding: "5px 12px", background: "var(--glass-bg-3)", color: "var(--muted)" }} onClick={() => cancelAction(i)}>
                        Cancel
                      </button>
                    </div>
                  </div>
            ...
            {loading && (
              <div style={{ alignSelf: "flex-start", display: "flex", gap: 7, alignItems: "flex-end" }}>
                <TalkingLogiAvatar talking />
                <div className="logi-chat-bubble" style={{ background: "var(--glass-bg-2)", padding: "9px 12px", borderRadius: 12, fontSize: 13 }}>
