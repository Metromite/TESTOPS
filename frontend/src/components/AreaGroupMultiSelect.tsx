...
export default function AreaGroupMultiSelect({
  ...
  selectedAreas: { id: string; code: string; name: string }[];
  selectedGroups: { id: string; name: string }[];
  onChange: (areas: { id: string; code: string; name: string }[], groups: { id: string; name: string }[]) => void;
  placeholder?: string;
  emptyHint?: string;
}) {
  const [query, setQuery] = useState("");
  const [areaSuggestions, setAreaSuggestions] = useState<{ id: string; code: string; name: string; sector: string }[]>([]);
  const [groupSuggestions, setGroupSuggestions] = useState<{ id: string; name: string }[]>([]);
  const dragItem = useState<{ current: { kind: "area" | "group"; id: string } | null }>({ current: null })[0];
    ...
    const handle = setTimeout(() => {
      supabase
        ...
        .then(({ data, error }) => {
          ...
          if ((data ?? []).length > 0) document.dispatchEvent(new CustomEvent("nav-dropdown-open", { detail: instanceId }));
        });
      supabase
        .from("area_groups")
        .select("id,name")
        ...
        .then(({ data, error }) => {
          if (error) return;
          setGroupSuggestions((data ?? []).map((g) => ({ id: g.id, name: g.name })));
        });
    }, 200);
  ...
  function handleDragStart(kind: "area" | "group", id: string) { dragItem.current = { kind, id }; }
  function handleDragOver(e: DragEvent) { e.preventDefault(); }
  function handleDrop(targetKind: "area" | "group", targetId: string) {
