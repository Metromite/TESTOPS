...
export default function RoutePlanningToolbar() {
  return (
    <div className="route-planning-shell" style={{ maxWidth: 1440, margin: "0 auto", padding: "24px 24px 0" }}>
      <div className="route-planning-toolbar mb-5 flex flex-wrap gap-1.5 rounded-[20px] border border-[var(--glass-border)] p-2">
        {TABS.map((t) => (
          <NavLink
            key={t.to}
            to={t.to}
            className={({ isActive }) =>
              cn(
                "route-planning-tab relative rounded-[13px] px-3.5 py-2 text-[13px] font-semibold transition-colors duration-150",
