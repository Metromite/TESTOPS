...
export default function DriverSlicerBar({
  ...
  return (
    <div
      className={cn(
        "glass-filter driver-slicer-bar flex flex-col gap-2 rounded-[16px] border border-[var(--glass-border)]",
        "bg-[var(--liquid-glass-surface)] p-2.5 backdrop-blur-glass shadow-elevation1"
      )}
      role="group"
    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        ...
        {options.length > 12 && (
          <div className="relative shrink-0">
            ...
            <input
              ...
              className="driver-slicer-search w-[140px] rounded-md border border-[var(--glass-border)] bg-[var(--liquid-glass-control)] py-1.5 pl-7 pr-2 text-[12.5px] text-ink placeholder:text-muted focus:outline-none"
            />
          </div>
        )}
        ...
        <button
          ...
          className={cn(
            ...
            allActive ? "driver-slicer-active bg-[var(--blue)] text-white" : "bg-[var(--liquid-glass-control)] text-ink hover:text-ink"
          )}
        ...
        {selected.length > 0 && (
          <button
            type="button"
            onClick={() => onChange([])}
            className="driver-slicer-clear shrink-0 whitespace-nowrap rounded-md px-2 py-1.5 text-[12px] font-semibold text-[var(--blue)] hover:underline"
          >
        ...
        {visibleOptions.map((opt) => {
          ...
          return (
            <button
              ...
              className={cn(
                "shrink-0 whitespace-nowrap rounded-full px-3 py-1.5 text-[12.5px] font-semibold transition-colors duration-150",
                isSelected ? "driver-slicer-active bg-[var(--blue)] text-white" : "bg-[var(--liquid-glass-control)] text-ink hover:text-ink"
              )}
      ...
      {needsExpand && (
        <button
          ...
          className="flex w-fit shrink-0 items-center gap-1 self-center whitespace-nowrap rounded-md px-2 py-1 text-[12px] font-medium text-[var(--blue)] hover:underline"
        >
          {expanded ? (
