...
export function useHomeData(driver: string, globalFilters: GlobalFilters | undefined, onDriverListLoaded?: (names: string[]) => void) {
  ...
  function openDetail(metric: string, title: string, driverOverride?: string) {
    // Paint the existing Dashboard modal first. The old implementation started
    // the potentially expensive detail scan in the same click task, which made
    // the browser appear frozen before the modal could paint.
    ...
    window.setTimeout(() => {
      void (async () => {
        try {
          ...
          const res = await getHomeDetailRows(metric, params) as HomeDetailResponse;
