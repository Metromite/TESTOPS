...
export function useHomeData(driver: string, globalFilters: GlobalFilters | undefined, onDriverListLoaded?: (names: string[]) => void) {
  ...
  async function openDetail(metric: string, title: string, driverOverride?: string) {
    // Paint the dialog shell first. Previously the detail fetch could start
    // a long client-side scan in the same click task, making the KPI appear
    // to freeze and preventing the existing popup from being painted.
