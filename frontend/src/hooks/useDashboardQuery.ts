 ...
 * Supabase analytics query + optional setInterval poll) with only the endpoint, poll
...
import { fetchDashboardEndpoint, invalidateDashboardAnalyticsCache } from "../services/dashboardAnalytics";
...
export function useDashboardQuery<T>(
  ...
  async function load(force = false) {
    ...
    try {
      if (force) invalidateDashboardAnalyticsCache();
      const p = opts?.skipFilters ? new URLSearchParams() : new URLSearchParams(dashboardFilterParams(driver, gf));
      const d: T = await fetchDashboardEndpoint<T>(`${endpoint}${p.toString() ? `?${p.toString()}` : ""}`);
  ...
  useEffect(() => {
    setData(getCached<T>(cacheKey) ?? null);
    setError("");
    // Slicer clicks can arrive in bursts (multi-select, Select All, date edits).
    // Debounce the network execution very slightly so superseded filter states
    // never become simultaneous database work. The UI selection itself updates
    // immediately; only the server round-trip is coalesced.
    const initialId = window.setTimeout(() => load(), 120);
    let pollId: number | undefined;
    if (opts?.pollMs) pollId = window.setInterval(load, opts.pollMs);
    return () => {
      window.clearTimeout(initialId);
      if (pollId !== undefined) window.clearInterval(pollId);
      // Invalidate the sequence so a response belonging to an unmounted/old
      // filter state cannot update this hook.
      requestSeq.current += 1;
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [endpoint, driver, gf.drivers, gf.areas, gf.division, gf.route_type, gf.vehicle_type, gf.facility_type, gf.salesman, gf.start_date, gf.end_date, gf.lead_time_band, gf.classification, reloadTick.current]);
  ...
  useEffect(() => subscribeDashboardRefresh(() => loadRef.current(true)), []);
