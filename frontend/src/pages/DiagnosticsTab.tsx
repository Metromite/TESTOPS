...
import { getDiagnostics } from "../services/operational";
...
export default function DiagnosticsTab() {
  ...
  useEffect(() => {
    getDiagnostics<DiagData>().then(setData).catch((e) => setError(e.message));
  }, []);

  if (error) return <div className="glass-card error-text diagnostics-state">{error}</div>;
  if (!data) return <div className="glass-card diagnostics-state"><span className="diagnostics-spinner" />Loading diagnostics…</div>;
  ...
  return (
    <div className="diagnostics-grid">
      ...
      <div className="glass-card diagnostics-wide">
