...
import { getControlCenterConfig, saveControlCenterConfig } from "../services/operational";
...
export default function ControlCenter() {
  ...
  useEffect(() => {
    let cancelled = false;
    getControlCenterConfig<{ config: GlobalConfig | null; version: number }>()
  ...
  // Supabase Realtime: one shared subscription for the singleton Control Center row.
  useEffect(() => {
    const channel = supabase
      .channel("control-center-config")
      .on("postgres_changes", { event: "*", schema: "public", table: "control_center_config", filter: "id=eq.00000000-0000-0000-0000-000000000001" }, (payload) => {
        const next = payload.new as { config?: GlobalConfig; version?: number };
        if (next.config && Number(next.version || 0) > serverVersion.current) {
          adoptRemote(next.config, Number(next.version || 0));
          setSyncStatus("synced");
        }
      })
      .subscribe();
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      ...
      try {
        const body = await saveControlCenterConfig<{ config: GlobalConfig; version: number }>(config);
  ...
  return (
    <div
      ...
      style={{ "--occ-accent": config.branding.accentColor } as React.CSSProperties & Record<"--occ-accent", string>}
    >
      {!kiosk && (
        <div className="page">
