      ...
      <div className="glass-card table-scroll">
        <table className="data-table">
          ...
          <tbody>
            {rows.map((v) => (
              <tr key={v.id}>
                <td>{v.number}</td><td>{v.type}</td><td>{v.capacity_tons ? `${v.capacity_tons} ton` : "—"}</td><td>{v.pallet_capacity != null ? `${v.pallet_capacity} big` : "—"}</td><td>{v.division || "Any"}</td><td>{v.status}</td>
                <td>
