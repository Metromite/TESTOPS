...
interface VehicleForm {
  number: string; type: string; division: string; status: string; capacity_tons: string; pallet_capacity: string;
}
interface AreaForm {
...
function VehiclesPanel({ meta }: { meta: MetaOptions }) {
  const empty = { number: "", type: meta.vehicle_types[0] || "Van", division: "", status: "Active", capacity_tons: "", pallet_capacity: "" };
  ...
  function startEdit(v: VehicleRow) {
    setEditingId(v.id);
    setForm({ number: v.number, type: v.type, division: v.division, status: v.status, capacity_tons: v.capacity_tons == null ? "" : String(v.capacity_tons), pallet_capacity: v.pallet_capacity == null ? "" : String(v.pallet_capacity) });
    ...
    <div>
      ...
      {canWrite && (
        <form className="glass-card" onSubmit={handleSubmit} style={{ marginBottom: 20, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
          ...
          <Field label="Pallet Capacity"><input type="number" min="0" step="1" value={form.pallet_capacity} onChange={(e) => setForm({ ...form, pallet_capacity: e.target.value })} placeholder="e.g. 20" /></Field>
          <Field label="Status">
      ...
      <div className="glass-card table-scroll">
        <table className="data-table">
          ...
          <tbody>
            {rows.map((v) => (
              <tr key={v.id}>
                <td>{v.number}</td><td>{v.type}</td><td>{v.capacity_tons ? `${v.capacity_tons} ton` : "—"}</td><td>{v.pallet_capacity ?? "—"}</td><td>{v.division || "Any"}</td><td>{v.status}</td>
                <td>
