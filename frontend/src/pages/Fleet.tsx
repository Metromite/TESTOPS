...
interface VehicleRow {
  ...
  big_pallet_capacity?: number | null;
  small_pallet_capacity?: number | null;
  permitted_areas: AreaRef[];
  permitted_area_groups: GroupRef[];
}


interface DriverForm {
  code: string; name: string; veh_type: string; anchor_area: string; health_card: string; division: string; preferred_helper: string; status: string;
}
interface HelperForm {
  code: string; name: string; anchor_area: string; health_card: string; division: string; status: string;
}
interface VehicleForm {
  number: string; type: string; division: string; status: string; capacity_tons: string;
  big_pallet_capacity: string; small_pallet_capacity: string;
}
interface AreaForm {
  code: string; name: string; sector: string; needs_driver: string; needs_helper: string; route_type: string; vehicle_type: string;
}
interface AreaGroupForm { name: string }
...
function VehiclesPanel({ meta }: { meta: MetaOptions }) {
  const empty = { number: "", type: meta.vehicle_types[0] || "Van", division: "", status: "Active", capacity_tons: "", big_pallet_capacity: "", small_pallet_capacity: "" };
  ...
  function startEdit(v: VehicleRow) {
    setEditingId(v.id);
    setForm({ number: v.number, type: v.type, division: v.division, status: v.status, capacity_tons: v.capacity_tons == null ? "" : String(v.capacity_tons), big_pallet_capacity: v.big_pallet_capacity == null ? "" : String(v.big_pallet_capacity), small_pallet_capacity: v.small_pallet_capacity == null ? "" : String(v.small_pallet_capacity) });
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const payload = {
        ...
        big_pallet_capacity: form.big_pallet_capacity === "" ? null : Number(form.big_pallet_capacity),
        small_pallet_capacity: form.small_pallet_capacity === "" ? null : Number(form.small_pallet_capacity),
        pallet_capacity: (form.big_pallet_capacity !== "" || form.small_pallet_capacity !== "") ? Number(form.big_pallet_capacity || 0) + Number(form.small_pallet_capacity || 0) : null,
      };
      const saved = editingId
    ...
    <div>
      ...
      {canWrite && (
        <form className="glass-card" onSubmit={handleSubmit} style={{ marginBottom: 20, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
          ...
          <Field label="Big Pallet Capacity"><input type="number" min="0" step="1" value={form.big_pallet_capacity} onChange={(e) => setForm({ ...form, big_pallet_capacity: e.target.value })} placeholder="e.g. 10" /></Field>
          <Field label="Small Pallet Capacity"><input type="number" min="0" step="1" value={form.small_pallet_capacity} onChange={(e) => setForm({ ...form, small_pallet_capacity: e.target.value })} placeholder="e.g. 20" /></Field>
          <Field label="Status">
      ...
      <div className="glass-card table-scroll">
        <table className="data-table">
          <thead><tr><th>Number</th><th>Type</th><th>Capacity</th><th>Big Pallets</th><th>Small Pallets</th><th>Total Pallets</th><th>Division Restriction</th><th>Status</th><th>Permitted Areas</th>{canWrite && <th></th>}</tr></thead>
          <tbody>
            {rows.map((v) => (
              <tr key={v.id}>
                <td>{v.number}</td><td>{v.type}</td><td>{v.capacity_tons ? `${v.capacity_tons} ton` : "—"}</td><td>{v.big_pallet_capacity ?? "—"}</td><td>{v.small_pallet_capacity ?? "—"}</td><td>{(v.big_pallet_capacity != null || v.small_pallet_capacity != null) ? Number(v.big_pallet_capacity || 0) + Number(v.small_pallet_capacity || 0) : (v.pallet_capacity ?? "—")}</td><td>{v.division || "Any"}</td><td>{v.status}</td>
                <td>
...
function AreaGroupsPanel() {
  ...
  const [form, setForm] = useState<AreaGroupForm>(empty);
