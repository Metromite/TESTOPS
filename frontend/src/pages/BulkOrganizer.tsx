...
export default function BulkOrganizer() {
  ...
  if (loading) return <div className="page bulk-game-page"><GlassCard style={{ padding: 28 }}>Loading Bulk Organizer game world…</GlassCard></div>;
  ...
  return <div className="page bulk-game-page bulk-world-page">
    <div className="bulk-game-header glass-card"><div><div className="bulk-eyebrow"><Sparkles size={14}/> BULK ORGANIZER · DISPATCH WORLD</div><h1>Build today&apos;s dispatch</h1><p>Place destinations, select Fleet vehicles, then load scheduled invoices and route vehicles across the isometric world.</p></div><div className="bulk-header-actions"><GlassButton variant="secondary" size="sm" onClick={() => void loadSourceData(date)}><RefreshCw size={14}/> Refresh</GlassButton><GlassButton size="sm" onClick={() => void save()} disabled={saving}><Save size={14}/> {saving ? "Saving…" : "Save Day"}</GlassButton></div></div>
    {error && <div className="bulk-game-error"><CircleAlert size={15}/> {error}<button onClick={() => setError("")}><X size={14}/></button></div>}
    <div className="bulk-date-game glass-card"><button className="btn icon-btn" onClick={() => shiftDate(-1)}><ChevronLeft size={18}/></button><div className="bulk-date-center"><CalendarDays size={18}/><strong>{dayLabel(date)}</strong><input type="date" value={date} onChange={e => setDate(e.target.value)}/></div><button className="btn icon-btn" onClick={() => shiftDate(1)}><ChevronRight size={18}/></button><div className="bulk-live-pill"><span/> LIVE WORLD</div></div>
    <div className="bulk-game-layout bulk-game-layout-v2">
      <aside className="bulk-side-rail bulk-command-rail"><div className="bulk-side-title"><Bell size={16}/> TODAY&apos;S MISSIONS</div><div className="bulk-side-date">{dayName} · {date}</div>{notifications.length ? notifications.map(n => <div key={n.id} className={`bulk-notification ${n.tone}`}><div className="bulk-notification-dot"/><div><b>{n.title}</b><span>{n.text}</span></div></div>) : <div className="bulk-empty-side">No scheduled alerts for this day.</div>}<div className="bulk-side-stats"><div><span>Customers</span><b>{customers.length}</b></div><div><span>Invoices</span><b>{invoices.length}</b></div><div><span>Pallets</span><b>{totalPallets}</b></div><div><span>Waiting</span><b>{pendingPallets}</b></div></div><div className="bulk-command-section"><div className="bulk-command-title">WORLD TOOLS</div><GlassButton variant="secondary" size="sm" onClick={() => setShowBuildingEditor(x => !x)}><Building2 size={14}/>{showBuildingEditor ? "Close Builder" : "Build Destination"}</GlassButton><GlassButton variant="secondary" size="sm" onClick={() => setShowFleet(x => !x)}><Truck size={14}/>{showFleet ? "Close Fleet" : "Choose Vehicles"}</GlassButton><GlassButton variant="secondary" size="sm" onClick={() => setShowCustomerManager(x => !x)}><Users size={14}/>{showCustomerManager ? "Close Customers" : "Customers"}</GlassButton><GlassButton variant="secondary" size="sm" onClick={() => setShowScheduleManager(x => !x)}><CalendarDays size={14}/>{showScheduleManager ? "Close Schedules" : "Schedules"}</GlassButton><GlassButton size="sm" onClick={() => {setEditingInvoice(null);setInvoiceForm(x => ({...x,schedule_date:date,invoice_no:""}))}}><Plus size={14}/> Add Invoice</GlassButton></div></aside>
      <main className="bulk-world-main">
        <section className="bulk-isometric-game glass-card"><div className="bulk-world-toolbar"><div><span className="bulk-world-kicker">DISPATCH MAP</span><b>Isometric Operations City</b><small>Drag vehicles to destinations · Drag order stacks into vehicles</small></div><div className="bulk-world-legend"><span><i className="dot yellow"/>Loading</span><span><i className="dot green"/>Full</span><span><i className="dot blue"/>Destination</span></div></div>
          <div className="bulk-world-viewport" onWheel={e=>{e.preventDefault();setWorldZoom(z=>Math.min(1.45,Math.max(.72,z+(e.deltaY<0?.06:-.06))))}}><div className="bulk-world-grid-art"/><div className="bulk-world-controls"><button className="world-nav-btn" onClick={()=>setWorldZoom(z=>Math.min(1.45,z+.1))}>＋</button><span>{Math.round(worldZoom*100)}%</span><button className="world-nav-btn" onClick={()=>setWorldZoom(z=>Math.max(.72,z-.1))}>−</button><button className="world-nav-btn" onClick={()=>setWorldZoom(1)}>⌂</button></div><div className="bulk-world-titleplate">{date} · {totalPallets} PALLETS · {selectedVehicles.length} VEHICLES</div><div className="bulk-isometric-scene" style={{transform:`scale(${worldZoom})`}}>
            {buildingCards.map((b,idx)=>{const routed=selectedVehicles.filter(v=>plan.vehicle_meta?.[v.vehicle_id]?.building_id===b.id);return <div key={b.id} className="bulk-world-building" style={{left:`${12+(idx*19)%72}%`,top:`${20+(Math.floor(idx/4)*29)%55}%`}} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();const raw=e.dataTransfer.getData("text/plain");const d=dragged||(raw?{type:"vehicle" as const,id:raw}:null);if(d?.type==="vehicle")routeVehicle(d.id,b.id);setDragged(null)}}><div className={`iso-building ${b.type}`}><div className="iso-building-shadow"/><div className="iso-building-left"/><div className="iso-building-right"/><div className="iso-building-roof"/><div className="iso-building-window w1"/><div className="iso-building-window w2"/><div className="iso-building-door"/><div className="iso-building-badge">{b.type==="hospital"?"✚":b.type==="store"?"▦":"⌂"}</div></div><div className="iso-label"><b>{b.name}</b><span>{b.area||"Area not set"}</span>{routed.length>0&&<em>🚚 {routed.length} routed</em>}</div><button className="iso-delete" onClick={()=>deleteBuilding(b.id)}><Trash2 size={11}/></button></div>})}
            {!buildingCards.length&&<div className="bulk-world-empty"><Building2 size={40}/><b>Build your first destination</b><span>Warehouse, hospital, store or any custom customer building.</span><button className="btn" onClick={()=>setShowBuildingEditor(true)}><Plus size={14}/> Build</button></div>}
            {selectedVehicles.map((cfg,idx)=>{const v=vehicleMap.get(cfg.vehicle_id);if(!v)return null;const used=loadedByVehicle(v.id),pct=Math.min(100,Math.round((used/Math.max(1,cfg.capacity))*100)),meta=plan.vehicle_meta?.[v.id]||{},bIndex=buildingCards.findIndex(b=>b.id===meta.building_id);return <div key={v.id} draggable onDragStart={e=>{e.dataTransfer.setData("text/plain",v.id);setDragged({type:"vehicle",id:v.id})}} onDragEnd={()=>setDragged(null)} className="bulk-world-vehicle" style={{left:`${18+(idx*23)%66}%`,top:`${59+(idx%2)*17}%`}}><div className="iso-vehicle-shadow"/><div className={`iso-vehicle ${isVan(v.type)?"van":"pickup"}`}><div className="vehicle-cab"/><div className="vehicle-box"><span>{v.number}</span></div><div className="vehicle-wheel a"/><div className="vehicle-wheel b"/></div><div className="vehicle-load"><div style={{width:`${pct}%`}} className={pct>=100?"full":""}/></div><div className="vehicle-label"><b>{v.number}</b><span>{used}/{cfg.capacity} pallets · {Math.max(0,cfg.capacity-used)} free</span><em>{meta.driver||"Driver not set"}</em></div>{bIndex>=0&&<div className="route-line-label">→ {buildingCards[bIndex].name}</div>}<button className="iso-vehicle-details" onClick={()=>setShowFleet(true)}><Gauge size={11}/></button></div>})}
          </div></div></section>
        <section className="bulk-world-orders glass-card"><div className="bulk-game-section-head"><div><div className="bulk-section-kicker">ORDER DEPOT · LEVEL 03</div><h2>Today&apos;s scheduled orders</h2><p>One visual 3D pallet stack per invoice. The quantity badge represents the complete pallet count.</p></div><div className="bulk-order-tools"><div className="bulk-search"><Search size={14}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search customer / invoice…"/></div></div></div><div className="bulk-order-depot" onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();const raw=e.dataTransfer.getData("text/plain");const d=dragged||(raw?{type:"invoice" as const,id:raw}:null);if(d?.type==="invoice")assignInvoice(d.id,null);setDragged(null)}}>{visibleInvoices.map(i=>{const loaded=invoiceLoaded(i);return <div key={i.id} className="bulk-order-token" draggable onDragStart={()=>setDragged({type:"invoice",id:i.id})} onDragEnd={()=>setDragged(null)}><div className="order-token-pallet"><div className="pallet-top"/><div className="pallet-left"/><div className="pallet-right"/><strong>{i.pallets}</strong><span>PALLETS</span></div><div className="order-token-info"><b>{i.customer_name}</b><span>{i.invoice_no} · {i.area||"Area not set"}</span><small>{i.division} · {i.scheduled_date}</small><em>{loaded}/{i.pallets} loaded</em></div><div className="order-token-actions"><button className="btn icon-btn" onClick={e=>{e.stopPropagation();editInvoice(i)}}><Pencil size={12}/></button><button className="btn icon-btn" onClick={e=>{e.stopPropagation();deleteInvoice(i.id)}}><Trash2 size={12}/></button></div></div>})}{!visibleInvoices.length&&<div className="bulk-world-empty small"><Package size={32}/><b>No scheduled orders for this date</b><span>Add an invoice or use today&apos;s SAP invoice facts.</span></div>}</div></section>
        {showBuildingEditor&&<GlassCard className="bulk-floating-panel"><div className="bulk-panel-title"><Building2 size={16}/> Build destination <button onClick={()=>setShowBuildingEditor(false)}><X size={14}/></button></div><div className="bulk-builder-row"><input value={buildingForm.name} placeholder="Building / customer name" onChange={e=>setBuildingForm({...buildingForm,name:e.target.value})}/><select value={buildingForm.type} onChange={e=>setBuildingForm({...buildingForm,type:e.target.value as Building["type"]})}><option value="warehouse">Warehouse</option><option value="hospital">Hospital</option><option value="store">Store</option><option value="other">Custom</option></select><select value={buildingForm.area} onChange={e=>setBuildingForm({...buildingForm,area:e.target.value})}><option value="">Area / City</option>{areas.map(a=><option key={a.id} value={a.name}>{a.code} · {a.name}</option>)}</select><GlassButton size="sm" onClick={addBuilding}><Plus size={14}/> Build</GlassButton><GlassButton variant="secondary" size="sm" onClick={()=>void saveDefaults()} disabled={savingDefaults}><Save size={14}/> Default</GlassButton></div></GlassCard>}
        {showFleet&&<GlassCard className="bulk-floating-panel"><div className="bulk-panel-title"><Truck size={16}/> Fleet selection <button onClick={()=>setShowFleet(false)}><X size={14}/></button></div><div className="bulk-fleet-grid">{fleet.map(v=>{const on=selectedVehicles.some(x=>x.vehicle_id===v.id);return <button key={v.id} className={`bulk-fleet-choice ${on?"selected":""}`} onClick={()=>toggleVehicle(v,true)}><span className="bulk-fleet-mini-icon">{isVan(v.type)?"🚐":"🛻"}</span><span><b>{v.number}</b><small>{v.type} · {v.division||"Unknown"}</small></span>{on?<Check size={15}/>:<Plus size={15}/>}</button>})}</div><div className="bulk-selected-vehicles">{selectedVehicles.map(cfg=>{const v=vehicleMap.get(cfg.vehicle_id),meta=plan.vehicle_meta?.[cfg.vehicle_id]||{};return <div className="bulk-vehicle-editor" key={cfg.vehicle_id}><b>{v?.number}</b><label>Capacity<input type="number" min={1} value={cfg.capacity} onChange={e=>setPlan(x=>({...x,vehicles:x.vehicles.map(q=>q.vehicle_id===cfg.vehicle_id?{...q,capacity:Math.max(1,Number(e.target.value)||1)}:q)}))}/></label><label>Driver<input value={meta.driver||""} onChange={e=>updateVehicleMeta(cfg.vehicle_id,"driver",e.target.value)}/></label><label>Helper<input value={meta.helper||""} onChange={e=>updateVehicleMeta(cfg.vehicle_id,"helper",e.target.value)}/></label></div>})}</div></GlassCard>}
        {(showCustomerManager||showScheduleManager||editingInvoice||invoiceForm.invoice_no)&&<GlassCard className="bulk-floating-panel"><div className="bulk-panel-title"><Users size={16}/> Data & scheduling <button onClick={()=>{setShowCustomerManager(false);setShowScheduleManager(false);setEditingInvoice(null);setInvoiceForm(x=>({...x,invoice_no:""}))}}><X size={14}/></button></div>{showCustomerManager&&<div className="bulk-builder-row"><input list="bulk-customer-library" value={customerForm.name} placeholder="Customer name" onChange={e=>setCustomerForm({...customerForm,name:e.target.value})}/><select value={customerForm.area} onChange={e=>setCustomerForm({...customerForm,area:e.target.value})}><option value="">Area</option>{areas.map(a=><option key={a.id} value={a.name}>{a.code} · {a.name}</option>)}</select><select value={customerForm.division} onChange={e=>setCustomerForm({...customerForm,division:e.target.value})}><option value="">Division</option><option>Pharma</option><option>Consumer</option></select><GlassButton size="sm" onClick={addCustomer}><Plus size={14}/> Add Customer</GlassButton></div>}{showScheduleManager&&<div className="bulk-schedule-editor"><input list="bulk-customer-library" value={scheduleForm.customer_name} placeholder="Customer" onChange={e=>setScheduleForm({...scheduleForm,customer_name:e.target.value})}/><select value={scheduleForm.area} onChange={e=>setScheduleForm({...scheduleForm,area:e.target.value})}><option value="">Area</option>{areas.map(a=><option key={a.id} value={a.name}>{a.code} · {a.name}</option>)}</select><select value={scheduleForm.division} onChange={e=>setScheduleForm({...scheduleForm,division:e.target.value})}><option value="">Division</option><option>Pharma</option><option>Consumer</option></select><div className="bulk-day-picker">{["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map((d,i)=><button key={d} className={scheduleForm.days.includes(i)?"active":""} onClick={()=>setScheduleForm(x=>({...x,days:x.days.includes(i)?x.days.filter(y=>y!==i):[...x.days,i]}))}>{d}</button>)}</div><GlassButton size="sm" onClick={addSchedule}><Plus size={14}/> Save Schedule</GlassButton></div>}<div className="bulk-invoice-form"><input value={invoiceForm.invoice_no} placeholder="Invoice no." onChange={e=>setInvoiceForm({...invoiceForm,invoice_no:e.target.value})}/><input list="bulk-customer-library" value={invoiceForm.customer_name} placeholder="Predictive customer" onChange={e=>{const name=e.target.value;const c=customers.find(x=>norm(x.name)===norm(name));setInvoiceForm({...invoiceForm,customer_name:name,customer_id:c?.id||"",area:c?.area||invoiceForm.area,division:c?.division||invoiceForm.division})}}/><select value={invoiceForm.area} onChange={e=>setInvoiceForm({...invoiceForm,area:e.target.value})}><option value="">Area</option>{areas.map(a=><option key={a.id} value={a.name}>{a.code} · {a.name}</option>)}</select><select value={invoiceForm.division} onChange={e=>setInvoiceForm({...invoiceForm,division:e.target.value})}><option value="">Division</option><option>Pharma</option><option>Consumer</option></select><input type="number" min={1} value={invoiceForm.pallets} onChange={e=>setInvoiceForm({...invoiceForm,pallets:Math.max(1,Number(e.target.value))})}/><input type="date" value={invoiceForm.invoice_date} onChange={e=>setInvoiceForm({...invoiceForm,invoice_date:e.target.value})}/><input type="date" value={invoiceForm.schedule_date} onChange={e=>setInvoiceForm({...invoiceForm,schedule_date:e.target.value})}/><GlassButton size="sm" onClick={editingInvoice?updateInvoice:addInvoice}>{editingInvoice?<Check size={14}/>:<Plus size={14}/>} {editingInvoice?"Update":"Add"} Invoice</GlassButton></div></GlassCard>}
      </main>
    </div>
