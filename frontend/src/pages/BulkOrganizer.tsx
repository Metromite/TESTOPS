...
type InvoicePlan = {
  ...
  division: string; pallets: number; pallet_size?: "big" | "small"; invoice_date: string | null; scheduled_date: string | null; vehicle_id: string | null; building_id?: string | null;
};
type VehicleConfig = { vehicle_id: string; capacity: number; big_pallet_capacity?: number; small_pallet_capacity?: number; driver?: string; helper?: string; building_id?: string | null };
...
export default function BulkOrganizer() {
  ...
  const [invoiceForm, setInvoiceForm] = useState({ invoice_no: "", customer_id: "", customer_name: "", area: "", division: "", pallets: 1, pallet_size: "big" as "big" | "small", invoice_date: "", schedule_date: "", building_id: "" });
  ...
  function startInvoiceBatchFromCurrent() {
    ...
    setInvoiceBatchRows(prev => prev.length ? prev : [seed, { ...seed, invoice_no: "" }]);
  ...
  function dropStoreOnVehicle(v: any, storeId: string) {
    ...
    const cap = capacityFor(cfg || { vehicle_id: v.id, capacity: defaultCapacity(v), big_pallet_capacity: Number(v.big_pallet_capacity || 0), small_pallet_capacity: Number(v.small_pallet_capacity || 0) }, v);
  ...
  function dropOnVehicle(v: any, activeDrag: { type: string; id: string }) {
    ...
    const room = vehicleRoomForInvoice(inv,cfg,v,adding);
    if (room < adding) { setError(`${v.number}: not enough ${inv.pallet_size || "big"} pallet capacity. ${room} space left.`); return; }
  ...
  function toggleVehicle(v: any, fromDefaults = false) {
    ...
    setPlan(x => x.vehicles.some(z => z.vehicle_id === v.id)
      ...
      : { ...x, vehicles: [...x.vehicles, { vehicle_id: v.id, capacity: defaultCapacity(v), big_pallet_capacity: Number(v.big_pallet_capacity || 0), small_pallet_capacity: Number(v.small_pallet_capacity || 0) }] });
    void listKey;
  }

  function updateVehiclePalletCapacity(vehicleId: string, size: "big" | "small", value: number) {
    const key = size === "big" ? "big_pallet_capacity" : "small_pallet_capacity";
    setPlan(x => ({ ...x, vehicles: (x.vehicles || []).map(v => v.vehicle_id === vehicleId ? { ...v, [key]: Math.max(0, Math.floor(value || 0)) } : v) }));
  }

  const vehicleSizeCaps = (cfg: VehicleConfig | undefined, v: any) => ({
    big: Number(v?.big_pallet_capacity ?? cfg?.big_pallet_capacity ?? 0),
    small: Number(v?.small_pallet_capacity ?? cfg?.small_pallet_capacity ?? 0),
  });
  const loadedByVehicleSize = (vehicleId: string, size: "big" | "small") => Object.entries(assignments).filter(([k,val]) => val === vehicleId && allPlanInvoices.some(i => i.scheduled_date === date && (i.pallet_size || "big") === size && k.startsWith(`${i.id}::`))).length;
  const vehicleRoomForInvoice = (inv: InvoicePlan, cfg: VehicleConfig | undefined, v: any, qty: number) => {
    const caps = vehicleSizeCaps(cfg,v);
    const sized = caps.big > 0 || caps.small > 0;
    if (!sized) return Math.max(0, capacityFor(cfg || {vehicle_id:v.id,capacity:defaultCapacity(v)},v) - loadedByVehicle(v.id));
    const size = inv.pallet_size || "big";
    return Math.max(0, caps[size] - loadedByVehicleSize(v.id,size));
  };
  ...
  async function addInvoiceBatch() {
    ...
    try {
      ...
      setInvoiceForm({ invoice_no:"",customer_id:"",customer_name:"",area:"",division:"",pallets:1,pallet_size:"big",invoice_date:"",schedule_date:"",building_id:"" });
  ...
  async function addInvoice() {
    ...
    const inv: InvoicePlan = {
      ...
      pallets: Math.max(1, Number(invoiceForm.pallets || 1)), pallet_size: invoiceForm.pallet_size || "big", invoice_date: invoiceForm.invoice_date || null,
    ...
    setInvoiceForm({ invoice_no: "", customer_id: "", customer_name: "", area: "", division: "", pallets: 1, pallet_size: "big" as "big" | "small", invoice_date: "", schedule_date: "", building_id: "" });
    setShowInvoiceManager(false);
  }

  async function updateInvoice() {
    if (!editingInvoice) return;
    const next = { ...editingInvoice, invoice_no: invoiceForm.invoice_no, customer_name: invoiceForm.customer_name, area: invoiceForm.area, division: invoiceForm.division, pallets: Math.max(1, Number(invoiceForm.pallets || 1)), pallet_size: invoiceForm.pallet_size || "big", invoice_date: invoiceForm.invoice_date || null, scheduled_date: invoiceForm.schedule_date || null, building_id: invoiceForm.building_id || plan.customer_schedules?.find(s => norm(s.customer_name) === norm(invoiceForm.customer_name))?.building_id || buildingForCustomer(invoiceForm.customer_name)?.id || editingInvoice.building_id || null };
  ...
  const capacityFor = (cfg: VehicleConfig, v: any) => {
    const big = Number(v.big_pallet_capacity ?? cfg.big_pallet_capacity ?? 0);
    const small = Number(v.small_pallet_capacity ?? cfg.small_pallet_capacity ?? 0);
    if (big > 0 || small > 0) return big + small;
    return Number(v.pallet_capacity ?? v.capacity_pallets ?? cfg.capacity ?? v.capacity ?? defaultCapacity(v));
  };
      ...
      {(showInvoiceManager||editingInvoice||invoiceForm.invoice_no)&&<div className="bulk-modal-block">
        ...
        {invoiceBatchMode&&!editingInvoice ? <div className="bulk-invoice-batch-list">
          {(invoiceBatchRows.length ? invoiceBatchRows : [invoiceForm]).map((row,idx)=>{
            ...
            const setRow=(patch:any)=>setInvoiceBatchRows(rows=>{
              const base=rows.length?rows:[{...invoiceForm}];
              return base.map((r,j)=>j===idx?{...r,...patch}:r);
            });
            return <div className="bulk-invoice-inline-card" key={idx}>
              <div className="bulk-invoice-inline-head"><span>Invoice {idx+1}</span>{invoiceBatchRows.length>1&&<button type="button" className="btn icon-btn" title="Remove invoice" onClick={()=>setInvoiceBatchRows(rows=>rows.filter((_,j)=>j!==idx))}><X size={12}/></button>}</div>
              <div className="bulk-invoice-form bulk-invoice-form-inline">
                <input value={row.invoice_no} placeholder="Invoice no." onChange={e=>setRow({invoice_no:e.target.value})}/>
                <input list="bulk-customer-library" value={row.customer_name} disabled={invoiceCustomerLocked} placeholder="Customer / Supply / Other customer" onChange={e=>{const name=e.target.value;const c=customers.find(x=>norm(x.name)===norm(name));setRow({customer_name:name,customer_id:c?.id||"",area:c?.area||row.area,division:c?.division||row.division})}}/>
                <label className="bulk-field-with-label"><span>Store / location</span><select value={row.building_id} disabled={invoiceBuildingLocked || batchBuildingLocked()} onChange={e=>{const buildingId=e.target.value;const b=(plan.buildings||[]).find(x=>x.id===buildingId);const isOthers=norm(b?.name)==="OTHERS";const linkedSchedule=plan.customer_schedules?.find(s=>s.building_id===buildingId);setRow({building_id:buildingId,customer_name:isOthers?row.customer_name:(b?.name||row.customer_name),customer_id:isOthers?row.customer_id:(customers.find(c=>norm(c.name)===norm(b?.name||""))?.id||`manual:${buildingId||""}`),area:b?.area||linkedSchedule?.area||row.area,division:b?.division||linkedSchedule?.division||row.division,schedule_date:buildingAllowsDate(buildingId,date)?date:""})}}><option value="">Select location</option>{(plan.buildings||[]).map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select></label>
                <select value={row.area} onChange={e=>setRow({area:e.target.value})}><option value="">Area</option>{areas.map(a=><option key={a.id} value={a.name}>{a.code} · {a.name}</option>)}</select>
                <select value={row.division} onChange={e=>setRow({division:e.target.value})}><option value="">Division</option><option>Pharma</option><option>Consumer</option></select>
                <label className="bulk-field-with-label"><span>Pallets</span><div className="bulk-pallet-entry"><input type="number" min={1} value={row.pallets} onChange={e=>setRow({pallets:Math.max(1,Number(e.target.value))})}/><select value={row.pallet_size||"big"} onChange={e=>setRow({pallet_size:e.target.value as "big"|"small"})}><option value="big">Big pallet</option><option value="small">Small pallet</option></select></div></label>
                <label className="bulk-field-with-label"><span>Invoice date</span><input type="date" value={row.invoice_date} onChange={e=>setRow({invoice_date:e.target.value})}/></label>
                <label className="bulk-field-with-label"><span>Schedule</span><div className="bulk-schedule-choice-box"><div className="bulk-schedule-choice-row">{rowDates.includes(date)&&<button type="button" className={row.schedule_date===date?"active":""} onClick={()=>setRow({schedule_date:date})}>{dayLabel(date)}</button>}<button type="button" className={!row.schedule_date?"active":""} onClick={()=>setRow({schedule_date:""})}>Waiting</button><button type="button" className={row.schedule_date&&row.schedule_date!==date?"active":""} onClick={()=>setRow({schedule_date:(row.schedule_date&&rowDates.includes(row.schedule_date)?row.schedule_date:(rowDates[0]||""))})}>Date</button></div>{row.schedule_date&&row.schedule_date!==date&&<input className="bulk-schedule-date-input" type="date" value={row.schedule_date} min={rowDates[0]||undefined} onChange={e=>{const v=e.target.value;if(v&&!rowDates.includes(v)){setError("Choose one of the allowed schedule dates for this store.");return;}setRow({schedule_date:v})}}/>}</div></label>
              </div>
            </div>;
          })}
