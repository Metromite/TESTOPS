...
export default function BulkOrganizer() {
  ...
  async function loadSourceData(selectedDate: string) {
    ...
    try {
      ...
      // Monthly store defaults are the source of truth for recurring store
      // schedules. A daily plan can contain an older snapshot; if we let that
      // snapshot win, a store changed to Every day (or to new weekdays) only
      // appears on dates that already have a newer daily row. Merge the daily
      // record for operational fields, but always take schedule_days/dates from
      // the current monthly defaults for matching store ids.
      ...
      const planBuildings = defaultBuildings.length
        ? [
            ...defaultBuildings.map(db => {
              const daily = dailyBuildings.find(b => b.id === db.id);
              return daily ? { ...daily, schedule_days: db.schedule_days || [], schedule_dates: db.schedule_dates || [] } : db;
            }),
      ...
      const planSchedules = defaultSchedules.length
        ? [
            ...defaultSchedules.map(ds => {
              const daily = dailySchedules.find(s => s.id === ds.id || (s.building_id && ds.building_id && s.building_id === ds.building_id));
              return daily ? { ...daily, days: ds.days || [] } : ds;
            }),
      ...
      setPlan({
        ...
        customer_schedules: planSchedules
      });
  ...
  function openBatchLoadModal() {
    const scheduled = allPlanInvoices.filter(i => i.scheduled_date === date && invoiceLoaded(i) < Number(i.pallets || 0));
  ...
  function confirmBatchLoad() {
    const candidates = [
      ...allPlanInvoices.filter(i => i.scheduled_date === date),
      ...(batchIncludeWaiting ? waiting : [])
    ];
    ...
    if (!selectedVehicles.length) { setError("Add a vehicle to the day before loading invoices."); return; }

    const nextAssignments = { ...(planRef.current.pallet_assignments || {}) };
    const usedByVehicle = new Map<string, number>();
    selectedVehicles.forEach(cfg => usedByVehicle.set(cfg.vehicle_id, loadedByVehicle(cfg.vehicle_id)));
    const loaded: string[] = [];
    const skipped: string[] = [];

    for (const inv of selected) {
      let remaining = Math.max(0, Number(inv.pallets || 0) - invoiceLoaded(inv));
      if (!remaining) continue;
      let placed = 0;
      for (const cfg of selectedVehicles) {
        ...
        for (let k = 0; k < Math.max(1, inv.pallets) && placed < (Number(inv.pallets || 0) - invoiceLoaded(inv)); k++) {
          const key = `${inv.id}::${k}`;
...
<main className="bulk-customer-section glass-card bulk-glass-surface">
    ...
    {showBatchLoadModal&&<div className="bulk-modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)closeBatchLoadModal()}}><GlassCard className="bulk-planner-modal bulk-load-modal bulk-modal-surface">
      <div className="bulk-panel-title"><ListChecks size={16}/> Load multiple invoices <button onClick={closeBatchLoadModal}><X size={14}/></button></div>
      <div className="bulk-modal-intro">Select the invoices you want to load together. Scheduled invoices are shown first; waiting invoices can be included when you choose.</div>
      <div className="bulk-batch-load-toolbar"><button type="button" className="bulk-batch-select-btn" onClick={()=>{const ids=allPlanInvoices.filter(i=>i.scheduled_date===date&&invoiceLoaded(i)<Number(i.pallets||0)).map(i=>i.id);setBatchSelectedIds(ids)}}>Select scheduled</button><button type="button" className="bulk-batch-select-btn" onClick={()=>setBatchSelectedIds([])}>Clear</button><label><input type="checkbox" checked={batchIncludeWaiting} onChange={e=>{setBatchIncludeWaiting(e.target.checked);if(e.target.checked){setBatchSelectedIds(ids=>Array.from(new Set([...ids,...waiting.filter(i=>invoiceLoaded(i)<Number(i.pallets||0)).map(i=>i.id)])))}}}/> Include waiting</label></div>
      <div className="bulk-batch-invoice-list">{[...allPlanInvoices.filter(i=>i.scheduled_date===date),...(batchIncludeWaiting?waiting:[])].filter((i,idx,arr)=>arr.findIndex(x=>x.id===i.id)===idx).map(i=>{const remaining=Math.max(0,Number(i.pallets||0)-invoiceLoaded(i));return <label key={i.id} className={`bulk-batch-invoice-row ${batchSelectedIds.includes(i.id)?"selected":""}`}><input type="checkbox" checked={batchSelectedIds.includes(i.id)} disabled={!remaining} onChange={()=>toggleBatchInvoice(i.id)}/><span><b>{i.invoice_no}</b><small>{i.customer_name} · {i.scheduled_date ? "Scheduled" : "Waiting"} · {remaining}/{i.pallets} pallets</small></span></label>})}</div>
      <div className="bulk-batch-load-note">Loading is distributed across the selected day vehicles according to vehicle compatibility and remaining capacity.</div>
      <div className="bulk-load-modal-actions"><GlassButton variant="secondary" size="sm" onClick={closeBatchLoadModal}>Cancel</GlassButton><GlassButton size="sm" onClick={confirmBatchLoad} disabled={!batchSelectedIds.length}><Truck size={13}/> Load selected</GlassButton></div>
    </GlassCard></div>}
