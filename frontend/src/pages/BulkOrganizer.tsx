...
export default function BulkOrganizer() {
  ...
  useEffect(() => {
    let cancelled = false;
    const resolveDark = () => bulkTheme !== "light";
    const loadWallpaper = async () => {
      const dark = resolveDark();
      const cacheKey = `bulk-wallpaper-${dark ? "dark" : "light"}`;
      const cached = localStorage.getItem(cacheKey);
      if (cached && !cancelled) setBulkWallpaper(cached);
      try {
        const config = await getAppearanceConfig();
        const path = dark ? config?.dark_bg_path : config?.light_bg_path;
        const url = path ? await getWallpaperUrl(path, 3600) : null;
        if (!cancelled && url) { setBulkWallpaper(url); localStorage.setItem(cacheKey, url); }
      } catch { /* keep cached wallpaper */ }
    };
    void loadWallpaper();
    const observer = new MutationObserver(() => { void loadWallpaper(); });
    observer.observe(document.documentElement, { attributes: true, attributeFilter: ["class", "data-theme"] });
    const onStorage = (e: StorageEvent) => { if (e.key === "theme-mode") void loadWallpaper(); };
    window.addEventListener("storage", onStorage);
    return () => { cancelled = true; observer.disconnect(); window.removeEventListener("storage", onStorage); };
  }, [bulkTheme]);
  ...
  return <div className={`page bulk-planner-page bulk-theme-${bulkTheme} ${loading ? "is-hydrating" : ""}`}>
    {bulkWallpaper && <div className="bulk-page-wallpaper" style={{ backgroundImage: `url("${bulkWallpaper}")` }} aria-hidden="true" />}
    ...
    <section className="bulk-vehicles-section glass-card">
      <div className="bulk-planner-section-head"><div><span>01 · FLEET</span><h2>Default & daily vehicles</h2><p>Vehicles selected as defaults repeat across days. Add or remove vehicles for this day without changing the Fleet Data Manager.</p></div><div style={{display:"flex",gap:8,alignItems:"center"}}><GlassButton variant="secondary" size="sm" onClick={()=>setVehiclesExpanded(x=>!x)}>{vehiclesExpanded?"Collapse":"Expand"} Vehicles</GlassButton><GlassButton size="sm" onClick={()=>setShowFleet(x=>!x)}><Truck size={14}/>{showFleet?"Close Fleet":"Choose Fleet"}</GlassButton></div></div>
