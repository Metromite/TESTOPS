...
export default function BulkOrganizer() {
  ...
  function addStore() {
    ...
    if (!name) return;
    const b: Building = {
      id: `store:${crypto.randomUUID()}`, name, type: storeForm.type, area: storeForm.area.trim(), sort_order: (plan.buildings || []).length, enabled: true, note: storeForm.note.trim() || undefined,
            ...
            {[...buildingCards].sort((a,b)=>(a.sort_order ?? 9999)-(b.sort_order ?? 9999)).map(store=>{
              ...
              return <section key={store.id} className="bulk-store-customer-box" draggable onDragOver={e=>{if(e.dataTransfer.types.includes("application/x-bulk-location")){e.preventDefault();e.currentTarget.classList.add("bulk-location-drop-target")}}} onDragLeave={e=>e.currentTarget.classList.remove("bulk-location-drop-target")} onDrop={e=>{const source=e.dataTransfer.getData("application/x-bulk-location");if(!source)return;e.preventDefault();e.stopPropagation();e.currentTarget.classList.remove("bulk-location-drop-target");void reorderBuildings(source,store.id);}} onDragStart={e=>{e.dataTransfer.setData("text/plain",store.id);setDragged({type:"store",id:store.id})}} onDragEnd={()=>setDragged(null)} title="Drag this location to a vehicle to load its scheduled invoices">
                <div className="bulk-store-hero">
                  <button type="button" className="bulk-store-reorder-handle" title="Drag to reorder this location" draggable onClick={e=>e.stopPropagation()} onDragStart={e=>{e.stopPropagation();e.dataTransfer.setData("application/x-bulk-location",store.id)}} onDragEnd={()=>{}}><GripVertical size={16}/></button><div className={`bulk-store-picture ${storeInvoices.length && storeInvoices.every(i => invoiceLoaded(i) >= Number(i.pallets || 0)) ? "store-complete" : ""}`}>{storeIcon(store.type)}{storeInvoices.length > 0 && storeInvoices.every(i => invoiceLoaded(i) >= Number(i.pallets || 0)) && <span className="bulk-store-complete-check" title="All store invoices are loaded into vehicles"><Check size={10}/></span>}</div>
                ...
                <div className="bulk-store-schedule-sections"><div className="bulk-store-schedule-section today"><div className="bulk-schedule-section-heading"><div><span className="bulk-section-kicker">TODAY</span><h4>Schedule for today</h4></div><b>{storeInvoices.filter(i=>i.scheduled_date===date).length} invoice(s)</b></div><div className="bulk-customer-grid compact-store-grid">
