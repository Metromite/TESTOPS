...
export default function BulkOrganizer() {
  ...
  async function loadSourceData(selectedDate: string) {
    ...
    try {
      ...
      setPlan({ ...old, plan_date: selectedDate, vehicles: savedVehicles.length ? savedVehicles : defaults, customers: Array.from(customerMap.values()), invoices: merged, pallets: merged as any, pallet_assignments: assignments });
      setDirty(false);
  ...
  function updatePlan(updater: (current: PlanV2) => PlanV2) {
    setDirty(true);
    setPlan(updater);
  }
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      void (async () => {
        try {
          await saveBulkOrganizerPlan({ ...plan, pallets: (plan.invoices || []) as any });
          setDirty(false);
          setError("");
        } catch (e: any) {
          setError(e?.message || "Automatic save failed");
        }
      })();
    }, 450);
  ...
  const defaultVehicles = fleet.filter(v => isVan(v.type) || isPickup(v.type));
  ...
  function readDrop(e: React.DragEvent) {
    const invoiceId = e.dataTransfer.getData("application/x-dispatchops-invoice");
    if (invoiceId) return { type: "invoice" as const, id: invoiceId };
    const raw = e.dataTransfer.getData("text/plain");
    if (raw.startsWith("invoice:")) return { type: "invoice" as const, id: raw.slice(8) };
    if (raw.includes("::")) return { type: "pallet" as const, id: raw };
    return dragged;
  }
  ...
  async function save() { setSaving(true); setError(""); try { await saveBulkOrganizerPlan({ ...plan, pallets: invoices as any }); setDirty(false); } catch (e: any) { setError(e?.message || "Save failed"); } finally { setSaving(false); } }
  function toggleVehicle(v: any) {
    updatePlan(x => x.vehicles.some(z => z.vehicle_id === v.id)
  ...
  function addCustomer() {
    ...
    updatePlan(x => ({ ...x, customers: [...(x.customers || []), { id, name, area: customerForm.area, division: customerForm.division || "Unknown", enabled: true }] }));
  ...
  function addInvoice() {
    ...
    updatePlan(x => ({ ...x, customers: [...(x.customers || []), ...newCustomer], invoices: [...(x.invoices || []), inv], pallets: [...(x.invoices || []), inv] as any }));
  ...
  function updateInvoice() {
    ...
    updatePlan(x => ({ ...x, invoices: (x.invoices || []).map(i => i.id === editingInvoice.id ? { ...i, invoice_no: invoiceForm.invoice_no, customer_name: invoiceForm.customer_name, area: invoiceForm.area, division: invoiceForm.division, pallets: Math.max(1, Number(invoiceForm.pallets || 1)), invoice_date: invoiceForm.invoice_date || null, scheduled_date: invoiceForm.schedule_date || date } : i), pallets: (x.invoices || []).map(i => i.id === editingInvoice.id ? { ...i, invoice_no: invoiceForm.invoice_no, customer_name: invoiceForm.customer_name, area: invoiceForm.area, division: invoiceForm.division, pallets: Math.max(1, Number(invoiceForm.pallets || 1)), invoice_date: invoiceForm.invoice_date || null, scheduled_date: invoiceForm.schedule_date || date } : i) as any }));
  ...
  function removeCustomer(id: string) { updatePlan(x => { const has = (x.invoices || []).some(i => i.customer_id === id); if (has) { setError("Move or remove this customer's invoices before removing the customer."); return x; } return { ...x, customers: (x.customers || []).filter(c => c.id !== id) }; }); }
  function deleteInvoice(id: string) { updatePlan(x => { const next = { ...(x.pallet_assignments || {}) }; Object.keys(next).filter(k => k.startsWith(`${id}::`)).forEach(k => delete next[k]); return { ...x, invoices: (x.invoices || []).filter(i => i.id !== id), pallets: (x.invoices || []).filter(i => i.id !== id) as any, pallet_assignments: next }; }); }
  ...
  return <div className="page bulk-organizer-page" style={{ paddingBottom: 90 }}>
    ...
    <section className="bulk-section">
      <div className="bulk-section-head"><div><h2>Vehicles</h2><div className="muted small">Everyday defaults: {defaultVehicles.length ? defaultVehicles.map(v => v.number).join(", ") : "None"}. These are active Vans and Pick-Ups from Fleet Database. Add/remove vehicles for this date.</div></div><GlassButton variant="secondary" size="sm" onClick={() => setShowFleet(x => !x)}><Database size={14} /> {showFleet ? "Hide Fleet" : "Add / Remove Vehicles"}</GlassButton></div>
      {showFleet && <GlassCard style={{ marginBottom: 10 }}><div className="bulk-vehicle-picker">{fleet.map(v => { const on = selectedVehicles.some(x => x.vehicle_id === v.id); return <button key={v.id} className={`bulk-fleet-choice ${on ? "selected" : ""}`} onClick={() => toggleVehicle(v)}><span className="bulk-icon-circle">{isPickup(v.type) ? <Truck size={16} /> : <CarFront size={16} />}</span><span><strong>{v.number}</strong><small>{v.type} · {v.division || "No division"}</small>{(isVan(v.type) || isPickup(v.type)) && <em className="bulk-default-label">Everyday default</em>}</span>{on && <Check size={15} />}</button>; })}</div></GlassCard>}
      <div className="bulk-vehicle-grid">
        {selectedVehicles.map(cfg => { const v = vehicleMap.get(cfg.vehicle_id); if (!v) return null; const itemIds = new Set(Object.entries(assignments).filter(([, val]) => val === v.id).map(([key]) => key.split("::")[0])); const items = invoices.filter(i => itemIds.has(i.id)); const used = Object.values(assignments).filter(val => val === v.id).length; const hoverDrag = dragOverVehicleId === v.id ? readHoverDragCapacity(invoices, dragged, v.id, cfg.capacity, assignments) : 0; const previewUsed = Math.min(cfg.capacity, used + hoverDrag); const free = Math.max(0, cfg.capacity - used); const pct = Math.min(100, used / Math.max(1, cfg.capacity) * 100); const previewPct = Math.min(100, previewUsed / Math.max(1, cfg.capacity) * 100); const permittedNames = permissions.filter(x => x.vehicle_id === v.id).map(x => x.areas?.code || x.areas?.name).filter(Boolean);
          return <div key={v.id} className="glass-card bulk-vehicle-card" onDragOver={e => { e.preventDefault(); setDragOverVehicleId(v.id); }} onDragLeave={() => setDragOverVehicleId(id => id === v.id ? null : id)} onDrop={e => { e.preventDefault(); setDragOverVehicleId(null); const activeDrag = readDrop(e); if (!activeDrag) return; const inv = invoices.find(i => activeDrag.type === "invoice" ? i.id === activeDrag.id : activeDrag.id.startsWith(`${i.id}::`)); if (inv) { const check = vehiclePermission(inv, v); const adding = activeDrag.type === "pallet" ? 1 : Math.max(1, inv.pallets); if (check.ok && used + adding <= cfg.capacity) { if (activeDrag.type === "invoice") assignInvoice(inv.id, v.id); else assignPallet(activeDrag.id, v.id); } else setError(`${v.number}: ${check.text || `capacity exceeded (${free} free)`}`); } setDragged(null); }}>
            <div className="bulk-vehicle-title"><div style={{ display: "flex", gap: 10, alignItems: "center" }}><span className="bulk-icon-circle vehicle">{isPickup(v.type) ? <Truck size={21} /> : <CarFront size={21} />}</span><div><strong>{v.number}</strong><div className="muted small">{v.type} · {v.division || "—"} <span className="bulk-default-label">Everyday default</span></div></div></div><button className="btn icon-btn" title="Remove from day" onClick={() => toggleVehicle(v)}><X size={14} /></button></div>
            ...
            <div className="bulk-capacity"><div><b>{used} / {cfg.capacity}</b> pallets</div><span>{free} free</span></div><div className={`bulk-capacity-bar ${dragOverVehicleId === v.id ? "preview" : ""}`}><div style={{ width: `${previewPct}%` }} className={previewPct >= 100 ? "full" : (dragOverVehicleId === v.id ? "preview-fill" : "")} /></div>
            <div className="bulk-dropzone">{items.length ? items.map(i => <InvoiceMini key={i.id} invoice={i} onDragStart={() => setDragged({ type: "invoice", id: i.id })} onEdit={() => editInvoice(i)} />) : <span className="muted small">Drop an invoice or one pallet here</span>}</div>
            <label className="muted small bulk-capacity-input">Capacity <input type="number" min={1} value={cfg.capacity} onChange={e => updatePlan(x => ({ ...x, vehicles: x.vehicles.map(z => z.vehicle_id === v.id ? { ...z, capacity: Math.max(1, Number(e.target.value)) } : z) }))} /></label>
          </div>;
        })}
      </div>
    </section>
    ...
    <section className="bulk-section">
      ...
      <div className="bulk-customer-grid">
        {customerCards.map(c => { const cis = invoices.filter(i => i.customer_id === c.id || norm(i.customer_name) === norm(c.name)); const cp = cis.reduce((n, i) => n + i.pallets, 0); return <div key={c.id} className="glass-card bulk-customer-card" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); const activeDrag = readDrop(e); if (!activeDrag || activeDrag.type !== "invoice") return; const i = invoices.find(x => x.id === activeDrag.id); if (i) updatePlan(x => ({ ...x, invoices: (x.invoices || []).map(z => z.id === i.id ? { ...z, customer_id: c.id, customer_name: c.name, area: c.area || z.area, division: c.division || z.division } : z), pallets: (x.invoices || []).map(z => z.id === i.id ? { ...z, customer_id: c.id, customer_name: c.name, area: c.area || z.area, division: c.division || z.division } : z) as any })); setDragged(null); }}>
          ...
          <div className="bulk-customer-body">{cis.map(i => <InvoiceCard key={i.id} invoice={i} onDragStart={() => setDragged({ type: "invoice", id: i.id })} onEdit={() => editInvoice(i)} onDelete={() => deleteInvoice(i.id)} showPallets assignments={assignments} />)}{!cis.length && <span className="muted small">Drop an invoice here</span>}</div>
        </div>; })}
      </div>
    </section>
    ...
    <section className="bulk-section">
      ...
      <div className="bulk-pool-grid" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); const activeDrag = readDrop(e); if (!activeDrag) return; if (activeDrag.type === "invoice") assignInvoice(activeDrag.id, null); else assignPallet(activeDrag.id, null); setDragged(null); }}>{visibleInvoices.filter(i => Array.from({ length: Math.max(1, i.pallets) }).some((_, k) => !assignments[`${i.id}::${k}`])).map(i => <InvoiceCard key={i.id} invoice={i} onDragStart={() => setDragged({ type: "invoice", id: i.id })} onEdit={() => editInvoice(i)} onDelete={() => deleteInvoice(i.id)} showPallets assignments={assignments} />)}</div>
    </section>
...
function readHoverDragCapacity(invoices: InvoicePlan[], dragged: { type: "invoice" | "pallet"; id: string } | null, vehicleId: string, capacity: number, assignments: Record<string, string | null>) {
  if (!dragged) return 0;
  if (dragged.type === "pallet") return assignments[dragged.id] === vehicleId ? 0 : 1;
  const inv = invoices.find(i => i.id === dragged.id);
  if (!inv) return 0;
  const already = Array.from({ length: Math.max(1, inv.pallets) }).filter((_, k) => assignments[`${inv.id}::${k}`] === vehicleId).length;
  return Math.max(0, Math.min(Math.max(1, inv.pallets) - already, capacity));
}
