...
export default function BulkOrganizer() {
  ...
  async function loadSourceData(selectedDate: string) {
    ...
    try {
      ...
      const [v, a, p, f, allCustomers, saved, savedDefaults] = await Promise.all([
        supabase.from("vehicles").select("id,number,type,division,status,capacity_tons,pallet_capacity").order("number"),
  ...
  useEffect(() => {
    ...
    const loadWallpaper = async () => {
      try {
        ...
        const dark = document.documentElement.classList.contains("dark") || localStorage.getItem("theme-mode") === "dark" || (localStorage.getItem("theme-mode") === "auto" && window.matchMedia("(prefers-color-scheme: dark)").matches);
        const path = dark ? config?.dark_bg_path : config?.light_bg_path;
        const url = path ? await getWallpaperUrl(path, 3600) : null;
        if (!cancelled) setBulkWallpaper(url);
      } catch { if (!cancelled) setBulkWallpaper(null); }
    };
    void loadWallpaper();
  ...
  if (loading) return <div className="page bulk-planner-page is-loading">{bulkWallpaper && <div className="bulk-page-wallpaper" style={{ backgroundImage: `url("${bulkWallpaper}")` }} aria-hidden="true" />}<div className="bulk-loading-shell glass-card"><div className="bulk-loading-orb"/><b>Loading Bulk Organizer</b><span>Preparing today&apos;s fleet and schedule…</span></div></div>;
  return <div className="page bulk-planner-page">
