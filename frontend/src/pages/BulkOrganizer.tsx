...
import { supabase, getTertiarySupabaseClient } from "../lib/supabase";
...
type InvoicePlan = {
  ...
  division: string; pallets: number; big_pallets?: number; small_pallets?: number;
...
type VehicleConfig = { vehicle_id: string; capacity: number; big_capacity?: number; small_capacity?: number; driver?: string; helper?: string; building_id?: string | null };
...
export default function BulkOrganizer() {
  ...
  const [invoiceForm, setInvoiceForm] = useState({ invoice_no: "", customer_id: "", customer_name: "", area: "", division: "", pallets: 1, big_pallets: 1, small_pallets: 0, invoice_date: "", schedule_date: "", building_id: "" });
  ...
  async function loadSourceData(selectedDate: string) {
    ...
    try {
      ...
      const [v, a, p, saved, savedDefaults, departmentEntriesResult] = await Promise.all([
      ...
      const departmentInvoices: InvoicePlan[] = departmentEntries
        ...
        .map(e => ({
          ...
          pallet_types: Array.from({length: Math.max(1, Number(e.pallet_count || 1))}, (_, k) => k < Number(e.big_pallet_count || 0) ? "big" : "small"),
      ...
      const mergedDepartmentInvoices = departmentInvoices.filter(di => !(old.invoices || []).some((i:any) => i.source_entry_id === di.source_entry_id || i.id === di.id));
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "department_dispatch_entries", filter: `target_date=eq.${date}` },
        (payload: any) => {
          ...
          const incoming: InvoicePlan = {
            ...
            pallet_types: Array.from({length: Math.max(1, Number(row.pallet_count || 1))}, (_, k) => k < Number(row.big_pallet_count || 0) ? "big" : "small"),
  ...
  function openInvoiceForStore(store: Building) {
    ...
    setInvoiceForm(x => ({ ...x, invoice_no: "", customer_id: matched?.id || "", customer_name: others ? "" : store.name, area: store.area || "", division: store.division || divisionView, pallets: 1, big_pallets: 1, small_pallets: 0, invoice_date: date, schedule_date: date, building_id: store.id }));
  }
  ...
  function openInvoiceForCustomer(customer: any, store: Building) {
    ...
    setEditingInvoice(null); setInvoiceBuildingLocked(true); setInvoiceCustomerLocked(!others);
    setShowInvoiceManager(true); setScheduleMode("today");
    setInvoiceForm(x => ({ ...x, invoice_no: "", customer_id: matched?.id || customer.id || "", customer_name: others ? "" : customer.name, area: customer.area || store.area || matched?.area || "", division: customer.division || store.division || divisionView, pallets: 1, big_pallets: 1, small_pallets: 0, invoice_date: date, schedule_date: date, building_id: store.id }));
  }
  ...
  function dropStoreOnVehicle(v: any, storeId: string) {
    ...
    const cap = capacityFor(cfg || { vehicle_id: v.id, capacity: defaultCapacity(v), big_capacity: Number(v.big_pallet_capacity || 0), small_capacity: Number(v.small_pallet_capacity || 0) }, v);
  ...
  function dropOnVehicle(v: any, activeDrag: { type: string; id: string }) {
    ...
    if (!canFitInvoice(inv, cfg, v, adding)) { setError(`${v.number}: the big/small pallet capacity would be exceeded.`); return; }
    if (used + adding > capacityFor(cfg, v)) { setError(`${v.number}: only ${Math.max(0, capacityFor(cfg, v) - used)} pallet space left.`); return; }
  ...
  function toggleVehicle(v: any, fromDefaults = false) {
    ...
    if (fromDefaults) {
      setDefaults(x => x.vehicles.some(z => z.vehicle_id === v.id)
        ...
        : { ...x, vehicles: [...x.vehicles, { vehicle_id: v.id, capacity: defaultCapacity(v), big_capacity: Number(v.big_pallet_capacity || 0), small_capacity: Number(v.small_pallet_capacity || 0), driver: "", helper: "" }] });
      return;
    }
    setPlan(x => x.vehicles.some(z => z.vehicle_id === v.id)
      ...
      : { ...x, vehicles: [...x.vehicles, { vehicle_id: v.id, capacity: defaultCapacity(v), big_capacity: Number(v.big_pallet_capacity || 0), small_capacity: Number(v.small_pallet_capacity || 0) }] });
    void listKey;
  }
  ...
  function invoiceRowForStore(store: Building) {
    const others = isOthersStore(store);
    return { invoice_no: "", customer_id: "", customer_name: others ? "" : store.name, area: store.area || "", division: store.division || divisionView, pallets: 1, big_pallets: 1, small_pallets: 0, invoice_date: date, schedule_date: date, building_id: store.id };
  }
  ...
  async function addInvoiceBatch() {
    ...
    for (const r of targetRows) {
      ...
      newInvoices.push({ id:`manual-invoice:${crypto.randomUUID()}`, invoice_no:r.invoice_no, customer_id:customerId, customer_name:customer?.name || r.customer_name, area:r.area || customer?.area || "", division:r.division || customer?.division || "Unknown", pallets:Math.max(1,Number(r.big_pallets || 0)+Number(r.small_pallets || 0) || Number(r.pallets||1)),
        big_pallets:Math.max(0,Number(r.big_pallets || 0)), small_pallets:Math.max(0,Number(r.small_pallets || 0)),
        pallet_types:Array.from({length:Math.max(1,Number(r.big_pallets || 0)+Number(r.small_pallets || 0) || Number(r.pallets||1))},(_,k)=>k<Number(r.big_pallets||0)?"big":"small"),
    ...
    try {
      ...
      setInvoiceForm({ invoice_no:"",customer_id:"",customer_name:"",area:"",division:"",pallets:1,big_pallets:1,small_pallets:0,invoice_date:"",schedule_date:"",building_id:"" });
  ...
  async function addInvoice() {
    ...
    const inv: InvoicePlan = {
      ...
      pallets: Math.max(1, Number(invoiceForm.big_pallets || 0) + Number(invoiceForm.small_pallets || 0) || Number(invoiceForm.pallets || 1)),
      big_pallets: Math.max(0, Number(invoiceForm.big_pallets || 0)), small_pallets: Math.max(0, Number(invoiceForm.small_pallets || 0)),
      pallet_types: Array.from({length: Math.max(1, Number(invoiceForm.big_pallets || 0) + Number(invoiceForm.small_pallets || 0) || Number(invoiceForm.pallets || 1))}, (_, k) => k < Number(invoiceForm.big_pallets || 0) ? "big" : "small"),
  ...
  async function updateInvoice() {
    ...
    const next = { ...editingInvoice, invoice_no: invoiceForm.invoice_no, customer_name: invoiceForm.customer_name, area: invoiceForm.area, division: invoiceForm.division, pallets: Math.max(1, Number(invoiceForm.big_pallets || 0) + Number(invoiceForm.small_pallets || 0) || Number(invoiceForm.pallets || 1)),
      big_pallets: Math.max(0, Number(invoiceForm.big_pallets || 0)), small_pallets: Math.max(0, Number(invoiceForm.small_pallets || 0)),
      pallet_types: Array.from({length: Math.max(1, Number(invoiceForm.big_pallets || 0) + Number(invoiceForm.small_pallets || 0) || Number(invoiceForm.pallets || 1))}, (_, k) => k < Number(invoiceForm.big_pallets || 0) ? "big" : "small"),
  ...
  function updateVehicleCapacity(vehicleId: string, field: "big_capacity" | "small_capacity", value: number) {
    setPlan(x => ({ ...x, vehicles: (x.vehicles || []).map(v => v.vehicle_id === vehicleId ? { ...v, [field]: Math.max(0, Number(value || 0)) } : v) }));
  }
  function updateDefaultVehicleCapacity(vehicleId: string, field: "big_capacity" | "small_capacity", value: number) {
    setDefaults(x => ({ ...x, vehicles: (x.vehicles || []).map(v => v.vehicle_id === vehicleId ? { ...v, [field]: Math.max(0, Number(value || 0)) } : v) }));
  }
  ...
  const palletTypeAt = (inv: any, index: number): "big" | "small" => {
    if (Array.isArray(inv?.pallet_types) && inv.pallet_types[index]) return inv.pallet_types[index] === "small" ? "small" : "big";
    const big = Math.max(0, Number(inv?.big_pallets || 0));
    return index < big ? "big" : "small";
  };
  const vehicleTypedCapacity = (cfg: VehicleConfig, v: any) => ({
    big: Math.max(0, Number(v?.big_pallet_capacity ?? cfg?.big_capacity ?? 0)),
    small: Math.max(0, Number(v?.small_pallet_capacity ?? cfg?.small_capacity ?? 0)),
  });
  const assignedTypedCounts = (vehicleId: string) => {
    const out = { big: 0, small: 0 };
    Object.entries(assignments).forEach(([key, value]) => {
      if (value !== vehicleId) return;
      const [invoiceId, indexText] = key.split("::");
      const inv = allPlanInvoices.find(i => i.id === invoiceId);
      if (!inv) return;
      out[palletTypeAt(inv, Number(indexText || 0))] += 1;
    });
    return out;
  };
  const canFitInvoice = (inv: any, cfg: VehicleConfig, v: any, quantity?: number) => {
    const typedCap = vehicleTypedCapacity(cfg, v);
    if (!typedCap.big && !typedCap.small) return true;
    const used = assignedTypedCounts(v.id);
    const total = Math.max(1, Number(inv?.pallets || 0));
    const requested = Math.min(total, Math.max(1, Number(quantity || total)));
    const types = Array.from({ length: requested }, (_, k) => palletTypeAt(inv, Math.min(k, total - 1)));
    const need = { big: types.filter(t => t === "big").length, small: types.filter(t => t === "small").length };
    return used.big + need.big <= typedCap.big && used.small + need.small <= typedCap.small;
  };
...
<main className="bulk-customer-section glass-card bulk-glass-surface">
    ...
    {showBatchLoadModal&&<div className="bulk-modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)closeBatchLoadModal()}}><GlassCard className="bulk-planner-modal bulk-load-modal bulk-modal-surface">
      ...
      <div className="bulk-batch-vehicle-grid">{selectedVehicles.map(cfg=>{const v=vehicleMap.get(cfg.vehicle_id);if(!v)return null;const used=loadedByVehicle(v.id),cap=capacityFor(cfg,v),free=Math.max(0,cap-used),active=batchVehicleIds.includes(v.id);return <button type="button" key={v.id} className={`bulk-load-vehicle-choice bulk-batch-vehicle-choice ${active?"selected":""}`} onClick={()=>setBatchVehicleIds(ids=>ids.includes(v.id)?ids.filter(x=>x!==v.id):[...ids,v.id])}>
        <img src={vehicleImage(v)} alt=""/><div><b>{v.number}</b><span>{v.type||"Vehicle"}</span><small>{free} pallets free · {used}/{cap} loaded</small><small>{(plan.vehicle_meta?.[v.id]?.driver||cfg.driver||"No driver")} · {(plan.vehicle_meta?.[v.id]?.helper||cfg.helper||"No helper")}</small></div>{active&&<Check size={15}/>}
      </button>})}</div>
    ...
    {loadInvoiceTarget&&<div className="bulk-modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)closeInvoiceLoadModal()}}><GlassCard className="bulk-planner-modal bulk-load-modal bulk-modal-surface">
      ...
      <div className="bulk-load-vehicle-grid">{selectedVehicles.map(cfg=>{const v=vehicleMap.get(cfg.vehicle_id);if(!v)return null;const free=Math.max(0,capacityFor(cfg,v)-loadedByVehicle(cfg.vehicle_id));const active=loadVehicleId===v.id;return <button type="button" key={v.id} className={`bulk-load-vehicle-choice ${active?"selected":""}`} onClick={()=>setLoadVehicleId(v.id)}><img src={vehicleImage(v)} alt=""/><div><b>{v.number}</b><span>{v.type||"Vehicle"}</span><small>{free} pallets free</small><small>{(plan.vehicle_meta?.[v.id]?.driver||cfg.driver||"No driver")} · {(plan.vehicle_meta?.[v.id]?.helper||cfg.helper||"No helper")}</small></div>{active&&<Check size={15}/>}</button>})}</div>
      ...
      {(showInvoiceManager||editingInvoice||invoiceForm.invoice_no)&&<div className="bulk-modal-block">
        ...
        {invoiceBatchMode&&!editingInvoice ? <div className="bulk-invoice-batch-list">
          {(invoiceBatchRows.length?invoiceBatchRows:[invoiceForm]).map((row,idx)=>{const rowBuilding=(plan.buildings||[]).find(b=>b.id===row.building_id);const rowDates=allowedDatesForBuilding(row.building_id,row.schedule_date);return <div className="bulk-invoice-batch-row" key={idx}>
            ...
            <div className="bulk-batch-pallet-fields"><label><span>Big</span><input type="number" min={0} value={(row as any).big_pallets ?? 1} onChange={e=>setInvoiceBatchRows(rows=>{const base=rows.length?rows:[{...invoiceForm}];return base.map((r,j)=>j===idx?{...r,big_pallets:Math.max(0,Number(e.target.value||0)),pallets:Math.max(1,Number(e.target.value||0)+Number(r.small_pallets||0))}:r)})}/></label><label><span>Small</span><input type="number" min={0} value={(row as any).small_pallets ?? 0} onChange={e=>setInvoiceBatchRows(rows=>{const base=rows.length?rows:[{...invoiceForm}];return base.map((r,j)=>j===idx?{...r,small_pallets:Math.max(0,Number(e.target.value||0)),pallets:Math.max(1,Number(r.big_pallets||0)+Number(e.target.value||0))}:r)})}/></label></div>
            ...
            <div className="bulk-batch-schedule-box" title="Schedule">
              <div className="bulk-schedule-choice-row">
                {rowDates.includes(date) && <button type="button" className={row.schedule_date===date?"active":""} onClick={()=>setInvoiceBatchRows(rows=>{const base=rows.length?rows:[{...invoiceForm}];return base.map((r,j)=>j===idx?{...r,schedule_date:date}:r)})}>{dayLabel(date)}</button>}
                <button type="button" className={!row.schedule_date?"active":""} onClick={()=>setInvoiceBatchRows(rows=>{const base=rows.length?rows:[{...invoiceForm}];return base.map((r,j)=>j===idx?{...r,schedule_date:""}:r)})}>Waiting</button>
                <button type="button" className={row.schedule_date && row.schedule_date!==date?"active":""} onClick={()=>{const first=row.schedule_date && rowDates.includes(row.schedule_date)?row.schedule_date:(rowDates[0]||"");setInvoiceBatchRows(rows=>{const base=rows.length?rows:[{...invoiceForm}];return base.map((r,j)=>j===idx?{...r,schedule_date:first}:r)})}}>Date</button>
              </div>
              {row.schedule_date && row.schedule_date!==date && <input className="bulk-schedule-date-input" type="date" value={row.schedule_date} min={rowDates[0]||undefined} onChange={e=>{const v=e.target.value;if(v && !rowDates.includes(v)){setError("Choose one of the allowed schedule dates for this store.");return;}setInvoiceBatchRows(rows=>{const base=rows.length?rows:[{...invoiceForm}];return base.map((r,j)=>j===idx?{...r,schedule_date:v}:r)})}} />}
            </div>
