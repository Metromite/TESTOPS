...
export default function BulkOrganizer() {
  ...
  return <div className={`page bulk-planner-page bulk-theme-${bulkTheme} ${loading ? "is-hydrating" : ""}`}>
    ...
    <div className="bulk-planner-grid">
      ...
      <main className="bulk-customer-section glass-card">
        ...
        <div className="bulk-store-customer-grid">
          {buildingCards.map(store=>{
            ...
            return <section key={store.id} className="bulk-store-customer-box">
              <div className="bulk-store-hero">
                <div className="bulk-store-picture">{storeIcon(store.type)}</div>
              ...
              <div className="bulk-customer-grid compact-store-grid">
                {storeCustomers.map(c=>{const ci=invoices.filter(i=>norm(i.customer_name)===norm(c.name) && (i.building_id===store.id || !i.building_id));const pallets=ci.reduce((n,i)=>n+i.pallets,0),loaded=assignedForCustomer(c.name),scheduled=plan.customer_schedules?.find(s=>norm(s.customer_name)===norm(c.name)&&s.days.includes(todayDow));return <div key={c.id} draggable onDragStart={e=>{e.dataTransfer.setData("text/plain",c.name);setDragged({type:"customer",id:c.name})}} onDragEnd={()=>setDragged(null)} className={`bulk-customer-card ${loaded>=pallets&&pallets?"complete":""}`}>
                  ...
                  <div className="customer-card-footer"><span>{loaded>=pallets&&pallets?"FULLY LINKED":"Drag to a permitted vehicle"}</span>{loaded>=pallets&&pallets?<Check size={13}/>:<Route size={13}/>}</div>
                </div>})}
                {!storeCustomers.length&&<div className="bulk-empty-state"><Store size={22}/><b>No scheduled customers for this store</b><span>Add an invoice/customer schedule and select this destination.</span></div>}
              </div>
            </section>;
          })}
