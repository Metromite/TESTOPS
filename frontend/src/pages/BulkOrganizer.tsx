...
export default function BulkOrganizer() {
  ...
  return <div className="page bulk-organizer-page" style={{ paddingBottom: 90 }}>
    ...
    <section className="bulk-section">
      ...
      <div className="bulk-vehicle-grid">
        {selectedVehicles.map(cfg => { const v = vehicleMap.get(cfg.vehicle_id); if (!v) return null; const itemIds = new Set(Object.entries(assignments).filter(([, val]) => val === v.id).map(([key]) => key.split("::")[0])); const items = invoices.filter(i => itemIds.has(i.id)); const used = Object.values(assignments).filter(val => val === v.id).length; const free = Math.max(0, cfg.capacity - used); const pct = Math.min(100, used / Math.max(1, cfg.capacity) * 100); const permittedNames = permissions.filter(x => x.vehicle_id === v.id).map(x => x.areas?.code || x.areas?.name).filter(Boolean);
          return <div key={v.id} className="glass-card bulk-vehicle-card" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); const nativePallet = e.dataTransfer.getData("text/plain"); const activeDrag = dragged || (nativePallet ? { type: "pallet" as const, id: nativePallet } : null); if (!activeDrag) return; const inv = invoices.find(i => activeDrag.type === "invoice" ? i.id === activeDrag.id : activeDrag.id.startsWith(`${i.id}::`)); if (inv) { const check = vehiclePermission(inv, v); const adding = activeDrag.type === "pallet" ? 1 : Math.max(1, inv.pallets); if (check.ok && used + adding <= cfg.capacity) { if (activeDrag.type === "invoice") assignInvoice(inv.id, v.id); else assignPallet(activeDrag.id, v.id); } else setError(`${v.number}: ${check.text || `capacity exceeded (${free} free)`}`); } setDragged(null); }}>
            <div className="bulk-vehicle-title"><div style={{ display: "flex", gap: 10, alignItems: "center" }}><span className="bulk-icon-circle vehicle">{isPickup(v.type) ? <Truck size={21} /> : <CarFront size={21} />}</span><div><strong>{v.number}</strong><div className="muted small">{v.type} · {v.division || "—"}</div></div></div><button className="btn icon-btn" title="Remove from day" onClick={() => toggleVehicle(v)}><X size={14} /></button></div>
    ...
    <section className="bulk-section">
      ...
      <div className="bulk-pool-grid" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); const nativePallet = e.dataTransfer.getData("text/plain"); const activeDrag = dragged || (nativePallet ? { type: "pallet" as const, id: nativePallet } : null); if (!activeDrag) return; if (activeDrag.type === "invoice") assignInvoice(activeDrag.id, null); else assignPallet(activeDrag.id, null); setDragged(null); }}>{visibleInvoices.filter(i => Array.from({ length: Math.max(1, i.pallets) }).some((_, k) => !assignments[`${i.id}::${k}`])).map(i => <InvoiceCard key={i.id} invoice={i} onDragStart={() => setDragged({ type: "invoice", id: i.id })} onEdit={() => editInvoice(i)} onDelete={() => deleteInvoice(i.id)} showPallets assignments={assignments} />)}</div>
    </section>
