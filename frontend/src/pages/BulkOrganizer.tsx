...
const SMALL_TO_BIG = 2 / 3; // 1 small pallet = 2/3 of one big-pallet space; 1 big = 1.5 small.
const palletEquivalent = (size: "big" | "small", quantity: number) => Math.max(0, Number(quantity || 0)) * (size === "small" ? SMALL_TO_BIG : 1);
const formatEquivalent = (value: number) => { const n = Math.round(value * 100) / 100; return Number.isInteger(n) ? String(n) : n.toFixed(2).replace(/0+$/, "").replace(/\.$/, ""); };
...
export default function BulkOrganizer() {
  ...
  const [invoiceForm, setInvoiceForm] = useState({ invoice_no: "", customer_id: "", customer_name: "", area: "", division: "", pallets: 1, pallet_size: "big" as "big" | "small", pallet_quantity: 1, invoice_date: "", schedule_date: "", building_id: "" });
  ...
  function moveInvoicePallets(inv: InvoicePlan, fromVehicleId: string, toVehicleId: string, qty: number) {
    ...
    const requestedEquivalent = palletEquivalent(invoicePalletSize(inv), amount);
  ...
  function confirmBatchLoad() {
    ...
    for (const inv of selected) {
      ...
      for (const cfg of batchVehicles) {
        ...
        const perPallet = palletEquivalent(invoicePalletSize(inv), 1);
        ...
        usedByVehicle.set(vehicle.id, used + palletEquivalent(invoicePalletSize(inv), placedOnVehicle));
  ...
  function confirmInvoiceLoad() {
    ...
    const requestedEquivalent = palletEquivalent(invoicePalletSize(inv), qty);
  ...
  function openInvoiceForStore(store: Building) {
    ...
    setInvoiceForm(x => ({ ...x, invoice_no: "", customer_id: matched?.id || "", customer_name: others ? "" : store.name, area: store.area || "", division: store.division || divisionView, pallets: 1, pallet_size: "big" as "big" | "small", pallet_quantity: 1, invoice_date: date, schedule_date: date, building_id: store.id }));
  }
  ...
  function dropStoreOnVehicle(v: any, storeId: string) {
    ...
    for (const inv of storeInvoices) {
      ...
      const requestedEquivalent = palletEquivalent(invoicePalletSize(inv), remaining);
  ...
  function dropOnVehicle(v: any, activeDrag: { type: string; id: string }) {
    ...
    const addingEquivalent = activeDrag.type === "pallet" ? palletEquivalent(invoicePalletSize(inv), 1) : palletEquivalent(invoicePalletSize(inv), adding);
  ...
  function invoiceRowForStore(store: Building) {
    const others = isOthersStore(store);
    return { invoice_no: "", customer_id: "", customer_name: others ? "" : store.name, area: store.area || "", division: store.division || divisionView, pallets: 1, pallet_size: "big" as "big" | "small", pallet_quantity: 1, invoice_date: date, schedule_date: date, building_id: store.id };
  }
  ...
  async function addInvoiceBatch() {
    ...
    for (const r of targetRows) {
      ...
      newInvoices.push({ id:`manual-invoice:${crypto.randomUUID()}`, invoice_no:r.invoice_no, customer_id:customerId, customer_name:customer?.name || r.customer_name, area:r.area || customer?.area || "", division:r.division || customer?.division || "Unknown", pallets:Math.max(1,Number(r.pallet_quantity || r.pallets || 1)),
        ...
        pallet_types:Array.from({length:Math.max(1,Number(r.pallet_quantity || r.pallets || 1))},()=>r.pallet_size === "small" ? "small" : "big"),
    ...
    try {
      ...
      setInvoiceForm({ invoice_no:"",customer_id:"",customer_name:"",area:"",division:"",pallets:1,pallet_size:"big",pallet_quantity:1,invoice_date:"",schedule_date:"",building_id:"" });
  ...
  async function addInvoice() {
    ...
    const inv: InvoicePlan = {
      ...
      pallets: Math.max(1, Number(invoiceForm.pallet_quantity || 1)),
      pallet_size: invoiceForm.pallet_size, pallet_quantity: Math.max(1, Number(invoiceForm.pallet_quantity || 1)),
      pallet_types: Array.from({length: Math.max(1, Number(invoiceForm.pallet_quantity || 1))}, () => invoiceForm.pallet_size),
    ...
    setInvoiceForm({ invoice_no: "", customer_id: "", customer_name: "", area: "", division: "", pallets: 1, pallet_size: "big" as "big" | "small", pallet_quantity: 1, invoice_date: "", schedule_date: "", building_id: "" });
    setShowInvoiceManager(false);
  }

  async function updateInvoice() {
    if (!editingInvoice) return;
    const next = { ...editingInvoice, invoice_no: invoiceForm.invoice_no, customer_name: invoiceForm.customer_name, area: invoiceForm.area, division: invoiceForm.division, pallets: Math.max(1, Number(invoiceForm.pallet_quantity || 1)),
      pallet_size: invoiceForm.pallet_size, pallet_quantity: Math.max(1, Number(invoiceForm.pallet_quantity || 1)),
      pallet_types: Array.from({length: Math.max(1, Number(invoiceForm.pallet_quantity || 1))}, () => invoiceForm.pallet_size),
  ...
  const loadedByVehicleByDivision = (vehicleId: string, div: string) => { let used = 0; Object.entries(assignments).forEach(([key,value]) => { if (value !== vehicleId) return; const [invoiceId] = key.split("::"); const inv = allPlanInvoices.find(i => i.id === invoiceId && i.scheduled_date === date && norm(i.division) === norm(div)); if (inv) used += palletEquivalent(invoicePalletSize(inv), 1); }); return used; };
  ...
  const invoicePalletSize = (inv: any): "big" | "small" => inv?.pallet_size === "small" ? "small" : "big";
  const invoicePhysicalQuantity = (inv: any) => Math.max(1, Number(inv?.pallet_quantity ?? inv?.pallets ?? 1));
  const invoiceEquivalentCapacity = (inv: any, quantity?: number) => palletEquivalent(invoicePalletSize(inv), Math.min(invoicePhysicalQuantity(inv), Math.max(1, Number(quantity || invoicePhysicalQuantity(inv)))));
  const assignedCapacityOnVehicle = (vehicleId: string) => {
    ...
    Object.entries(assignments).forEach(([key, value]) => {
      ...
      if (inv) used += palletEquivalent(invoicePalletSize(inv), 1);
    });
    return used;
  };
  ...
  const dropCustomerOnVehicle = (v: any, customerName: string) => {
    ...
    for (const inv of customerInvoices) {
      ...
      const cfg = selectedVehicles.find(x => x.vehicle_id === v.id);
      const used = loadedByVehicle(v.id);
      const requestedEquivalent = palletEquivalent(invoicePalletSize(inv), unassigned);
      if (!check.ok) { setError(`${v.number}: ${check.text}`); return; }
        ...
        {invoiceBatchMode&&!editingInvoice ? <div className="bulk-invoice-batch-list bulk-invoice-batch-cards">
          {(invoiceBatchRows.length?invoiceBatchRows:[invoiceForm]).map((row,idx)=>{
            ...
            return <div className="bulk-invoice-form bulk-invoice-batch-card" key={idx}>
              ...
              <div className="bulk-pallet-size-fields"><label className="bulk-field-with-label"><span>Pallet type</span><select value={row.pallet_size} onChange={e=>updateRow({pallet_size:e.target.value as "big" | "small"})}><option value="big">Big pallet</option><option value="small">Small pallet</option></select></label><label className="bulk-field-with-label"><span>Quantity</span><input type="number" min={1} step={1} value={row.pallet_quantity} onChange={e=>updateRow({pallet_quantity:Math.max(1,Number(e.target.value||1)),pallets:Math.max(1,Number(e.target.value||1))})}/></label><span className="bulk-pallet-total-chip">Load {formatEquivalent(palletEquivalent(row.pallet_size, Number(row.pallet_quantity || 1)))} big</span></div>
              ...
              <label className="bulk-field-with-label"><span>Schedule</span><div className="bulk-schedule-choice-box"><div className="bulk-schedule-choice-row">{rowAllowsToday&&<button type="button" className={row.schedule_date===date?"active":""} onClick={()=>updateRow({schedule_date:date})}>{dayLabel(date)}</button>}<button type="button" className={!row.schedule_date?"active":""} onClick={()=>updateRow({schedule_date:""})}>Waiting</button><button type="button" className={batchScheduleOpen[idx] ? "active" : ""} onClick={()=>{const first=row.schedule_date&&rowDates.includes(row.schedule_date)?row.schedule_date:"";setBatchScheduleOpen(x=>({...x,[idx]:true}));updateRow({schedule_date:first})}}>Date</button></div>{batchScheduleOpen[idx]&&<div className="bulk-batch-schedule-picker"><input className="bulk-schedule-date-input" type="date" value={row.schedule_date} min={rowDates[0]||undefined} onChange={e=>{const v=e.target.value;if(v && rowDates.length && !rowDates.includes(v)){setError("Choose one of the allowed schedule dates for this store.");return;}updateRow({schedule_date:v})}}/><span>{row.schedule_date ? dayLabel(row.schedule_date) : "Select schedule date"}</span></div>}</div></label>
            </div>;
          })}
