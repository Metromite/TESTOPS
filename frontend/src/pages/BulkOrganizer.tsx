...
export default function BulkOrganizer() {
  ...
  async function addStore() {
    ...
    setStoreDatePicker("");
    // Persist store + customer schedule immediately so the selection survives
    // date changes and page reloads instead of falling back to the old default.
    const latestDefaults: DefaultsV2 = {
      ...defaultsRef.current,
      month_key: monthKey(date),
      buildings: [...(planRef.current.buildings || [])],
      customer_schedules: [...(planRef.current.customer_schedules || [])],
    ...
    try {
      const saved = await saveBulkOrganizerDefaults(latestDefaults);
