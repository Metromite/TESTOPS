...
const defaultCapacity = (typeOrVehicle: any) => {
  ...
  if (v) {
    const splitTotal = Number(v.big_pallet_capacity ?? 0) + Number(v.small_pallet_capacity ?? 0);
...
export default function BulkOrganizer() {
  ...
  function confirmBatchLoad() {
    ...
    for (const inv of selected) {
      ...
      for (const cfg of batchVehicles) {
        ...
        let qty = Math.min(room, remaining);
  ...
  function confirmInvoiceLoad() {
    ...
    if (!canFitInvoice(inv, cfg, vehicle, qty)) {
  ...
  const vehicleTypedCapacity = (_cfg: VehicleConfig, v: any) => ({
    big: Math.max(0, Number(v?.big_pallet_capacity ?? 0)),
    small: Math.max(0, Number(v?.small_pallet_capacity ?? 0)),
  });
        ...
        {invoiceBatchMode&&!editingInvoice ? <div className="bulk-invoice-batch-list bulk-invoice-batch-cards">
          {(invoiceBatchRows.length?invoiceBatchRows:[invoiceForm]).map((row,idx)=>{
            ...
            return <div className="bulk-invoice-form bulk-invoice-batch-card" key={idx}>
              ...
              <div className="bulk-pallet-size-fields"><label className="bulk-field-with-label"><span>Big pallets</span><input type="number" min={0} value={row.big_pallets} onChange={e=>{const big=Math.max(0,Number(e.target.value||0));updateRow({big_pallets:big,pallets:Math.max(1,big+Number(row.small_pallets||0))})}}/></label><label className="bulk-field-with-label"><span>Small pallets</span><input type="number" min={0} value={row.small_pallets} onChange={e=>{const small=Math.max(0,Number(e.target.value||0));updateRow({small_pallets:small,pallets:Math.max(1,Number(row.big_pallets||0)+small)})}}/></label><span className="bulk-pallet-total-chip">Total {Math.max(1,Number(row.big_pallets||0)+Number(row.small_pallets||0))}</span></div>
              ...
              <label className="bulk-field-with-label"><span>Schedule</span><div className="bulk-schedule-choice-box"><div className="bulk-schedule-choice-row">{rowAllowsToday&&<button type="button" className={row.schedule_date===date?"active":""} onClick={()=>updateRow({schedule_date:date})}>{dayLabel(date)}</button>}<button type="button" className={!row.schedule_date?"active":""} onClick={()=>updateRow({schedule_date:""})}>Waiting</button><button type="button" className={row.schedule_date&&row.schedule_date!==date?"active":""} onClick={()=>{const first=row.schedule_date&&rowDates.includes(row.schedule_date)?row.schedule_date:(rowDates[0]||"");updateRow({schedule_date:first})}}>Date</button></div>{row.schedule_date&&row.schedule_date!==date&&<input className="bulk-schedule-date-input" type="date" value={row.schedule_date} min={rowDates[0]||undefined} onChange={e=>{const v=e.target.value;if(v&&!rowDates.includes(v)){setError("Choose one of the allowed schedule dates for this store.");return;}updateRow({schedule_date:v})}}/>}</div></label>
            </div>;
          })}
