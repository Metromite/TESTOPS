...
export default function BulkOrganizer() {
  ...
  function openInvoiceForStore(store: Building) {
    const isOthers = norm(store.name) === "others";
    const matched = customers.find(c => norm(c.name) === norm(store.name));
    setEditingInvoice(null); setInvoiceBuildingLocked(true); setInvoiceCustomerLocked(!isOthers);
    setShowInvoiceManager(true); setScheduleMode("today");
    setInvoiceForm(x => ({ ...x, invoice_no: "", customer_id: isOthers ? "" : (matched?.id || `manual:${norm(store.name)}`), customer_name: isOthers ? "" : store.name, area: store.area || matched?.area || "", division: matched?.division || store.division || divisionView, pallets: 1, invoice_date: date, schedule_date: date, building_id: store.id }));
  }
  ...
  function openInvoiceForCustomer(customer: any, store: Building) {
    ...
    setEditingInvoice(null); setInvoiceBuildingLocked(true); setInvoiceCustomerLocked(true);
    setShowInvoiceManager(true); setScheduleMode("today");
    setInvoiceForm(x => ({ ...x, invoice_no: "", customer_id: matched?.id || customer.id || "", customer_name: customer.name, area: customer.area || store.area || matched?.area || "", division: customer.division || store.division || divisionView, pallets: 1, invoice_date: date, schedule_date: date, building_id: store.id }));
  }
...
<main className="bulk-customer-section glass-card bulk-glass-surface">
    ...
    {loadInvoiceTarget&&<div className="bulk-modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)closeInvoiceLoadModal()}}><GlassCard className="bulk-planner-modal bulk-load-modal bulk-modal-surface">
      ...
      <div className="bulk-load-form">
        <label><span>Vehicle</span><select value={loadVehicleId} onChange={e=>setLoadVehicleId(e.target.value)}><option value="">Select vehicle</option>{selectedVehicles.map(cfg=>{const v=vehicleMap.get(cfg.vehicle_id);const free=Math.max(0,capacityFor(cfg,v)-loadedByVehicle(cfg.vehicle_id));return <option key={cfg.vehicle_id} value={cfg.vehicle_id}>{v?.number || cfg.vehicle_id} · {free} pallets free</option>})}</select></label>
      ...
      <div className="bulk-load-modal-actions"><GlassButton variant="secondary" size="sm" onClick={closeInvoiceLoadModal}>Cancel</GlassButton><GlassButton size="sm" onClick={confirmInvoiceLoad}><Truck size={13}/> Load {loadQuantity} pallet{loadQuantity===1?"":"s"}</GlassButton></div>
    </GlassCard></div>}
