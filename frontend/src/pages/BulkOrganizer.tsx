...
type Building = {
  id: string; name: string; division?: "Pharma" | "Consumer" | "Shared"; type: "warehouse" | "hospital" | "store" | "other" | "custom";
  custom_type?: string;
...
export default function BulkOrganizer() {
  ...
  const [divisionMode, setDivisionMode] = useState<"Pharma" | "Consumer">("Pharma");
  ...
  const [storeForm, setStoreForm] = useState({ name: "", type: "store" as Building["type"], custom_type: "", area: "", note: "", schedule_days: [] as number[], schedule_dates: "" });
  ...
  const scheduledInvoicesAll = allPlanInvoices.filter(i => i.scheduled_date === date);
  const divisionMatches = (value: unknown) => { const n = norm(value); return n.includes(norm(divisionMode)) || !n || n === "UNKNOWN"; };
  const invoices = scheduledInvoicesAll.filter(i => divisionMatches(i.division));
  const waitingAll = [...waitingInvoices, ...allPlanInvoices.filter(i => !i.scheduled_date && !waitingInvoices.some(w => w.id === i.id))];
  const waiting = waitingAll.filter(i => divisionMatches(i.division));
  const customers = (plan.customers || []).filter(c => divisionMatches(c.division));
  ...
  const loadedPallets = invoices.reduce((n, i) => n + Array.from({ length: Math.max(1, i.pallets) }).filter((_, k) => !!assignments[`${i.id}::${k}`]).length, 0);
  ...
  async function save() {
    ...
    try { await saveBulkOrganizerPlan({ ...plan, pallets: plan.invoices as any }); }
  ...
  const buildingForCustomer = (customerName: string) => {
    const schedule = (plan.customer_schedules || []).find(s => norm(s.customer_name) === norm(customerName) && divisionMatches(s.division));
  ...
  function addStore() {
    ...
    const b: Building = {
      id: `store:${crypto.randomUUID()}`, name, type: storeForm.type, custom_type: storeForm.type === "custom" ? storeForm.custom_type.trim() : undefined, area: storeForm.area.trim(), sort_order: (plan.buildings || []).length, enabled: true, note: storeForm.note.trim() || undefined,
      schedule_days: effectiveDays, schedule_dates: scheduleDates, division: divisionMode
    };
    const schedule: CustomerSchedule = { id: `schedule:${crypto.randomUUID()}`, customer_name: name, days: effectiveDays, area: b.area, division: divisionMode, building_id: b.id, enabled: true };
    ...
    setPlan(x => ({ ...x, buildings: [...(x.buildings || []), b], customers: existingCustomer ? (x.customers || []) : [...(x.customers || []), { id: `manual:${crypto.randomUUID()}`, name, area: b.area, division: divisionMode, enabled: true }], customer_schedules: [...(x.customer_schedules || []).filter(s => norm(s.customer_name) !== norm(name)), schedule] }));
    setStoreForm({ name: "", type: "store", custom_type: "", area: "", note: "", schedule_days: [], schedule_dates: "" });
    setStoreDatePicker("");
  }
  ...
  function addSchedule() {
    ...
    const s: CustomerSchedule = { id: `schedule:${crypto.randomUUID()}`, customer_name: scheduleForm.customer_name.trim(), days: scheduleForm.days, area: scheduleForm.area, division: scheduleForm.division || divisionMode, building_id: scheduleForm.building_id || null, enabled: true };
  ...
  async function addInvoice() {
    ...
    setDivisionMode(invoiceForm.division === "Consumer" ? "Consumer" : invoiceForm.division === "Pharma" ? "Pharma" : divisionMode);
    const inv: InvoicePlan = {
      id: `manual-invoice:${crypto.randomUUID()}`, invoice_no: invoiceForm.invoice_no.trim(), customer_id: customerId,
      customer_name: customer?.name || name, area: invoiceForm.area || customer?.area || "", division: invoiceForm.division || customer?.division || divisionMode,
  ...
  const buildingCards = (plan.buildings || []).filter(b => b.enabled && (!b.division || b.division === "Shared" || b.division === divisionMode));
  const allBuildingCards = (plan.buildings || []).filter(b => b.enabled);
  const dayName = new Date(`${date}T12:00:00`).toLocaleDateString("en-GB", { weekday: "long" });
  const loadedByVehicle = (vehicleId: string) => Object.entries(assignments).filter(([k,v]) => v === vehicleId && scheduledInvoicesAll.some(i => k.startsWith(`${i.id}::`))).length;
  const loadedByVehicleDivision = (vehicleId: string, division: "Pharma" | "Consumer") => Object.entries(assignments).filter(([k,v]) => v === vehicleId && scheduledInvoicesAll.some(i => i.division === division && k.startsWith(`${i.id}::`))).length;
  const crossDivisionLoad = (vehicleId: string) => ({ Pharma: loadedByVehicleDivision(vehicleId, "Pharma"), Consumer: loadedByVehicleDivision(vehicleId, "Consumer") });
  ...
  const customersForDay = useMemo(() => {
    ...
    const activeBuildingIds = new Set((plan.buildings || []).filter(b => b.enabled && (!b.division || b.division === "Shared" || b.division === divisionMode) && ((b.schedule_dates || []).includes(date) || (b.schedule_days || []).includes(todayDow))).map(b => b.id));
    (plan.customer_schedules || []).filter(s => s.enabled && divisionMatches(s.division) && (s.days.includes(todayDow) || activeBuildingIds.has(s.building_id || ""))).forEach(s => names.set(norm(s.customer_name), { id:`scheduled:${s.id}`, name:s.customer_name, area:s.area, division:s.division, enabled:true }));
  ...
  }, [customers, plan.customer_schedules, invoices, todayDow, divisionMode]);
    ...
    <div className="bulk-planner-header glass-card">
      <div className="bulk-dispatch-heading"><div><div className="bulk-planner-eyebrow"><Route size={14}/> BULK ORGANIZER · DAILY PLANNING</div><h1>Dispatch Allocation</h1><p>Select your default fleet, review today&apos;s scheduled customers, and link each customer&apos;s invoices to the right vehicle.</p></div>
        ...
        <div className="bulk-division-switch" role="tablist" aria-label="Business division">
          <button type="button" className={divisionMode === "Pharma" ? "active" : ""} onClick={() => setDivisionMode("Pharma")}>Pharma</button>
          <button type="button" className={divisionMode === "Consumer" ? "active" : ""} onClick={() => setDivisionMode("Consumer")}>Consumer</button>
        </div>
      </div>
            ...
            {[...buildingCards].sort((a,b)=>(a.sort_order ?? 9999)-(b.sort_order ?? 9999)).map(store=>{
              ...
              return <section key={store.id} className="bulk-store-customer-box" draggable onDragOver={e=>{if(e.dataTransfer.types.includes("application/x-bulk-location")){e.preventDefault();e.currentTarget.classList.add("bulk-location-drop-target")}}} onDragLeave={e=>e.currentTarget.classList.remove("bulk-location-drop-target")} onDrop={e=>{const source=e.dataTransfer.getData("application/x-bulk-location");if(!source)return;e.preventDefault();e.stopPropagation();e.currentTarget.classList.remove("bulk-location-drop-target");void reorderBuildings(source,store.id);}} onDragStart={e=>{e.dataTransfer.setData("text/plain",store.id);setDragged({type:"store",id:store.id})}} onDragEnd={()=>setDragged(null)} title="Drag this location to a vehicle to load its scheduled invoices">
                <div className="bulk-store-hero">
                  ...
                  <div className="bulk-store-heading"><span>{divisionMode.toUpperCase()} · {buildingTypeLabel(store).toUpperCase()}</span><h3>{store.name}</h3><small><MapPin size={12}/> {store.area||"Area not set"}</small></div>
