...
export default function BulkOrganizer() {
    ...
    try {
      ...
      savedInvoices.forEach((saved: any) => {
        if (!merged.some(x => x.id === saved.id || x.invoice_no === saved.invoice_no)) {
          merged.push({ ...saved, scheduled_date: saved.scheduled_date || null, vehicle_id: saved.vehicle_id || null });
        }
      });
  ...
  const invoices = allPlanInvoices.filter(i => i.scheduled_date === date);
  const waiting = [...waitingInvoices, ...allPlanInvoices.filter(i => !i.scheduled_date && !waitingInvoices.some(w => w.id === i.id))];
  ...
  function openInvoiceForStore(store: Building) {
    ...
    setInvoiceForm(x => ({ ...x, invoice_no: "", customer_id: "", customer_name: "", area: store.area || "", division: "", pallets: 1, invoice_date: date, schedule_date: "", building_id: store.id }));
  }
  ...
  async function addInvoice() {
    ...
    const inv: InvoicePlan = {
      ...
      scheduled_date: invoiceForm.schedule_date || null, vehicle_id: null, building_id: plan.customer_schedules?.find(s => norm(s.customer_name) === norm(name))?.building_id || buildingForCustomer(name)?.id || null
    };
  ...
  async function updateInvoice() {
    ...
    const next = { ...editingInvoice, invoice_no: invoiceForm.invoice_no, customer_name: invoiceForm.customer_name, area: invoiceForm.area, division: invoiceForm.division, pallets: Math.max(1, Number(invoiceForm.pallets || 1)), invoice_date: invoiceForm.invoice_date || null, scheduled_date: invoiceForm.schedule_date || null, building_id: plan.customer_schedules?.find(s => norm(s.customer_name) === norm(invoiceForm.customer_name))?.building_id || buildingForCustomer(invoiceForm.customer_name)?.id || editingInvoice.building_id || null };
        ...
        <section className="bulk-vehicles-section glass-card bulk-glass-surface">
          ...
          <div className={`bulk-vehicle-strip ${vehiclesExpanded?"expanded":""}`}>
            {selectedVehicles.map(cfg=>{const v=vehicleMap.get(cfg.vehicle_id);if(!v)return null;const cap=capacityFor(cfg,v),used=loadedByVehicle(v.id),pct=Math.min(100,Math.round(used/Math.max(1,cap)*100)),meta=plan.vehicle_meta?.[v.id]||{};return <div key={v.id} className={`bulk-vehicle-card ${pct>=100?"is-full":pct>=80?"is-near-full":""}`} onDragOver={e=>{e.preventDefault();e.currentTarget.classList.add("drop-ready")}} onDragLeave={e=>e.currentTarget.classList.remove("drop-ready")} onDrop={e=>{e.preventDefault();e.currentTarget.classList.remove("drop-ready");const raw=e.dataTransfer.getData("text/plain");const d=dragged||(raw?{type:"customer" as const,id:raw}:null);if(d?.type==="customer")dropCustomerOnVehicle(v,d.id);else if(d?.type==="invoice")dropOnVehicle(v,d);else if(d?.type==="store")dropStoreOnVehicle(v,d.id);setDragged(null)}}>
              ...
              <div className="bulk-vehicle-main"><div className="bulk-vehicle-top"><b>{v.number}</b><button className="btn icon-btn" title="Remove from day" onClick={(e)=>{e.stopPropagation();toggleVehicle(v)}}><X size={12}/></button></div><div className="bulk-capacity-row"><span>{used} / {cap} pallets</span><strong>{Math.max(0,cap-used)} free</strong></div><div className="bulk-capacity-bar"><i style={{width:`${pct}%`}}/></div><div className="bulk-vehicle-meta"><label>Driver<input list="bulk-driver-library" value={meta.driver||""} placeholder="Driver name or code" onChange={e=>updateVehicleMeta(v.id,"driver",e.target.value)} onBlur={e=>updateVehicleMeta(v.id,"driver",e.target.value)}/><button type="button" className="bulk-person-month-btn" onClick={()=>applyPersonForMonth(v.id,"driver")}>Use rest of month</button></label><label>Helper<input list="bulk-helper-library" value={meta.helper||""} placeholder="Helper name or code" onChange={e=>updateVehicleMeta(v.id,"helper",e.target.value)} onBlur={e=>updateVehicleMeta(v.id,"helper",e.target.value)}/><button type="button" className="bulk-person-month-btn" onClick={()=>applyPersonForMonth(v.id,"helper")}>Use rest of month</button></label></div><div className="bulk-vehicle-load-list">{invoicesInVehicle(v.id).map(i=><div key={i.id} className="bulk-loaded-invoice" draggable onDragStart={e=>{e.stopPropagation();e.dataTransfer.setData("text/plain",i.id);setDragged({type:"invoice",id:i.id})}} onDragEnd={()=>setDragged(null)}><div className="bulk-loaded-pallet"><Package size={12}/><b>{i.pallets}</b><span>PLT</span></div><div className="bulk-loaded-invoice-info"><b>{i.invoice_no} {invoiceLoaded(i) >= Number(i.pallets || 0) && <span className="bulk-invoice-complete" title="Invoice fully loaded"><Check size={10}/></span>}</b><span>{i.customer_name}</span><small>{i.pallets} pallets · {i.area||"Area not set"} · {i.division||"Division not set"}</small><small>Scheduled {i.scheduled_date}</small></div><div className="bulk-loaded-invoice-actions"><button className="btn icon-btn" title="Edit invoice" onClick={(e)=>{e.stopPropagation();editInvoice(i)}}><Pencil size={11}/></button><button className="btn icon-btn" title="Remove invoice from vehicle" onClick={(e)=>{e.stopPropagation();assignInvoice(i.id,null)}}><X size={12}/></button></div></div>)}{!invoicesInVehicle(v.id).length&&<span className="bulk-vehicle-empty">Drop invoice or customer here</span>}</div></div>
            </div>})}
            ...
            {buildingCards.map(store=>{
              ...
              return <section key={store.id} className="bulk-store-customer-box" draggable onDragStart={e=>{e.dataTransfer.setData("text/plain",store.id);setDragged({type:"store",id:store.id})}} onDragEnd={()=>setDragged(null)} title="Drag this store to a vehicle to load its scheduled invoices">
                ...
                <div className="bulk-store-schedule-sections"><div className="bulk-store-schedule-section today"><h4>Schedule for today</h4><div className="bulk-customer-grid compact-store-grid">
                  ...
                  {!storeCustomers.some(c=>storeInvoices.some(i=>norm(i.customer_name)===norm(c.name) && i.scheduled_date===date))&&<div className="bulk-empty-state">No invoices scheduled for today.</div>}
                </div></div><div className="bulk-store-schedule-section waiting"><h4>Waiting for schedule</h4><div className="bulk-store-schedule-invoices">{waiting.filter(i=>i.building_id===store.id).map(i=><div className="bulk-schedule-invoice-row" key={`waiting-${i.id}`}><div className="invoice-pallet-icon"><span>{i.pallets}</span></div><div><b>{i.invoice_no}</b><small>{i.customer_name} · {i.area||"No area"} · {i.division||"Division"}</small></div><em>Waiting</em></div>)}{!waiting.some(i=>i.building_id===store.id)&&<div className="bulk-empty-state">Nothing waiting for schedule.</div>}</div></div></div>
              </section>;
            })}
