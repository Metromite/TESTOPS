...
type Building = {
  ...
  area: string; enabled: boolean; note?: string; schedule_days?: number[]; schedule_dates?: string[];
};
...
type InvoicePlan = {
  ...
  division: string; pallets: number; invoice_date: string | null; scheduled_date: string; vehicle_id: string | null; building_id?: string | null;
};
...
export default function BulkOrganizer() {
  ...
  const [driverOptions, setDriverOptions] = useState<{code:string;name:string}[]>([]);
  ...
  const [storeForm, setStoreForm] = useState({ name: "", type: "store" as Building["type"], area: "", note: "", schedule_days: [] as number[], schedule_dates: "" });
  ...
  const [invoiceForm, setInvoiceForm] = useState({ invoice_no: "", customer_id: "", customer_name: "", area: "", division: "", pallets: 1, invoice_date: "", schedule_date: date, building_id: "" });
  ...
  async function loadSourceData(selectedDate: string) {
    ...
    try {
      ...
      const [v, a, p, f, allCustomers, drivers, helpers, historyRows, saved, savedDefaults] = await Promise.all([
        ...
        supabase.from("drivers").select("code,name,status").order("name"),
        supabase.from("helpers").select("code,name,status").order("name"),
        supabase.from("bulk_organizer_plans").select("plan_date,invoices").lte("plan_date", selectedDate).order("plan_date", { ascending: false }).limit(62),
      ...
      setDriverOptions((drivers.data ?? []).filter((x:any)=>String(x.status||"Active").toLowerCase() !== "inactive").map((x:any)=>({code:String(x.code||""),name:String(x.name||"")})).filter((x:any)=>x.name));
      setHelperOptions((helpers.data ?? []).filter((x:any)=>String(x.status||"Active").toLowerCase() !== "inactive").map((x:any)=>({code:String(x.code||""),name:String(x.name||"")})).filter((x:any)=>x.name));
      ...
      const sourceInvoices: InvoicePlan[] = (f.data ?? []).map((r: any, i: number) => ({
        ...
        invoice_date: r.invoice_date || null, scheduled_date: r.dispatch_date || "", vehicle_id: null,
      }));

      const old = (saved || {}) as PlanV3;
      const savedInvoices = (old.invoices || []).filter((x: any) => !x.scheduled_date || x.scheduled_date === selectedDate);
      const historyInvoices: any[] = (historyRows.data ?? []).flatMap((r:any) => Array.isArray(r.invoices) ? r.invoices : []).filter((x:any) => !x.scheduled_date);
      const scheduledHistory = new Set<string>((historyRows.data ?? []).flatMap((r:any) => Array.isArray(r.invoices) ? r.invoices : []).filter((x:any) => x.scheduled_date && x.scheduled_date >= selectedDate).map((x:any) => String(x.invoice_no || x.id)));
      const carryWaiting = [...savedInvoices, ...historyInvoices].filter((x:any) => !x.scheduled_date && !scheduledHistory.has(String(x.invoice_no || x.id)));
      const merged: InvoicePlan[] = sourceInvoices.map(src => {
        const s = savedInvoices.find((x: any) => x.invoice_no === src.invoice_no);
        return s ? { ...src, ...s, scheduled_date: s.scheduled_date || src.scheduled_date, customer_name: src.customer_name, area: src.area, division: src.division } : src;
      });
      // Keep manually-created/planning invoices that are not present in SAP facts.
      carryWaiting.forEach((saved: any) => {
        if (!merged.some(x => x.id === saved.id || x.invoice_no === saved.invoice_no)) {
          merged.push({ ...saved, scheduled_date: "", vehicle_id: null });
        }
      });
      savedInvoices.filter((x:any) => x.scheduled_date === selectedDate).forEach((saved: any) => {
        if (!merged.some(x => x.id === saved.id || x.invoice_no === saved.invoice_no)) {
          merged.push({ ...saved, scheduled_date: selectedDate, vehicle_id: saved.vehicle_id || null });
        }
      });
      ...
      const vehicleMeta: Record<string, any> = { ...(loadedDefaults as any).vehicle_meta || {} , ...(old.vehicle_meta || {}) };
      (loadedDefaults.vehicles || []).forEach((cfg:any) => {
        if (!cfg.vehicle_id) return;
        vehicleMeta[cfg.vehicle_id] = { ...(vehicleMeta[cfg.vehicle_id] || {}), driver: vehicleMeta[cfg.vehicle_id]?.driver || cfg.driver || "", helper: vehicleMeta[cfg.vehicle_id]?.helper || cfg.helper || "" };
      });
  ...
  function openInvoiceForStore(store: Building) {
    ...
    setInvoiceForm(x => ({ ...x, invoice_no: "", customer_id: "", customer_name: "", area: store.area || "", division: "", pallets: 1, invoice_date: date, schedule_date: date, building_id: store.id }));
  }
  ...
  async function saveDefaults() {
    ...
    try {
      ...
      const nextDefaults: DefaultsV2 = {
        ...
        vehicle_meta: { ...(defaultsRef.current as any).vehicle_meta || {}, ...(planRef.current.vehicle_meta || {}) },
      } as any;
  ...
  const buildingForCustomer = (customerName: string) => {
    const schedule = (plan.customer_schedules || []).find(s => norm(s.customer_name) === norm(customerName));
  ...
  function addStore() {
    ...
    if (!name) return;
    const b: Building = {
      id: `store:${crypto.randomUUID()}`, name, type: storeForm.type, area: storeForm.area.trim(), enabled: true, note: storeForm.note.trim() || undefined,
      schedule_days: storeForm.schedule_days, schedule_dates: storeForm.schedule_dates.split(",").map(x => x.trim()).filter(Boolean)
    };
    setDefaults(x => ({ ...x, buildings: [...x.buildings, b] }));
    setPlan(x => ({ ...x, buildings: [...(x.buildings || []), b] }));
    setStoreForm({ name: "", type: "store", area: "", note: "", schedule_days: [], schedule_dates: "" });
  }
  ...
  async function addInvoice() {
    ...
    if (!customer) { setError("Add the customer first, then attach the invoice to that customer."); return; }
    const customerId = customer.id;
    const linkedBuildingId = plan.customer_schedules?.find(s => norm(s.customer_name) === norm(name))?.building_id || buildingForCustomer(name)?.id || null;
    const inv: InvoicePlan = {
      ...
      scheduled_date: invoiceForm.schedule_date || "", vehicle_id: null, building_id: linkedBuildingId
    };
    ...
    setInvoiceForm({ invoice_no: "", customer_id: "", customer_name: "", area: "", division: "", pallets: 1, invoice_date: "", schedule_date: date, building_id: "" });
    setShowInvoiceManager(false);
  }

  function updateInvoice() {
    if (!editingInvoice) return;
    const next = { ...editingInvoice, invoice_no: invoiceForm.invoice_no, customer_name: invoiceForm.customer_name, area: invoiceForm.area, division: invoiceForm.division, pallets: Math.max(1, Number(invoiceForm.pallets || 1)), invoice_date: invoiceForm.invoice_date || null, scheduled_date: invoiceForm.schedule_date || "", building_id: plan.customer_schedules?.find(s => norm(s.customer_name) === norm(invoiceForm.customer_name))?.building_id || buildingForCustomer(invoiceForm.customer_name)?.id || editingInvoice.building_id || null };
    setPlan(x => ({ ...x, invoices: (x.invoices || []).map(i => i.id === editingInvoice.id ? next : i) }));
    setEditingInvoice(null);
  }

  function editInvoice(i: InvoicePlan) { setEditingInvoice(i); setInvoiceForm({ invoice_no: i.invoice_no, customer_id: i.customer_id, customer_name: i.customer_name, area: i.area, division: i.division, pallets: i.pallets, invoice_date: i.invoice_date || "", schedule_date: i.scheduled_date, building_id: i.building_id || buildingForCustomer(i.customer_name)?.id || "" }); }
  ...
  function canonicalPerson(value: string, options: {code:string;name:string}[]) {
    const n = norm(value);
    const hit = options.find(x => norm(x.name) === n || norm(x.code) === n);
    return hit?.name || value.trim();
  }
  function updateVehicleMeta(vehicleId: string, field: "driver" | "helper", value: string) {
    const options = field === "driver" ? driverOptions : helperOptions;
    const canonical = canonicalPerson(value, options);
    setPlan(x => ({ ...x, vehicle_meta: { ...(x.vehicle_meta || {}), [vehicleId]: { ...(x.vehicle_meta?.[vehicleId] || {}), [field]: canonical } } }));
  }
  function applyVehiclePerson(vehicleId: string, field: "driver" | "helper", scope: "day" | "month") {
    const meta = planRef.current.vehicle_meta?.[vehicleId] || {};
    const value = String(meta[field] || "").trim();
    if (!value) return;
    if (scope === "day") { updateVehicleMeta(vehicleId, field, value); return; }
    const nextDefaults: any = { ...defaultsRef.current, month_key: monthKey(date), vehicles: (defaultsRef.current.vehicles || []).map((v:any) => v.vehicle_id === vehicleId ? { ...v, [field]: value } : v), vehicle_meta: { ...((defaultsRef.current as any).vehicle_meta || {}), [vehicleId]: { ...(((defaultsRef.current as any).vehicle_meta || {})[vehicleId] || {}), [field]: value } } };
    setDefaults(nextDefaults);
    void saveBulkOrganizerDefaults(nextDefaults).catch((e:any)=>setError(e?.message || "Could not save monthly default."));
  }
  ...
  const loadedByVehicle = (vehicleId: string) => Object.entries(assignments).filter(([k,v]) => v === vehicleId && invoices.some(i => k.startsWith(`${i.id}::`))).length;
  ...
  const scheduledInvoices = useMemo(() => invoices.filter(i => i.scheduled_date === date), [invoices, date]);
  const waitingInvoices = useMemo(() => invoices.filter(i => !i.scheduled_date), [invoices]);
  const customersForDay = useMemo(() => {
    const names = new Map<string, any>();
    customers.forEach(c => names.set(norm(c.name), c));
    (plan.customer_schedules || []).filter(s => s.enabled && s.days.includes(todayDow)).forEach(s => names.set(norm(s.customer_name), { id:`scheduled:${s.id}`, name:s.customer_name, area:s.area, division:s.division, enabled:true }));
    scheduledInvoices.forEach(i => names.set(norm(i.customer_name), { id:i.customer_id, name:i.customer_name, area:i.area, division:i.division, enabled:true }));
    return Array.from(names.values());
  }, [customers, plan.customer_schedules, invoices, todayDow]);
  ...
  return <div className={`page bulk-planner-page bulk-theme-${bulkTheme} ${loading ? "is-hydrating" : ""}`}>
    <div className="bulk-planner-header glass-card">
      <div className="bulk-dispatch-heading"><div><div className="bulk-planner-eyebrow"><Route size={14}/> BULK ORGANIZER · DAILY PLANNING</div><h1>Dispatch Allocation</h1><p>Select your default fleet, review today&apos;s scheduled customers, and link each customer&apos;s invoices to the right vehicle.</p></div>
        <div className="bulk-planner-date bulk-planner-date-inline">
          ...
          <div><small>PLANNING DATE</small><strong>{dayLabel(date)}</strong></div>
    ...
    <div className="bulk-allocation-layout">
      <aside className="bulk-allocation-left">
        <div className="bulk-schedule-panel bulk-glass-surface"><div className="bulk-planner-section-head compact"><div><span>02 · ALERTS</span><h2>Today&apos;s schedule</h2></div><Bell size={17}/></div><div className="bulk-alert-list">{notifications.length?notifications.map(n=><div key={n.id} className={`bulk-alert ${n.tone}`}><div className="bulk-alert-icon">{n.tone==="green"?<Check size={13}/>:n.tone==="amber"?<CircleAlert size={13}/>:<Bell size={13}/>}</div><div><b>{n.title}</b><span>{n.text}</span></div></div>):<div className="bulk-empty-state">No scheduled alerts for this date.</div>}</div><div className="bulk-kpis"><div><span>Invoices</span><b>{scheduledInvoices.length}</b></div><div><span>Pallets</span><b>{scheduledInvoices.reduce((n,i)=>n+Number(i.pallets||0),0)}</b></div><div><span>Allocated</span><b>{loadedPallets}</b></div><div><span>Waiting</span><b>{pendingPallets}</b></div></div><div className="bulk-side-tools"><GlassButton variant="secondary" size="sm" onClick={()=>setShowCustomerManager(x=>!x)}><Users size={13}/> Customers</GlassButton><GlassButton variant="secondary" size="sm" onClick={()=>setShowScheduleManager(x=>!x)}><CalendarDays size={13}/> Schedules</GlassButton><GlassButton variant="secondary" size="sm" onClick={()=>setShowBuildingEditor(x=>!x)}><Store size={13}/> Stores</GlassButton><GlassButton size="sm" onClick={()=>{setEditingInvoice(null);setShowInvoiceManager(true);setInvoiceForm(x=>({...x,schedule_date:date,invoice_no:"",building_id:""}))}}><Plus size={13}/> Invoice</GlassButton></div></div>
      </aside>
        ...
        <section className="bulk-vehicles-section glass-card bulk-glass-surface">
          ...
          <div className={`bulk-vehicle-strip ${vehiclesExpanded?"expanded":""}`}>
            {selectedVehicles.map(cfg=>{const v=vehicleMap.get(cfg.vehicle_id);if(!v)return null;const cap=capacityFor(cfg,v),used=loadedByVehicle(v.id),pct=Math.min(100,Math.round(used/Math.max(1,cap)*100)),meta=plan.vehicle_meta?.[v.id]||{};return <div key={v.id} className={`bulk-vehicle-card ${pct>=100?"is-full":pct>=80?"is-near-full":""}`} onDragOver={e=>{e.preventDefault();e.currentTarget.classList.add("drop-ready")}} onDragLeave={e=>e.currentTarget.classList.remove("drop-ready")} onDrop={e=>{e.preventDefault();e.currentTarget.classList.remove("drop-ready");const raw=e.dataTransfer.getData("text/plain");const d=dragged||(raw?{type:"customer" as const,id:raw}:null);if(d?.type==="customer")dropCustomerOnVehicle(v,d.id);else if(d?.type==="invoice")dropOnVehicle(v,d);else if(d?.type==="store")dropStoreOnVehicle(v,d.id);setDragged(null)}}>
              ...
              <div className="bulk-vehicle-main"><div className="bulk-vehicle-top"><b>{v.number}</b><button className="btn icon-btn" title="Remove from day" onClick={(e)=>{e.stopPropagation();toggleVehicle(v)}}><X size={12}/></button></div><div className="bulk-capacity-row"><span>{used} / {cap} pallets</span><strong>{Math.max(0,cap-used)} free</strong></div><div className="bulk-capacity-bar"><i style={{width:`${pct}%`}}/></div><div className="bulk-vehicle-meta"><label>Driver<input list={`bulk-driver-list-${v.id}`} value={meta.driver||""} placeholder="Driver name / code" onChange={e=>updateVehicleMeta(v.id,"driver",e.target.value)} onBlur={e=>updateVehicleMeta(v.id,"driver",e.target.value)}/><datalist id={`bulk-driver-list-${v.id}`}>{driverOptions.map(x=><option key={`${x.code}-${x.name}`} value={x.name}>{x.code}</option>)}</datalist><div className="bulk-person-scope"><button type="button" onClick={()=>applyVehiclePerson(v.id,"driver","day")}>This day</button><button type="button" onClick={()=>applyVehiclePerson(v.id,"driver","month")}>Rest of month</button></div></label><label>Helper<input list={`bulk-helper-list-${v.id}`} value={meta.helper||""} placeholder="Helper name / code" onChange={e=>updateVehicleMeta(v.id,"helper",e.target.value)} onBlur={e=>updateVehicleMeta(v.id,"helper",e.target.value)}/><datalist id={`bulk-helper-list-${v.id}`}>{helperOptions.map(x=><option key={`${x.code}-${x.name}`} value={x.name}>{x.code}</option>)}</datalist><div className="bulk-person-scope"><button type="button" onClick={()=>applyVehiclePerson(v.id,"helper","day")}>This day</button><button type="button" onClick={()=>applyVehiclePerson(v.id,"helper","month")}>Rest of month</button></div></label></div><div className="bulk-vehicle-load-list">{invoicesInVehicle(v.id).map(i=><div key={i.id} className="bulk-loaded-invoice" draggable onDragStart={e=>{e.stopPropagation();e.dataTransfer.setData("text/plain",i.id);setDragged({type:"invoice",id:i.id})}} onDragEnd={()=>setDragged(null)}><div className="bulk-loaded-pallet"><Package size={12}/><b>{i.pallets}</b><span>PLT</span></div><div className="bulk-loaded-invoice-info"><b>{i.invoice_no} {invoiceLoaded(i) >= Number(i.pallets || 0) && <span className="bulk-invoice-complete" title="Invoice fully loaded"><Check size={10}/></span>}</b><span>{i.customer_name}</span><small>{i.pallets} pallets · {i.area||"Area not set"} · {i.division||"Division not set"}</small><small>Scheduled {i.scheduled_date}</small></div><div className="bulk-loaded-invoice-actions"><button className="btn icon-btn" title="Edit invoice" onClick={(e)=>{e.stopPropagation();editInvoice(i)}}><Pencil size={11}/></button><button className="btn icon-btn" title="Remove invoice from vehicle" onClick={(e)=>{e.stopPropagation();assignInvoice(i.id,null)}}><X size={12}/></button></div></div>)}{!invoicesInVehicle(v.id).length&&<span className="bulk-vehicle-empty">Drop invoice or customer here</span>}</div></div>
            </div>})}
...
<main className="bulk-customer-section glass-card bulk-glass-surface">
          ...
          <div className="bulk-schedule-split"><section className="bulk-schedule-lane waiting"><div className="bulk-schedule-lane-head"><div><span>WAITING FOR SCHEDULE</span><b>{waitingInvoices.length}</b></div><small>These invoices stay with you every day until a schedule date is assigned.</small></div>{waitingInvoices.map(i=><div className="bulk-waiting-invoice" key={i.id}><b>{i.invoice_no}</b><span>{i.customer_name}</span><small>{i.pallets} pallets · {i.area||"No area"} · waiting for schedule</small><button className="btn icon-btn" title="Schedule invoice" onClick={()=>editInvoice(i)}><CalendarDays size={12}/></button></div>)}{!waitingInvoices.length&&<div className="bulk-empty-state">No invoices waiting for schedule.</div>}</section><section className="bulk-schedule-lane scheduled"><div className="bulk-schedule-lane-head"><div><span>SCHEDULED TODAY</span><b>{scheduledInvoices.length}</b></div><small>Only invoices scheduled for {date} appear in today&apos;s customer load plan.</small></div></section></div>
          <div className="bulk-store-customer-grid">
            {buildingCards.map(store=>{
              ...
              const storeInvoices=scheduledInvoices.filter(i=>i.building_id===store.id || storeCustomers.some(c=>norm(c.name)===norm(i.customer_name)));
              ...
              return <section key={store.id} className="bulk-store-customer-box" draggable onDragStart={e=>{e.dataTransfer.setData("text/plain",store.id);setDragged({type:"store",id:store.id})}} onDragEnd={()=>setDragged(null)} title="Drag this store to a vehicle to load its scheduled invoices">
                <div className="bulk-store-hero">
                  <div className={`bulk-store-picture ${storeInvoices.length && storeInvoices.every(i => invoiceLoaded(i) >= Number(i.pallets || 0)) ? "store-complete" : ""}`}>{storeIcon(store.type)}{storeInvoices.length > 0 && storeInvoices.every(i => invoiceLoaded(i) >= Number(i.pallets || 0)) && <span className="bulk-store-complete-check" title="All store invoices are loaded into vehicles"><Check size={10}/></span>}</div>
                  <div className="bulk-store-heading"><span>{store.type.toUpperCase()}</span><h3>{store.name}</h3><small><MapPin size={12}/> {store.area||"Area not set"}</small></div>
                ...
                <div className="bulk-customer-grid compact-store-grid">
                  {storeCustomers.map(c=>{const ci=scheduledInvoices.filter(i=>norm(i.customer_name)===norm(c.name) && (i.building_id===store.id || !i.building_id));const pallets=ci.reduce((n,i)=>n+i.pallets,0),loaded=assignedForCustomer(c.name),scheduled=plan.customer_schedules?.find(s=>norm(s.customer_name)===norm(c.name)&&s.days.includes(todayDow));return <div key={c.id} draggable onDragStart={e=>{e.dataTransfer.setData("text/plain",c.name);setDragged({type:"customer",id:c.name})}} onDragEnd={()=>setDragged(null)} className={`bulk-customer-card ${loaded>=pallets&&pallets?"complete":""}`}>
                    ...
                    <div className="customer-card-invoices">{ci.map(i=>{const l=invoiceLoaded(i);return <div key={i.id} className="customer-invoice-row" draggable onDragStart={e=>{e.stopPropagation();e.dataTransfer.setData("text/plain",i.id);setDragged({type:"invoice",id:i.id})}} onDragEnd={()=>setDragged(null)}><div className="invoice-pallet-icon"><span>{i.pallets}</span></div><div><b>{i.invoice_no}</b><small>{i.scheduled_date} · {i.area||"No area"} · {i.division||"Division"}</small></div><em>{l}/{i.pallets}</em><button className="btn icon-btn invoice-edit-btn" title="Edit invoice" onClick={e=>{e.stopPropagation();editInvoice(i)}}><Pencil size={11}/></button></div>})}</div>
                    <div className="customer-card-footer"><span>{loaded>=pallets&&pallets?"FULLY LINKED":"Drag to a permitted vehicle"}</span>{loaded>=pallets&&pallets?<span className="bulk-customer-complete-check"><Check size={10}/></span>:<Route size={13}/>}</div>
                  </div>})}
                  {!storeCustomers.length&&<div className="bulk-empty-state"><Store size={22}/><b>No scheduled invoices for this store</b><span>Add an invoice for this store and its scheduled date will appear here.</span></div>}
                </div>
              </section>;
            })}
    ...
    {showBuildingEditor&&<div className="bulk-modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setShowBuildingEditor(false)}}><GlassCard className="bulk-planner-modal bulk-modal-surface"><div className="bulk-panel-title"><Store size={16}/> Stores & destinations <button onClick={()=>setShowBuildingEditor(false)}><X size={14}/></button></div><div className="bulk-modal-intro">Create reusable store categories with default weekdays, specific dates, area and operational notes.</div><div className="bulk-store-form"><input value={storeForm.name} placeholder="Store / hospital / destination name" onChange={e=>setStoreForm({...storeForm,name:e.target.value})}/><select value={storeForm.type} onChange={e=>setStoreForm({...storeForm,type:e.target.value as Building["type"]})}><option value="store">Store</option><option value="hospital">Hospital</option><option value="warehouse">Warehouse</option><option value="other">Other</option></select><input value={storeForm.area} placeholder="Area" onChange={e=>setStoreForm({...storeForm,area:e.target.value})}/><input value={storeForm.note} placeholder="Details / notes" onChange={e=>setStoreForm({...storeForm,note:e.target.value})}/><div className="bulk-store-days">{["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map((d,i)=><button type="button" key={d} className={storeForm.schedule_days.includes(i)?"active":""} onClick={()=>toggleStoreDay(i)}>{d}</button>)}</div><input value={storeForm.schedule_dates} placeholder="Specific dates, comma separated (YYYY-MM-DD)" onChange={e=>setStoreForm({...storeForm,schedule_dates:e.target.value})}/><GlassButton size="sm" onClick={addStore}><Plus size={13}/> Add Store</GlassButton></div><div className="bulk-store-list">{(plan.buildings||[]).map(b=><div className="bulk-store-row" key={b.id}><div className="bulk-store-icon">{storeIcon(b.type)}</div><div><b>{b.name}</b><span>{b.area||"Area not set"}{b.note?` · ${b.note}`:""}</span><small>{b.schedule_days?.length?b.schedule_days.map((d:number)=>["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][d]).join(" · "):"No default weekdays"}{b.schedule_dates?.length?` · ${b.schedule_dates.length} specific date(s)`:""}</small></div><button className="btn icon-btn" title="Remove store" onClick={()=>removeStore(b.id)}><Trash2 size={13}/></button></div>)}</div></GlassCard></div>}

    {(showCustomerManager||showScheduleManager||showInvoiceManager||editingInvoice||invoiceForm.invoice_no)&&<div className="bulk-modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget){setShowCustomerManager(false);setShowScheduleManager(false);setShowInvoiceManager(false);setEditingInvoice(null);setInvoiceForm(x=>({...x,invoice_no:""}))}}}><GlassCard className="bulk-planner-modal bulk-modal-surface"><div className="bulk-panel-title"><Users size={16}/> Data & scheduling <button onClick={()=>{setShowCustomerManager(false);setShowScheduleManager(false);setShowInvoiceManager(false);setEditingInvoice(null);setInvoiceForm(x=>({...x,invoice_no:""}))}}><X size={14}/></button></div><div className="bulk-modal-intro">Keep customer, schedule and invoice data in one clean workspace. Each invoice appears on its scheduled date.</div>{showCustomerManager&&<div className="bulk-modal-block"><h4>Add customer</h4><div className="bulk-builder-row"><input list="bulk-customer-library" value={customerForm.name} placeholder="Customer name" onChange={e=>setCustomerForm({...customerForm,name:e.target.value})}/><select value={customerForm.area} onChange={e=>setCustomerForm({...customerForm,area:e.target.value})}><option value="">Area</option>{areas.map(a=><option key={a.id} value={a.name}>{a.code} · {a.name}</option>)}</select><select value={customerForm.division} onChange={e=>setCustomerForm({...customerForm,division:e.target.value})}><option value="">Division</option><option>Pharma</option><option>Consumer</option></select><GlassButton size="sm" onClick={addCustomer}><Plus size={14}/> Add Customer</GlassButton></div></div>}{showScheduleManager&&<div className="bulk-modal-block"><h4>Customer schedule</h4><div className="bulk-schedule-editor"><input list="bulk-customer-library" value={scheduleForm.customer_name} placeholder="Customer" onChange={e=>setScheduleForm({...scheduleForm,customer_name:e.target.value})}/><select value={scheduleForm.area} onChange={e=>setScheduleForm({...scheduleForm,area:e.target.value})}><option value="">Area</option>{areas.map(a=><option key={a.id} value={a.name}>{a.code} · {a.name}</option>)}</select><select value={scheduleForm.division} onChange={e=>setScheduleForm({...scheduleForm,division:e.target.value})}><option value="">Division</option><option>Pharma</option><option>Consumer</option></select><select value={scheduleForm.building_id} onChange={e=>setScheduleForm({...scheduleForm,building_id:e.target.value})}><option value="">Store / destination</option>{(plan.buildings||[]).map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select><div className="bulk-day-picker">{["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map((d,i)=><button key={d} className={scheduleForm.days.includes(i)?"active":""} onClick={()=>setScheduleForm(x=>({...x,days:x.days.includes(i)?x.days.filter(y=>y!==i):[...x.days,i]}))}>{d}</button>)}</div><GlassButton size="sm" onClick={addSchedule}><Plus size={14}/> Save Schedule</GlassButton></div></div>}{(showInvoiceManager||editingInvoice||invoiceForm.invoice_no)&&<div className="bulk-modal-block"><h4>{editingInvoice?"Edit invoice":"Add invoice"}</h4><div className="bulk-invoice-form"><input value={invoiceForm.invoice_no} placeholder="Invoice no." onChange={e=>setInvoiceForm({...invoiceForm,invoice_no:e.target.value})}/><input list="bulk-customer-library" value={invoiceForm.customer_name} placeholder="Predictive customer" onChange={e=>{const name=e.target.value;const c=customers.find(x=>norm(x.name)===norm(name));const sched=plan.customer_schedules?.find(s=>norm(s.customer_name)===norm(name));setInvoiceForm({...invoiceForm,customer_name:name,customer_id:c?.id||"",building_id:sched?.building_id||buildingForCustomer(name)?.id||"",area:c?.area||invoiceForm.area,division:c?.division||invoiceForm.division})}}/><select value={invoiceForm.area} onChange={e=>setInvoiceForm({...invoiceForm,area:e.target.value})}><option value="">Area</option>{areas.map(a=><option key={a.id} value={a.name}>{a.code} · {a.name}</option>)}</select><select value={invoiceForm.division} onChange={e=>setInvoiceForm({...invoiceForm,division:e.target.value})}><option value="">Division</option><option>Pharma</option><option>Consumer</option></select><label className="bulk-field-with-label"><span>Pallets</span><input type="number" min={1} value={invoiceForm.pallets} onChange={e=>setInvoiceForm({...invoiceForm,pallets:Math.max(1,Number(e.target.value))})}/></label><label className="bulk-field-with-label"><span>Invoice date</span><input type="date" value={invoiceForm.invoice_date} onChange={e=>setInvoiceForm({...invoiceForm,invoice_date:e.target.value})}/></label><label className="bulk-field-with-label"><span>Schedule</span><select value={invoiceForm.schedule_date ? "scheduled" : "waiting"} onChange={e=>setInvoiceForm({...invoiceForm,schedule_date:e.target.value === "scheduled" ? date : ""})}><option value="waiting">Waiting for schedule</option><option value="scheduled">Scheduled</option></select></label><label className="bulk-field-with-label"><span>Schedule date</span><input type="date" value={invoiceForm.schedule_date} disabled={!invoiceForm.schedule_date} onChange={e=>setInvoiceForm({...invoiceForm,schedule_date:e.target.value})}/></label><GlassButton size="sm" onClick={editingInvoice?updateInvoice:addInvoice}>{editingInvoice?<Check size={14}/>:<Plus size={14}/>} {editingInvoice?"Update":"Add"} Invoice</GlassButton></div></div>}</GlassCard></div>}
