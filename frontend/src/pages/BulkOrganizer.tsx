...
export default function BulkOrganizer() {
  ...
  const [dragged, setDragged] = useState<{ type: "invoice" | "pallet" | "vehicle" | "customer"; id: string } | null>(null);
  ...
  // Defaults are also persistent without requiring a second Save click.
  useEffect(() => {
    if (loading || hydratedDateRef.current !== date) return;
    if (autoDefaultsTimer.current) window.clearTimeout(autoDefaultsTimer.current);
    autoDefaultsTimer.current = window.setTimeout(async () => {
      if (savingDefaultsRef.current) return;
      savingDefaultsRef.current = true;
      try { await saveBulkOrganizerDefaults(defaultsRef.current); }
      catch (e: any) { setError(e?.message || "Automatic defaults save failed"); }
      finally { savingDefaultsRef.current = false; }
    }, 650);
    return () => { if (autoDefaultsTimer.current) window.clearTimeout(autoDefaultsTimer.current); };
  }, [defaults, loading, date]);
    ...
    try {
      ...
      savedInvoices.forEach((saved: any) => {
        if (!merged.some(x => x.id === saved.id || x.invoice_no === saved.invoice_no)) {
          merged.push({ ...saved, scheduled_date: saved.scheduled_date || selectedDate, vehicle_id: saved.vehicle_id || null });
        }
      });
  ...
  async function saveDefaults() {
    ...
    try {
      ...
      setPlan(x => ({ ...x, buildings: result.buildings, customer_schedules: result.customer_schedules, vehicles: result.vehicles, vehicle_meta: { ...(x.vehicle_meta || {}) } }));
  ...
  async function addInvoice() {
    ...
    const inv: InvoicePlan = {
      ...
      scheduled_date: invoiceForm.schedule_date || date, vehicle_id: null, building_id: invoiceForm.building_id || plan.customer_schedules?.find(s => norm(s.customer_name) === norm(name))?.building_id || buildingForCustomer(name)?.id || null
    };
  ...
  function updateInvoice() {
    ...
    const next = { ...editingInvoice, invoice_no: invoiceForm.invoice_no, customer_name: invoiceForm.customer_name, area: invoiceForm.area, division: invoiceForm.division, pallets: Math.max(1, Number(invoiceForm.pallets || 1)), invoice_date: invoiceForm.invoice_date || null, scheduled_date: invoiceForm.schedule_date || date, building_id: invoiceForm.building_id || editingInvoice.building_id || null };
  ...
  return <div className={`page bulk-planner-page bulk-theme-${bulkTheme} ${loading ? "is-hydrating" : ""}`}>
    <div className="bulk-planner-header glass-card">
      <div><div className="bulk-planner-eyebrow"><Route size={14}/> BULK ORGANIZER · DAILY PLANNING</div><h1>Dispatch Allocation</h1><p>Select your default fleet, review today&apos;s scheduled customers, and link each customer&apos;s invoices to the right vehicle.</p></div>
    ...
    <div className="bulk-planner-date glass-card"><button className="btn icon-btn" onClick={()=>shiftDate(-1)}><ChevronLeft size={18}/></button><div><small>PLANNING DATE</small><strong>{dayLabel(date)}</strong></div><input type="date" value={date} onChange={e=>void changePlanningDate(e.target.value)}/><button className="btn icon-btn" onClick={()=>shiftDate(1)}><ChevronRight size={18}/></button><span className="bulk-live-pill"><span/> LIVE</span></div>

    <section className="bulk-vehicles-section glass-card">
      ...
      {showFleet&&<div className="bulk-fleet-panel"><div className="bulk-fleet-panel-head"><b>Select Fleet vehicles</b><span>{selectedVehicles.length} selected</span></div><div className="bulk-fleet-choice-grid">{fleet.map(v=>{const on=selectedVehicles.some(x=>x.vehicle_id===v.id),def=defaults.vehicles.some(x=>x.vehicle_id===v.id);return <button key={v.id} className={`bulk-fleet-choice-v2 ${on?"selected":""}`} onClick={()=>toggleVehicle(v)}><img src={vehicleImage(v)} alt=""/><div><b>{v.number}</b><span>{v.type||"Vehicle"} · {v.division||"Division"}</span></div><div className="fleet-choice-state">{on?<Check size={14}/>:<Plus size={14}/>} {def&&<small>DEFAULT</small>}</div></button>})}</div><div className="bulk-default-actions"><GlassButton variant="secondary" size="sm" onClick={()=>void saveDefaults()} disabled={savingDefaults}><Save size={13}/>{savingDefaults?"Saving…":"Save selected as monthly defaults"}</GlassButton></div></div>}
    </section>

    <div className="bulk-planner-grid">
      <aside className="bulk-schedule-panel glass-card"><div className="bulk-planner-section-head compact"><div><span>02 · ALERTS</span><h2>Today&apos;s schedule</h2></div><Bell size={17}/></div><div className="bulk-alert-list">{notifications.length?notifications.map(n=><div key={n.id} className={`bulk-alert glass-card ${n.tone}`}><div className="bulk-alert-icon">{n.tone==="green"?<Check size={13}/>:n.tone==="amber"?<CircleAlert size={13}/>:<Bell size={13}/>}</div><div><b>{n.title}</b><span>{n.text}</span></div></div>):<div className="bulk-empty-state">No scheduled alerts for this date.</div>}</div><div className="bulk-kpis"><div><span>Invoices</span><b>{invoices.length}</b></div><div><span>Pallets</span><b>{totalPallets}</b></div><div><span>Allocated</span><b>{loadedPallets}</b></div><div><span>Waiting</span><b>{pendingPallets}</b></div></div><div className="bulk-side-tools"><GlassButton variant="secondary" size="sm" onClick={()=>setShowCustomerManager(x=>!x)}><Users size={13}/> Customers</GlassButton><GlassButton variant="secondary" size="sm" onClick={()=>setShowScheduleManager(x=>!x)}><CalendarDays size={13}/> Schedules</GlassButton><GlassButton variant="secondary" size="sm" onClick={()=>setShowBuildingEditor(x=>!x)}><Store size={13}/> Stores</GlassButton><GlassButton size="sm" onClick={()=>{setEditingInvoice(null);setInvoiceForm(x=>({...x,schedule_date:date,invoice_no:"",building_id:""}))}}><Plus size={13}/> Invoice</GlassButton></div></aside>

      <main className="bulk-customer-section glass-card"><div className="bulk-planner-section-head"><div><span>03 · CUSTOMER LOAD PLAN</span><h2>Scheduled customers</h2><p>Drag a customer card onto a vehicle. The system links all scheduled invoices for that customer and checks vehicle permissions and remaining capacity.</p></div><div className="bulk-search"><Search size={14}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search customer / invoice…"/></div></div><div className="bulk-customer-grid">{customersForDay.filter(c=>!search||norm(c.name).includes(norm(search))||invoices.some(i=>norm(i.customer_name)===norm(c.name)&&norm(i.invoice_no).includes(norm(search)))).map(c=>{const ci=invoices.filter(i=>norm(i.customer_name)===norm(c.name)),pallets=ci.reduce((n,i)=>n+i.pallets,0),loaded=assignedForCustomer(c.name),scheduled=plan.customer_schedules?.find(s=>norm(s.customer_name)===norm(c.name)&&s.days.includes(todayDow));return <div key={c.id} draggable onDragStart={e=>{e.dataTransfer.setData("text/plain",c.name);setDragged({type:"customer",id:c.name})}} onDragEnd={()=>setDragged(null)} className={`bulk-customer-card ${loaded>=pallets&&pallets?"complete":""}`}><div className="customer-card-top"><div className="customer-avatar">{storeIcon(buildingForCustomer(c.name)?.type || "store")}</div><div><b>{c.name}</b><span>{c.area||"Area not set"} · {c.division||"Unknown"}</span></div><GripVertical size={16}/></div><div className="customer-card-details"><span><strong>{ci.length}</strong> invoices</span><span><strong>{pallets}</strong> pallets</span><span><strong>{loaded}</strong> linked</span></div>{scheduled&&<div className="customer-schedule-chip"><CalendarDays size={11}/> {scheduled.days.map((d: number)=>["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][d]).join(" · ")}</div>}<div className="customer-card-invoices">{ci.map(i=>{const l=invoiceLoaded(i);return <div key={i.id} className="customer-invoice-row" draggable onDragStart={e=>{e.stopPropagation();e.dataTransfer.setData("text/plain",i.id);setDragged({type:"invoice",id:i.id})}} onDragEnd={()=>setDragged(null)}><div className="invoice-pallet-icon"><span>{i.pallets}</span></div><div><b>{i.invoice_no}</b><small>{i.scheduled_date} · {i.area||"No area"} · {i.division||"Division"}</small></div><em>{l}/{i.pallets}</em><button className="btn icon-btn invoice-edit-btn" title="Edit invoice" onClick={e=>{e.stopPropagation();editInvoice(i)}}><Pencil size={11}/></button></div>})}</div><div className="customer-card-footer"><span>{loaded>=pallets&&pallets?"FULLY LINKED":"Drag to a permitted vehicle"}</span>{loaded>=pallets&&pallets?<Check size={13}/>:<Route size={13}/>}</div></div>})}{!customersForDay.length&&<div className="bulk-empty-state large"><Users size={30}/><b>No scheduled customers</b><span>Add a customer schedule or invoice for this date.</span></div>}</div></main>
    </div>

    {showBuildingEditor&&<GlassCard className="bulk-store-manager"><div className="bulk-panel-title"><Store size={16}/> Stores & destinations <button onClick={()=>setShowBuildingEditor(false)}><X size={14}/></button></div><div className="bulk-store-form"><input value={storeForm.name} placeholder="Store / hospital / destination name" onChange={e=>setStoreForm({...storeForm,name:e.target.value})}/><select value={storeForm.type} onChange={e=>setStoreForm({...storeForm,type:e.target.value as Building["type"]})}><option value="store">Store</option><option value="hospital">Hospital</option><option value="warehouse">Warehouse</option><option value="other">Other</option></select><input value={storeForm.area} placeholder="Area" onChange={e=>setStoreForm({...storeForm,area:e.target.value})}/><input value={storeForm.note} placeholder="Details / notes" onChange={e=>setStoreForm({...storeForm,note:e.target.value})}/><div className="bulk-store-days">{["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map((d,i)=><button type="button" key={d} className={storeForm.schedule_days.includes(i)?"active":""} onClick={()=>toggleStoreDay(i)}>{d}</button>)}</div><input value={storeForm.schedule_dates} placeholder="Specific dates, comma separated (YYYY-MM-DD)" onChange={e=>setStoreForm({...storeForm,schedule_dates:e.target.value})}/><GlassButton size="sm" onClick={addStore}><Plus size={13}/> Add Store</GlassButton></div><div className="bulk-store-list">{(plan.buildings || []).map(b=><div className="bulk-store-row" key={b.id}><div className="bulk-store-icon">{storeIcon(b.type)}</div><div><b>{b.name}</b><span>{b.area || "Area not set"}{b.note ? ` · ${b.note}` : ""}</span><small>{b.schedule_days?.length ? b.schedule_days.map(d=>["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][d]).join(" · ") : "No default weekdays"}{b.schedule_dates?.length ? ` · ${b.schedule_dates.length} specific date(s)` : ""}</small></div><button className="btn icon-btn" title="Remove store" onClick={()=>removeStore(b.id)}><Trash2 size={13}/></button></div>)}{!(plan.buildings || []).length&&<div className="bulk-empty-state">No stores or destinations configured yet.</div>}</div></GlassCard>}

    {(showCustomerManager||showScheduleManager||editingInvoice||invoiceForm.invoice_no)&&<GlassCard className="bulk-planner-modal"><div className="bulk-panel-title"><Users size={16}/> Data & scheduling <button onClick={()=>{setShowCustomerManager(false);setShowScheduleManager(false);setEditingInvoice(null);setInvoiceForm(x=>({...x,invoice_no:""}))}}><X size={14}/></button></div>{showCustomerManager&&<div className="bulk-builder-row"><input list="bulk-customer-library" value={customerForm.name} placeholder="Customer name" onChange={e=>setCustomerForm({...customerForm,name:e.target.value})}/><select value={customerForm.area} onChange={e=>setCustomerForm({...customerForm,area:e.target.value})}><option value="">Area</option>{areas.map(a=><option key={a.id} value={a.name}>{a.code} · {a.name}</option>)}</select><select value={customerForm.division} onChange={e=>setCustomerForm({...customerForm,division:e.target.value})}><option value="">Division</option><option>Pharma</option><option>Consumer</option></select><GlassButton size="sm" onClick={addCustomer}><Plus size={14}/> Add Customer</GlassButton></div>}{showScheduleManager&&<div className="bulk-schedule-editor"><input list="bulk-customer-library" value={scheduleForm.customer_name} placeholder="Customer" onChange={e=>setScheduleForm({...scheduleForm,customer_name:e.target.value})}/><select value={scheduleForm.area} onChange={e=>setScheduleForm({...scheduleForm,area:e.target.value})}><option value="">Area</option>{areas.map(a=><option key={a.id} value={a.name}>{a.code} · {a.name}</option>)}</select><select value={scheduleForm.division} onChange={e=>setScheduleForm({...scheduleForm,division:e.target.value})}><option value="">Division</option><option>Pharma</option><option>Consumer</option></select><div className="bulk-day-picker">{["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map((d,i)=><button key={d} className={scheduleForm.days.includes(i)?"active":""} onClick={()=>setScheduleForm(x=>({...x,days:x.days.includes(i)?x.days.filter(y=>y!==i):[...x.days,i]}))}>{d}</button>)}</div><GlassButton size="sm" onClick={addSchedule}><Plus size={14}/> Save Schedule</GlassButton></div>}<div className="bulk-invoice-form"><input value={invoiceForm.invoice_no} placeholder="Invoice no." onChange={e=>setInvoiceForm({...invoiceForm,invoice_no:e.target.value})}/><input list="bulk-customer-library" value={invoiceForm.customer_name} placeholder="Predictive customer" onChange={e=>{const name=e.target.value;const c=customers.find(x=>norm(x.name)===norm(name));const sched=plan.customer_schedules?.find(s=>norm(s.customer_name)===norm(name));setInvoiceForm({...invoiceForm,customer_name:name,customer_id:c?.id||"",building_id:sched?.building_id||buildingForCustomer(name)?.id||"",area:c?.area||invoiceForm.area,division:c?.division||invoiceForm.division})}}/><select value={invoiceForm.building_id} onChange={e=>setInvoiceForm({...invoiceForm,building_id:e.target.value})}><option value="">Store / destination</option>{(plan.buildings || []).map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select><select value={invoiceForm.area} onChange={e=>setInvoiceForm({...invoiceForm,area:e.target.value})}><option value="">Area</option>{areas.map(a=><option key={a.id} value={a.name}>{a.code} · {a.name}</option>)}</select><select value={invoiceForm.division} onChange={e=>setInvoiceForm({...invoiceForm,division:e.target.value})}><option value="">Division</option><option>Pharma</option><option>Consumer</option></select><input type="number" min={1} value={invoiceForm.pallets} onChange={e=>setInvoiceForm({...invoiceForm,pallets:Math.max(1,Number(e.target.value))})}/><input type="date" value={invoiceForm.invoice_date} onChange={e=>setInvoiceForm({...invoiceForm,invoice_date:e.target.value})}/><input type="date" value={invoiceForm.schedule_date} onChange={e=>setInvoiceForm({...invoiceForm,schedule_date:e.target.value})}/><GlassButton size="sm" onClick={editingInvoice?updateInvoice:addInvoice}>{editingInvoice?<Check size={14}/>:<Plus size={14}/>} {editingInvoice?"Update":"Add"} Invoice</GlassButton></div></GlassCard>}
