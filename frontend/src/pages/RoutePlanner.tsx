...
import { exportCombinedSheetExcel, generateCandidatePlan, generatePersistedPlan, getCombinedSheet, getCurrentPlan, getSheetLayout, getVacationStatus, reorderRouteSheet, saveSheetLayout, updatePlanRow } from "../services/routePlanner";
...
function CandidateReviewPanel({ syncDate }: { syncDate?: string | null }) {
  ...
  async function generate(dateOverride?: string) {
    ...
    setError("");
    try {
      const data: PlanResponse = await generateCandidatePlan(dateOverride ?? targetDate);
      ...
      await Promise.all([...codes].map(async (code) => {
        try {
          const s = await getVacationStatus(code, dateOverride ?? targetDate);
...
function EditRowModal({ row, onClose, onSaved }: { row: RouteAssignmentRow; onClose: () => void; onSaved: () => void }) {
  ...
  async function save(e: FormEvent) {
    e.preventDefault();
    try {
      await updatePlanRow(String(row.id), form);
      onSaved();
...
function PersistedPlanPanel({ role, onGenerated, onDataChanged }: {
  ...
  async function load() {
    try { setRows(await getCurrentPlan<RouteAssignmentRow>(role)); } catch (e: any) { setError(e.message); }
  }
  useEffect(() => { load(); }, [role]);
  useOperationalRealtime(`route-plan-${role}`, ["route_assignments", "vacations", "drivers", "helpers", "vehicles"], load);
  ...
  async function generate() {
    ...
    try {
      await generatePersistedPlan(role, targetDate);
      await load();
...
interface SheetLayoutResponse { columns: SheetColumnDef[]; hidden_columns: string[] }

function renderCellValue(row: CombinedSheetRow, key: string): React.ReactNode {
...
function CombinedSheetPanel({ refreshToken }: { refreshToken: number }) {
  ...
  async function load() {
    setLoading(true);
    try { setRows(await getCombinedSheet<CombinedSheetRow>()); } catch (e: any) { setError(e.message); } finally { setLoading(false); }
  }
  async function loadLayout() {
    try {
      const layout = await getSheetLayout<SheetLayoutResponse>();
      setColumns(layout.columns.map((c) => ({ key: c.key, label: c.label, width: c.width })));
  ...
  useOperationalRealtime("route-sheet", ["route_assignments"], load);

  async function persistLayout(cols: SheetColumnDef[], hiddenSet: Set<string>) {
    try {
      await saveSheetLayout({
  ...
  async function handleRowDrop(dropIndex: number) {
    ...
    try {
      await reorderRouteSheet(next.map((r) => r.area_code));
    } catch (e: any) { setError(e.message); }
  }
  ...
  async function restoreDefaults() {
    try {
      ...
      const layout = await saveSheetLayout<SheetLayoutResponse>({ column_order: [], hidden_columns: [], column_widths: {} });
      setColumns(layout.columns.map((c) => ({ key: c.key, label: c.label, width: c.width })));
      setHidden(new Set<string>(layout.hidden_columns));
    } catch (e: any) { setError(e.message); }
  }

  function downloadExcel() { exportCombinedSheetExcel().catch((error) => setError(error instanceof Error ? error.message : String(error))); }
                ...
                <tr key={`${r.area_code}-${r.sector}`} draggable onDragStart={() => handleRowDragStart(i)} onDragOver={handleRowDragOver} onDrop={() => handleRowDrop(i)}>
                  ...
                  {visibleColumns.map((c) => (
                    <td key={c.key} style={c.key === "assignment_reason"
                      ...
                      {CELL_RENDERERS[c.key] ? CELL_RENDERERS[c.key](r) : renderCellValue(r, c.key)}
                    </td>
                  ))}
                </tr>
              ))}
