...
import { addConfigRule, addConsumerSalesman, deleteConfigRule, deleteConsumerSalesman, getConfigRules, getConsumerSalesmen, getSalesmanCandidates } from "../services/operational";
import { refreshExperienceClassification } from "../services/importEngine";
...
export default function DashboardConfigTab() {
  ...
  async function load() {
    try {
      const res = await getConfigRules<RulesResponse>();
      setData(res);
  ...
  async function deleteRule(id: number) {
    try { await deleteConfigRule(String(id)); load(); } catch (e: any) { setError(e.message); }
  }
...
function SalesmanCategorization() {
  ...
  async function loadAssigned() {
    try { setAssigned(await getConsumerSalesmen<{ id: number; name: string; assigned_by: string }>()); } catch (e: any) { setError(e.message); }
  }
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(() => {
      getSalesmanCandidates<{ name: string; is_consumer: boolean }>(query).then((c) => { setCandidates(c); setHighlighted(0); }).catch(() => {});
    }, 200);
  ...
  async function assign(name: string) {
    try {
      await addConsumerSalesman(name);
      await refreshExperienceClassification();
      setQuery(""); setCandidates([]); loadAssigned();
    } catch (e: any) { setError(e.message); }
  }
  async function unassign(id: number) {
    try { await deleteConsumerSalesman(String(id)); await refreshExperienceClassification(); loadAssigned(); } catch (e: any) { setError(e.message); }
  }
