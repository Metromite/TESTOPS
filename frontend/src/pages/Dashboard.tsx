...
export default function Dashboard() {
  ...
  const [vehicleTypeOptions, setVehicleTypeOptions] = useState<string[]>(
    cachedFilterMeta?.vehicle_types?.length ? cachedFilterMeta.vehicle_types : []
  );
  const [routeTypeOptions, setRouteTypeOptions] = useState<string[]>(
    cachedFilterMeta?.route_types?.length ? cachedFilterMeta.route_types : []
  );
  ...
  return (
    <div className="page dashboard-page dashboard-shell">
      <GlassHeader
        eyebrow="Dispatch Operations"
        title="Dashboard"
        actions={
          <div className="dashboard-header-actions flex flex-wrap items-center justify-end gap-2">
            <div className="dashboard-refresh-control flex items-center gap-1.5 text-[12px] text-muted">
            ...
            <div className="relative" onMouseLeave={() => setExportMenuOpen(false)}>
              ...
              <div className="dashboard-export-menu absolute right-0 top-full z-[300] mt-2 min-w-[250px] max-w-[calc(100vw-28px)] rounded-[16px] border border-[var(--glass-border)] bg-[var(--glass-bg-3)] p-1.5 shadow-elevation2 backdrop-blur-[var(--glass-blur-3)] backdrop-saturate-[200%]" style={{ display: exportMenuOpen ? "block" : "none", boxShadow: "0 18px 50px rgba(0,0,0,.35)" }}>
                {[
      ...
      <GlassFilterBar className="mb-3">
        ...
        <div className="dashboard-date-filter flex flex-wrap items-center gap-1.5">
