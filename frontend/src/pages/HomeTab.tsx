 ...
 */
import { useHomeData, METRIC_COLUMNS, DEFAULT_COLUMNS } from "../hooks/useHomeData";
...
export default function HomeTab({ driver, onSelectDriver, onDriverListLoaded, globalFilters }: Props) {
  ...
  // The full Overview response is now a compact server-side RPC, so a
  // second parallel KPI request is no longer necessary.
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
        {HOME_KPI_DEFS.map((def) => kpis ? (
          <KpiTile key={def.key} icon={def.icon} label={def.label} value={def.getValue(kpis)}
            onClick={def.metric ? () => openDetail(def.metric!, def.label) : undefined} />
        ) : (
