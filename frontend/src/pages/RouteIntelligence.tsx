...
import { getRouteDriverDates, reconstructRoute } from "../services/operational";
...
export default function RouteIntelligence() {
  ...
  useEffect(() => {
    getRouteDriverDates<DriverDate>().then(setOptions).catch((e) => setError(e.message));
  }, []);
  ...
  async function load(driverDate: string) {
    ...
    try {
      const data = await reconstructRoute<Reconstruction>(driver_name, date);
      setResult(data);
    } catch (e: any) {
      setError(e.message);
    }
