...
import { applyDriverMapping, getDriverMappingReview, resetDriverMappings } from "../services/operational";
...
export default function DriverMappingReviewTab() {
  ...
  async function load() {
    try { setData(await getDriverMappingReview<ReviewData>()); } catch (e: any) { setError(e.message); }
  }
  useEffect(() => { load(); }, []);

  async function apply(vehicle_key: string, sap_driver: string) {
    try {
      await applyDriverMapping(vehicle_key, sap_driver);
      load();
    } catch (e: any) { setError(e.message); }
  }

  async function resetAll() {
    try { await resetDriverMappings(); load(); } catch (e: any) { setError(e.message); }
  }
