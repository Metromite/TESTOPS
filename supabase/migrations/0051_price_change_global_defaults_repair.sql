-- Repair global Price Change default areas and expose them consistently.
-- This is intentionally separate from campaign-specific area history.
...
create unique index if not exists ux_price_change_default_area_name on public.price_change_default_areas(lower(trim(area_name)));
...
select trim(ca.area_name), min(ca.sort_order), true
from public.price_change_campaign_areas ca
where ca.active and nullif(trim(ca.area_name),'') is not null
group by lower(trim(ca.area_name)), trim(ca.area_name)
on conflict ((lower(trim(area_name)))) do update
set active=true, sort_order=excluded.sort_order, updated_at=now();

update public.price_change_default_areas d
set active=false, updated_at=now()
where lower(trim(d.area_name))='d'
  and not exists (
    select 1 from public.price_change_campaign_areas ca
    where ca.active and lower(trim(ca.area_name))=lower(trim(d.area_name))
  );
...
begin
 ...
 if nullif(trim(coalesce(p_row->>'area_name','')),'') is null then raise exception 'Area name is required'; end if;
 if nullif(p_row->>'id','') is null then
   insert into public.price_change_default_areas(area_name,sort_order,active)
   values(trim(p_row->>'area_name'),coalesce(nullif(p_row->>'sort_order','')::integer,0),true)
   on conflict ((lower(trim(area_name)))) do update set active=true,sort_order=excluded.sort_order,updated_at=now()
   returning * into r;
 else
   update public.price_change_default_areas
   set area_name=trim(p_row->>'area_name'),sort_order=coalesce(nullif(p_row->>'sort_order','')::integer,0),active=coalesce((p_row->>'active')::boolean,true),updated_at=now()
...
begin
 ...
 select jsonb_build_object(
  ...
  'campaign_areas',coalesce((select jsonb_agg(a order by a.campaign_id,a.sort_order,a.area_name) from price_change_campaign_areas a where p_campaign_id is null or a.campaign_id=p_campaign_id),'[]'::jsonb),
...
begin
 ...
 select jsonb_build_object(
  ...
  'campaigns',coalesce((select jsonb_agg(jsonb_build_object(
    'campaign',c,
    'areas',coalesce((select jsonb_agg(da order by da.sort_order,da.area_name) from price_change_default_areas da where da.active),'[]'::jsonb),
