-- Price Change V4 submission rules:
-- 1) Customer name, location and pharmacist are mandatory.
-- 2) A pharmacy/place photo is mandatory on every saved visit (existing photo may be reused when editing).
-- 3) Driver must enter at least one positive quantity OR explicitly select NO_STOCK.
-- 4) Server time remains authoritative; drivers cannot provide visit time.
...
begin
 ...
 if coalesce(trim(p_visit->>'location'),'')='' and not locked then raise exception 'Location is required'; end if;
 if locked and coalesce(trim(assigned_location),'')='' then raise exception 'Admin locked location is empty'; end if;
 ...
 select attachment_path into existing_attachment from price_change_customer_visits where id=vid and driver_id=did;
