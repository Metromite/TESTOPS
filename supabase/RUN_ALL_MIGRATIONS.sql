...
create policy allow_app_access_bulk_organizer_plans on public.bulk_organizer_plans for all to anon, authenticated using (true) with check (true);
create index if not exists ix_bulk_organizer_plans_date on public.bulk_organizer_plans(plan_date);
notify pgrst, 'reload schema';

-- 0050: Bulk Organizer vehicle capacity
